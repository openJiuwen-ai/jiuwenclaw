...
class TreeRepairEngine:
    ...
    def postprocess_tree(self, root: TreeNode, verbose: bool = False) -> None:
        ...
        for repair_pass in range(1, self._builder._postprocess_max_passes + 1):
        ...
        if total_reassignments > 0:
            console.print(Panel(
                f"[bold green]Post-process repaired {total_reassignments} misplaced skill assignments.[/bold green]",
    ...
    def rebalance_child_assignments(self, node: TreeNode, verbose: bool = False) -> int:
        ...
        if len(skill_entries) < builder._postprocess_min_skills:
            return 0

        assignments = builder._classify_skills(skill_entries, groups, verbose=False)
        assignments = builder._validate_and_recover(skill_entries, groups, assignments, verbose=False)
        ...
        for skill_id, target_child_id in moves:
            ...
            builder._insert_skill_into_subtree(target_child, skill_data)

        removed_empty = builder._prune_empty_children(node)
        reassigned_tiny = builder._repair_small_leaf_children(node)
        if removed_empty or reassigned_tiny:
            builder._prune_empty_children(node)
            if len(node.children) >= 2:
                builder._rewrite_node_label_after_singleton(
                    node,
                    builder._existing_child_groups(node.children),
                    verbose=verbose,
                )
    ...
    def collect_subtree_skill_locations(self, node: TreeNode) -> list[tuple[TreeNode, dict]]:
        if node.is_leaf:
            return [(node, self._builder._skill_to_data(skill)) for skill in node.skills]
    ...
    def normalize_to_equivalence_groups(self, root: TreeNode, verbose: bool = False) -> None:
        ...
        if verbose and split_count > 0:
            console.print(f"[dim]  Equivalence regrouping updated {split_count} second-leaf nodes under '{root.id}'[/dim]")
    ...
    def split_second_leaf_node_into_equiv_groups(
        ...
        groups = builder._discover_equivalence_groups(second_leaf_node, leaf_children, verbose=verbose)
        if not groups:
            return [second_leaf_node]
        normalized_groups = builder._normalize_equivalence_groups(leaf_children, groups)
        ...
        for idx, group in enumerate(normalized_groups, start=1):
            base_id = builder._build_equivalence_group_id(
        ...
        if verbose:
            console.print(f"[dim]  Split '{second_leaf_node.id}' into {len(replacement_nodes)} equivalence groups[/dim]")
