...
class TreeGroupingEngine:
    ...
    def sorted_skills(self, skills: list[dict]) -> list[dict]:
        ...
        items = list(skills)
        if not builder._deterministic_prompts:
            return items
    ...
    def build_groups_from_assignments(self, groups: dict, assignments: dict) -> dict:
        ...
        for group_id, payload in self.iter_group_items(groups):
            ...
            if self._builder._deterministic_prompts:
    ...
    def classify_skills_single(
        ...
        response = self._builder._call_llm_json(prompt, is_retry=is_retry)
    ...
    def validate_and_recover(
        ...
        if missing:
            console.print(
                Panel(
                    f"[bold red]{len(missing)}/{len(skills)} skills were placed into fallback group '{fallback_group_id}'.[/bold red]",
    ...
    def discover_groups(self, skills: list[dict], parent_context: Optional[dict], verbose: bool = False) -> dict:
        ...
        response = builder._call_llm_json(prompt)
