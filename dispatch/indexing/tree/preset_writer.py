# pylint: disable=line-too-long,add-staticmethod-or-classmethod-decorator
...
class TreePresetWriter:
            ...
            nodes.append(
                {
                    ...
                    "keywords": self.extract_keywords(branch_id, branch_name, branch_description, branch_select_when, branch_dont_select_when),
                    "examples": [],
    ...
    def compact_leaf_cid_seed(self, *, worker_id: str, display_name: str, old_term: str) -> str:
    ...
    def cid_term(self, value: str, fallback: str = "Node") -> str:
        raw = str(value or "")
    ...
    def build_tree_sketch(self, nodes: list[dict]) -> str:
        ...
        def render_branch(cid: str, indent: int) -> None:
            ...
            if child_branches:
                details.append(
                    "child_branches="
                    + ", ".join(str(item.get("cid", "")).split(".")[-1] for item in child_branches[:5] if item.get("cid"))
                )
            if child_leaves:
                details.append(
    ...
    def slug_term(self, value: str, fallback: str = "node") -> str:
    ...
    def extract_keywords(self, *values: str, limit: int = 8) -> list[str]:
        seen: set[str] = set()
        result: list[str] = []
        for value in values:
    ...
    def _branch_label(self, node_dict: dict, skill_count: int) -> str:
    ...
    def count_skills_in_dict(self, node_dict: dict) -> int:
        total = 0
        stack = [node_dict]
        while stack:
            current = stack.pop()
