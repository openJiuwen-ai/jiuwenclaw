...
from .prompts import (
    GROUP_MERGE_PROMPT,
    GROUP_DISCOVERY_PROMPT, SKILL_ASSIGNMENT_PROMPT,
...
class TreeBuilder:
    ...
    def __init__(
        ...
        self._client = client if client is not None else (OpenAI(api_key=self.api_key, base_url=self.base_url) if OpenAI is not None else None)
    ...
    def _generate_skill_profiles(self, skills: list[dict], *, verbose: bool = False) -> dict[str, dict[str, str]]:
        ...
        for skill_id, payload in raw_profiles.items():
            ...
            profiles[normalized_id] = {
                "description": self._compact_profile_field(payload.get("description"), limit=self._skill_profile_description_limit),
                "select_when": self._compact_profile_field(payload.get("select_when"), limit=self._skill_profile_rule_limit),
                "dont_select_when": self._compact_profile_field(payload.get("dont_select_when"), limit=self._skill_profile_rule_limit),
            }
        return profiles
    ...
    def _apply_skill_profile(self, skill: dict, profile: dict[str, str] | None) -> dict:
        ...
        description = self._compact_profile_field(profile.get("description"), limit=self._skill_profile_description_limit)
        if not description:
            return self._with_fallback_skill_profile(skill)

        select_when = self._compact_profile_field(profile.get("select_when"), limit=self._skill_profile_rule_limit)
        dont_select_when = self._compact_profile_field(profile.get("dont_select_when"), limit=self._skill_profile_rule_limit)
    ...
    def _compose_skill_routing_description(self, *, description: str, select_when: str = "", dont_select_when: str = "") -> str:
        ...
        if self._skill_profile_select_rules_enabled and select_when:
            parts.append(f"Select when: {self._compact_profile_field(select_when, limit=self._skill_profile_rule_limit)}")
        if self._skill_profile_select_rules_enabled and dont_select_when:
            parts.append(f"Don't select when: {self._compact_profile_field(dont_select_when, limit=self._skill_profile_rule_limit)}")
    ...
    def _audit_tree(self, root: TreeNode, input_skills: list[dict]) -> None:
        ...
        console.print(
            Panel(
                "\n".join(
                    [
                        f"[bold red]Recovered {len(recovered_payloads)} missing skills after tree construction.[/bold red]",
    ...
    def _skill_from_data(self, skill_data: dict, *, path: str) -> Skill:
    ...
    def _build_routing_policy(self, nodes: list[dict]) -> str:
        root_entries = sorted([item for item in nodes if "." not in str(item.get("cid", ""))], key=lambda item: str(item.get("cid", "")))
        lines = [
            "Route by descending the node tree one level at a time.",
            "Treat a user request as potentially multi-step unless the latest observation already fully satisfies every explicit requirement.",
            "Prefer leaves whose descriptions best match the next unmet sub-problem in the user request.",
            "After a worker returns, check whether unmet requirements still remain; if they do, continue routing instead of finishing early.",
            "Do not jump to User.Final after a single worker call when the user asked for multiple actions, dependencies, or deliverables.",
            "Use worker observations as intermediate state: one skill may gather facts or create prerequisites for a later skill.",
            "When multiple branches overlap, use the child descriptions as the local decision surface.",
            "Choose User.Final only when the latest observation set is sufficient to answer the whole user request, not just one subtask.",
        ]
