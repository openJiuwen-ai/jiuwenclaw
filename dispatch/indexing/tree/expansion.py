# pylint: disable=line-too-long,protected-access,add-staticmethod-or-classmethod-decorator
...
class TreeExpansionEngine:
    ...
    def process_node(
        ...
        *,
        ...
        if configured_groups:
            ...
            grouped = builder._split_skills(skills, parent_context, verbose)
    ...
    def _update_progress(self, *, node_id: str, skill_count: int, depth: int) -> None:
        ...
        if builder._progress and builder._progress_task is not None:
            builder._progress.update(
                builder._progress_task,
    ...
    def _emit_leaf(self, node: TreeNode, skills: list[dict], *, verbose: bool) -> None:
        self._builder._assign_skills_to_leaf(node, skills)
        if verbose:
    ...
    def _classify_configured_groups(
        ...
        *,
        ...
        initial = builder._classify_skills(skills, groups, verbose)
    ...
    def create_child_node(self, *, parent: TreeNode, group_id: str, group_data: dict, depth: int) -> TreeNode:
        child_node = TreeNode(
            id=group_id,
    ...
    def build_children_from_groups(
        ...
        for group_id, group_data in groups.items():
            ...
            if not child_skills:
                continue
            if len(child_skills) < 2 and not builder._equiv_allow_singleton_groups:
                if len(child_skills) == 1:
                    singleton_triggered = True
                if verbose:
                    skipped_ids = [s["id"] for s in child_skills]
                    console.print(
                        f"[dim]  Skipping singleton group '{group_id}' ({skipped_ids}), will reassign into existing groups[/dim]"
                    )
                continue
        ...
        if skill_map:
            if children_to_process:
                ...
                builder._assign_skills_to_leaf(node, skills)
                return []
    ...
    def reassign_skills_to_children(
        ...
        assignments = builder._classify_skills_single(
            builder._sorted_skills(unassigned_skills),
            groups,
            verbose=False,
            is_retry=True,
        )
    ...
    def rewrite_node_label_after_singleton(
        ...
        result = builder._call_llm_json(prompt)
    ...
    def existing_child_groups(self, children: list[TreeNode]) -> list[ChildGroup]:
        builder = self._builder
        return [
            ChildGroup(node=child, skills=builder._collect_subtree_skill_dicts(child))
