...
## 在 jiuwenavatar 中通过配置文件启用 jiuwenbox

jiuwenavatar 通过 `config.yaml` 的 `sandbox` 段决定**是否启用沙箱、连接哪台 jiuwenbox、是否自己拉起 jiuwenbox 子进程、用哪个 policy**。一般用 TUI 的 `/sandbox` 命令操作时会自动落盘到这里，但也可以提前在 `config.yaml` 里手写。
...
| `sandbox.type` | 字符串 | `jiuwenbox` | sandbox provider 名。当前 jiuwenavatar 只接通了 `jiuwenbox` |
| `sandbox.startup_mode` | `internal` / `external` | `internal` | `internal`：agent-server 启动时自动 spawn `jiuwenbox-server` 子进程并落盘最终生效的 `url`（端口被占用时自动换端口）；`external`：jiuwenavatar 完全不碰 jiuwenbox 进程，要求按本 README 顶部的方式提前自己启动 |
...
适合需要把 jiuwenbox 跑在独立机器、容器里，或者 jiuwenavatar 进程不便用 root 的场景。
...
跨机部署要求 jiuwenbox 主机能访问 jiuwenavatar 的固有 agent 文件路径——`preserve_file_sharing_mode` 现在只支持 `mount` jiuwenavatar 会把 intrinsic 文件（`AGENT.md` / `HEARTBEAT.md` / `IDENTITY.md` / `SOUL.md` / `USER.md` / `memory/daily_memory/`）和 `project_dir` 通过 bind mount 暴露给沙箱，因此目标主机必须能在同样的 host path 下看到这些文件（例如共享文件系统、容器 volume 等）。
