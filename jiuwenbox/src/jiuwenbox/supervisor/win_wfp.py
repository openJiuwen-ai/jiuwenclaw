...
def _add_sublayer(engine: wintypes.HANDLE, subLayerKey: str, weight: int) -> Guid:
    ...
    if hr != 0 and hr != 0x80320009 and hr != 0x800700B7:
...
def _build_ale_user_condition(sandbox_user_sid: str) -> "tuple[FwpmFilterCondition0, object]":
    ...
    # 用 win32security 构造自相关 SD. 延迟 import
    try:
...
def _add_filter(  # pylint: disable=huawei-too-many-arguments
    ...
    **kwargs: object,
    ...
    if hr != 0 and hr != 0x80320009 and hr != 0x800700B7:  # FWP_E_ALREADY_EXISTS (WFP 段 / Win32 段)
        try:
            def _ptr_val(p):
...
def install_wfp_filters(
    ...
    try:
        uninstall_wfp_filters()
    except Exception:  # noqa: BLE001
        logger.warning("install 前清旧 filter 异常, 继续装 (sublayer 已存在会被幂等忽略)", exc_info=True)
