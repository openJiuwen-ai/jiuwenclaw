...
def _add_sublayer(engine: wintypes.HANDLE, subLayerKey: str, weight: int) -> Guid:
    ...
    sublayer.flags = 0  # noqa: N815 - Win32 SDK 字段名
    sublayer.weight = weight  # noqa: N815 - Win32 SDK 字段名 # 字段已是 c_uint16, 直接赋 int
...
def uninstall_wfp_filters(
    ...
    review #1: 改用枚举法 — FwpmFilterEnum0 枚举 JBX sublayer 下全部 filter,
    按 filterKey 逐个删除, 不依赖端口范围. 解决 install 用非默认端口范围时
    旧版按默认范围枚举删不到致 Permit filter 残留、sublayer FWP_E_IN_USE
    删不掉的问题.

    ``permit_port_start/end`` 参数保留 (向后兼容 win_setup.uninstall 现有调用),
    内部不再使用, 枚举法自动覆盖任意端口范围.
    """
    _require_windows()
    fwpu = _get_fwpuclnt()
    engine = _open_engine()
    try:
        # 枚举 JBX sublayer 下所有 filter, 逐个按 key 删除 (覆盖任意端口范围).
        ...
        if enum_keys:
            ...
            # 枚举为空 (sublayer 不存在 / 无 filter), 回退按固定 key 删 (兼容
            # 旧版残留 + 不依赖枚举成功). 含 Block V4/V6 + Permit base_key
            # (兼容 82001d09 全端口放行版本残留).
            for fkey in (
