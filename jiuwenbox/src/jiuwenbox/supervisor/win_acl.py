...
def grant_aces(
    ...
    *,
    ...
    """对 ``path`` 一次性施加多个 ACE (合并 Get/Set, 省重复递归).

    Args:
        path: 文件/目录绝对路径.
        aces: 待施加 ACE 列表, 每项 (sid, rights, mode):
            sid 为 SID 字符串或 pywin32 SID 对象;
            rights 为访问掩码 (FILE_GENERIC_READ / WRITE_DAC|READ_CONTROL 等);
            mode 为 "ALLOW" 或 "DENY".
    ...
    # 解析所有 sid + 组装 (ace_type, flags, mask, sid) 元组列表.
    new_ace_tuples: list[tuple[int, int, int, object]] = []
    ace_desc: list[str] = []  # 埋点用: sid_key+mode+rights 摘要
    for sid, rights, mode in aces:
        ...
        new_ace_tuples.append((ace_type, inherit_flags, int(rights), sid_obj))
        ace_desc.append(f"{mode}:0x{int(rights):X}:{_sid_dedup_key(sid_obj)}")

    # 1 次 Get → 1 次 rebuild(多 ACE) → 1 次 Set
    _t0 = time.perf_counter()
    sd = win32security.GetNamedSecurityInfo(
        path,
    ...
    acl = _rebuild_acl_with_order(existing_dacl, new_ace_tuples)
    ...
    try:
        ...
        logger.debug(
            "施加 ACE(合并): path=%s aces=[%s] recursive=%s "
            "ace_count=%d t_read=%.3fs t_rebuild=%.3fs t_set=%.3fs t_total=%.3fs",
            path, ",".join(ace_desc), recursive, acl.GetAceCount(),
            _t_read - _t0, _t_rebuild - _t_read, _t_set - _t_rebuild, _t_set - _t0,
        )
    except Exception as exc:
        raise OSError(
            f"SetNamedSecurityInfo 失败 path={path} aces=[{','.join(ace_desc)}] "
            f"recursive={recursive} ace_count={acl.GetAceCount()}: {exc}"
        ) from exc
...
def grant_ace(
    ...
    *,
    ...
    Args:
        ...
        recursive: 目录时是否让子对象继承.
    """
    grant_aces(path, [(sid, rights, mode)], recursive=recursive)
...
def apply_sandbox_acl(
    ...
    *,
    ...
    # 待清 SID 对象列表 (合成 + 真实). 每个路径施加新 ACE 前先 purge 旧 ACE,
    ...
    for path in write_targets:
        ...
        try:
            ...
            logger.error(
                ...
                "原因=%s; 沙箱内该路径写权限不受控", expanded, exc,
            )
            continue
