...
Skill 自动演进功能通过在配置信息中开启自演进配置项 `react.evolution_auto_scan` 开关启用。

![打开自演进自动检测](../assets/images/skill演进_自动检测开关.png)

### 2.2 自动演进

当 `auto_scan: true` 时，系统会在**每次完整对话结束后**（`after_invoke` 阶段）统一检测演进信号，确保拥有完整的对话上下文来判断。当检测到执行异常或用户纠错时，会生成演进记录。

生成的演进记录处理方式取决于 `auto_save` 配置：

- **`auto_save: true`（默认值）**：演进记录自动写入 `evolutions.json`，无需用户干预
- **`auto_save: false`（当前默认）**：系统会弹出审批请求，需要用户确认后才写入；这是当前配置的默认行为

在下一次调用该 Skill 时，会自动加载包含演进经验的内容。
