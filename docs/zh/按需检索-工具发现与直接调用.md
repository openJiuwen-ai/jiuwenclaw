...
**默认开启（opt-out）。** 按需检索默认启用；如需关闭，把 `progressive_tool_enabled` 改为 `false`——关闭时所有工具全量注入 `tools[]`，系统行为和没有这个功能一样，零影响。
...
react:
  progressive_tool_enabled: true        # 默认开启（opt-out）；设为 false 关闭
```
...
> ⚠️ **prompt capture 暂未实现**：`JIUWENSWARM_PROMPT_CAPTURE` 目前是占位开关——设置后服务端只会打一条 warning 日志，**不会**真正写出 `~/.jiuwenswarm/agent/.logs/prompt_capture/<session_id>.jsonl`。该能力规划中，落地前请勿依赖。如需离线分析每轮注入/搜索/调用/命中率，请暂时用服务日志里的 `[JiuWenRail] CONTEXT DEBUG` 与 `[DenseSearchTool]` 行。
...
是否启用按需检索。**默认 `true`（opt-out）**——开启时按需检索生效；设为 `false` 则代码不构建、不 import、零影响（所有工具全量注入）。
...
| `min_sim` | `0.35` | dense 检索最低相似度阈值。低于此值的工具不返回，避免"无匹配时仍强制返回 top-k"的误导；调高更严格、调低更宽松，`0` 或负值=不过滤 |

#### 环境变量

| 变量 | 默认 | 作用 |
|------|------|------|
| `JIUWENSWARM_PROMPT_CAPTURE` | 未设 | 占位（暂未实现）；设为 `1` 只会打 warning，不产出捕获文件 |
