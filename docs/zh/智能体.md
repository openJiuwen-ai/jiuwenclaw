...
`workspace` 是 JiuwenSwarm 的运行时工作目录，存放 Agent 的记忆、技能、会话数据及用户可配置的心跳任务。在**源码模式**下使用项目根目录的 `workspace`；在**包安装模式**（whl）下，首次运行会将包内预置的 `workspace` 复制到 `~/.jiuwenclaw/workspace`。
