...
记忆采用纯 Markdown 文件存储，Agent 通过文件工具直接操作。默认情况下，
`{workspace_dir}` 指 JiuwenSwarm 的 Agent 工作空间：

- **Windows**：`C:\Users\<你的用户名>\.jiuwenswarm\agent\workspace`
- **Linux/macOS**：`~/.jiuwenswarm/agent/workspace`

因此，长期记忆文件 `MEMORY.md` 的完整默认路径为：
