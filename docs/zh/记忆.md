...
JiuwenSwarm 支持两种记忆模式，可通过 `memory.engine` 配置项控制。配置文件位于 `~/.jiuwenswarm/config/config.yaml` 的 `memory` 段，默认配置如下：

```yaml
memory:
  engine: ${MEMORY_ENGINE:-builtin}    # 默认 builtin，可通过环境变量覆盖
...
| `both` | 内置 + 外接并存 |
| `none` | 全部关闭，Agent 不再自动写入/检索记忆，每次对话均为全新上下文 |
...
> 💡 **更多配置选项**：完整的 Embedding 配置（包括 Web UI 配置方式、示例值、模型选择建议）请参考 [配置信息](./配置信息.md#embedding-配置) 文档。
...
| 环境变量 | 说明 | 示例值 |
|----------|------|--------|
| EMBED_API_KEY | Embedding 服务的 API Key（不配置则使用 mock provider） | `sk-xxxxxxxxxxxxxxxx` |
| EMBED_API_BASE | Embedding 服务的 URL | `https://api.siliconflow.cn/v1` |
| EMBED_MODEL | Embedding 模型名称 | `BAAI/bge-m3` |

> ⚠️ **Mock Provider 限制**：若不配置 Embedding 服务，系统将使用 mock provider，仅支持基础关键词检索，**不支持语义检索**。如需按意图模糊召回记忆（如"上次那个数据库问题"），必须配置 Embedding 服务。
...
memory:
  ...
  external:
    ...
    # Mem0（云端事实抽取+语义检索）
    mem0:
      api_key: ${MEM0_API_KEY:-}             # 从 mem0.ai 获取
...
| Provider | 说明 | 必要配置 |
|----------|------|----------|
| `openjiuwen` | 本地长期记忆 LTM（Long Term Memory）：相对内置记忆的增强版，提供更强的向量检索能力、持久化保障和结构化存储。支持 KV + 向量 + DB 三种存储模式。 | 无（使用默认路径 ~/.jiuwenswarm/memory/ltm） |
| `mem0` | 云端事实抽取与语义检索服务。自动从对话中提取关键事实并持久化存储，支持跨会话记忆召回。 | `api_key`（获取步骤：1. 访问 [mem0.ai](https://mem0.ai) 注册账号 → 2. 创建项目 → 3. 获取 API Key） |
| `openviking` | 字节上下文数据库：字节跳动内部服务，提供企业级上下文存储与检索能力。⚠️ 此为内部服务，外部用户无法自行获取 API Key。 | `endpoint`, `api_key`（需联系字节跳动获取访问权限） |
