...
**Q：HEARTBEAT.md 放在哪里？**
A：必须放在 DeepAgent workspace 根目录下：`~/.jiuwenclaw/agent/jiuwenclaw_workspace/HEARTBEAT.md`（安装模式）或 `jiuwenclaw/resources/agent/jiuwenclaw_workspace/HEARTBEAT.md`（源码模式）。否则会被视为「无自定义任务」，仅返回 `HEARTBEAT_OK`。
...
- Agent 侧 HEARTBEAT.md 处理：`jiuwenclaw/server/runtime/agent_adapter/interface.py` 中根据 `request.params` 的 `heartbeat` 键读取 `HEARTBEAT.md` 并触发任务。
- 前端：`jiuwenclaw/channels/web/frontend/src/components/HeartbeatPanel/`、`heartbeat.get_conf` / `heartbeat.set_conf`、`heartbeat.relay` 事件。
