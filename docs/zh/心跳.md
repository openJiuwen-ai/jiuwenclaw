...
A：应用启动时读取配置；若通过前端「心跳」面板修改，会写回 YAML 并自动重启心跳服务。若直接改 YAML，需重启整个应用（如重启 jiuwenavatar-web）后新配置才会生效。
...
**Q：HEARTBEAT.md 放在哪里？**
A：必须放在 DeepAgent workspace 根目录下：`~/.jiuwenavatar/agent/workspace/HEARTBEAT.md`（安装模式）或 `jiuwenavatar/resources/agent/workspace/HEARTBEAT_ZH.md` / `HEARTBEAT_EN.md`（源码模板，init 时按语言复制）。否则会被视为「无自定义任务」，仅返回 `HEARTBEAT_OK`。

---

## 6. 相关代码与配置索引

- 心跳服务实现：`jiuwenavatar/gateway/heartbeat.py`（`GatewayHeartbeatService`、`HeartbeatConfig`）。
...
- Agent 侧 HEARTBEAT.md 处理：`jiuwenavatar/server/runtime/agent_adapter/interface.py` 中根据 `request.params` 的 `heartbeat` 键读取 `HEARTBEAT.md` 并触发任务。
- 前端：`jiuwenavatar/channels/web/frontend/src/components/HeartbeatPanel/`、`heartbeat.get_conf` / `heartbeat.set_conf`、`heartbeat.relay` 事件。
