...
- **可选任务**：若项目根下存在 `workspace/HEARTBEAT.md`，Agent 会读取其中的「活跃的任务项」，按顺序执行并将结果返回；若无该文件或无任务，则仅返回 `HEARTBEAT_OK`。
...
- **路径**：在web端，该文件在面板右侧，可以点击 `编辑` 进行修改。
![](../assets/images/heartbeat5.png)
- **作用**：若该文件存在且任务不为空，每次心跳时 Agent 会读取该段内容，按顺序执行任务，并把结果返回；若文件不存在或无任务，则仅返回 `HEARTBEAT_OK`。

### 3.2 Agent 行为简述

- 服务端实现会读取 `HEARTBEAT.md`，解析出任务列表，拼成一条 chat 请求发给 Agent，走正常对话流程；解析失败或任务列表为空时，直接返回 `HEARTBEAT_OK`；否则执行任务返回响应。

---

## 4. 前端展示与事件

- **状态**：心跳面板和工具栏会显示最近心跳信息（如 `HEARTBEAT_OK`）与时间。
- **事件**：当 `target` 为 `web` 时，每次心跳响应会通过 `heartbeat.relay` 事件推到前端，用于更新状态与历史记录；若内容非 `HEARTBEAT_OK`，则以弹窗形式提示，便于查看任务执行结果或异常信息。
...
- 心跳服务实现：`jiuwenswarm/gateway/heartbeat/heartbeat.py`。
- 配置读取与写回：`jiuwenswarm/common/config.py` 的 `update_heartbeat_in_config`；应用启动时在 `app.py` 中从 `~/.jiuwenswarm/config/config.yaml` 的 `heartbeat` 段及环境变量构建 `HeartbeatConfig`。
- Agent 侧 HEARTBEAT.md 处理：`jiuwenswarm/server/runtime/agent_adapter/interface_deep.py` 中确定当前是心跳会话，则读取 `HEARTBEAT.md` 并触发任务。
- 前端：`jiuwenswarm/channels/web/frontend/src/components/HeartbeatPanel/`、`heartbeat.get_conf` / `heartbeat.set_conf`、`heartbeat.relay` 事件。
