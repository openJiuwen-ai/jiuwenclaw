...
**Q：HEARTBEAT.md 放在哪里？**
A：必须放在 DeepAgent workspace 根目录下：`~/.jiuwenswarm/agent/jiuwenswarm_workspace/HEARTBEAT.md`（安装模式）或 `jiuwenswarm/resources/agent/jiuwenswarm_workspace/HEARTBEAT.md`（源码模式）。否则会被视为「无自定义任务」，仅返回 `HEARTBEAT_OK`。

---

## 6. 相关代码与配置索引

- 心跳服务实现：`jiuwenswarm/gateway/heartbeat.py`（`GatewayHeartbeatService`、`HeartbeatConfig`）。
...
- Agent 侧 HEARTBEAT.md 处理：`jiuwenswarm/server/runtime/agent_adapter/interface.py` 中根据 `request.params` 的 `heartbeat` 键读取 `HEARTBEAT.md` 并触发任务。
