...
如果浏览器服务启动失败，可以按照以下步骤排查：

| 问题现象 | 可能原因 | 解决方法 |
|----------|----------|----------|
| 点击"启动浏览器服务"后无反应 | Chrome 路径配置错误 | 检查 `CHROME_PATH` 是否正确，确保路径指向真实存在的 `chrome.exe` |
| Chrome 启动后立即关闭 | 端口被占用 | 检查 `9222` 端口是否被其他进程占用，或修改 `config.yaml` 中的 `remote_debugging_port` |
| Chrome 未安装 | Chrome 未安装或路径错误 | 安装 Chrome 浏览器，并更新 `CHROME_PATH` 配置 |
| 前端提示连接失败 | 浏览器服务未正常启动 | 检查后端服务日志，确认 `browser_start_client.py` 是否成功执行 |
| 无法打开特定网页 | 网络或权限问题 | 检查网络连接，确保目标网站可访问，必要时在弹出的 Chrome 中手动测试 |
...
> **说明**：案例操作配图待前端更新后补充。当前已提供完整的文字描述步骤，可按照步骤完成操作。后续每个案例将展示1-2张图片，主要展示关键执行流程和最终结果。
...
- **`.env.template`**：环境变量模板文件，包含所有可用的环境变量及其默认值，可作为配置参考。该文件位于源码包 `jiuwenswarm/resources/.env.template`，安装后会同步到用户配置目录
...
> **提示**：请将 `YOUR_USER` 替换为您的实际 Windows 用户名（如 `Administrator`），可在 `chrome://version` 页面中查看当前用户的路径。

### 5.3 .env 中浏览器配置

#### 5.3.1 浏览器 MCP wrapper 配置
...
#### 浏览器 MCP 接入
- `jiuwenswarm/agents/harness/common/tools/browser_tools.py`
  - 浏览器 MCP wrapper 的接入、自动拉起、client patch、配置构建

#### 浏览器运行时
- `jiuwenswarm/agents/harness/common/tools/browser-move/src/playwright_runtime_mcp_server.py`
  - 浏览器 runtime 的 MCP server 入口
- `jiuwenswarm/agents/harness/common/tools/browser-move/src/playwright_runtime/runtime.py`
  - 浏览器 runtime 编排层
- `jiuwenswarm/agents/harness/common/tools/browser-move/src/playwright_runtime/service.py`
  - 浏览器任务执行、session 复用、timeout guardrails
- `jiuwenswarm/agents/harness/common/tools/browser-move/src/playwright_runtime/agents.py`
  - 主编排 agent 与浏览器 worker agent 的提示词和构造逻辑
- `jiuwenswarm/agents/harness/common/tools/browser-move/src/playwright_runtime/config.py`
...
| 后端 Web RPC 入口 | `jiuwenswarm/gateway/channel_manager/web/app_web_handlers.py` | 提供 `path.get`、`path.set`、`browser.start` 等前端调用入口 |
| Chrome 启动脚本 | `jiuwenswarm/agents/harness/common/tools/browser_start_client.py` | 读取 `config/config.yaml` 中的 `browser.*` 配置，启动带远程调试能力的 Chrome |
| 浏览器 MCP 接入 | `jiuwenswarm/agents/harness/common/tools/browser_tools.py` | 浏览器 MCP wrapper 的接入、自动拉起、client patch、配置构建 |
| 浏览器运行时 MCP 服务 | `jiuwenswarm/agents/harness/common/tools/browser-move/src/playwright_runtime_mcp_server.py` | 浏览器 runtime 的 MCP server 入口 |
| 浏览器运行时编排层 | `jiuwenswarm/agents/harness/common/tools/browser-move/src/playwright_runtime/runtime.py` | 浏览器 runtime 编排层 |
| 浏览器任务执行 | `jiuwenswarm/agents/harness/common/tools/browser-move/src/playwright_runtime/service.py` | 浏览器任务执行、session 复用、timeout guardrails |
