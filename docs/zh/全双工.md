...
| 用户打断 | 相同的本地语音检测、停止播放、保留文字和继续收音 | 相同的本地语音检测、停止播放、保留文字和继续收音 |
...
全双工专属配置集中在侧边栏 **全双工 → 右上角设置** 中：

1. 用电源按钮随时启用或禁用插件；禁用会立即结束当前媒体和模型会话。
2. 在设置中选择 `JoyAI` 或 `Qwen Omni Realtime`。
3. 选择 JoyAI 时，再选择 `JoyAI WebSocket` 或 `OpenAI HTTP` 语音通道。
4. 保存后配置立即写入 Jiuwen 实例的 `.env` 并用于下一次连接。

主配置页面只负责主对话、搜索等 Jiuwen 核心配置，不再展示全双工模型、ASR
和 TTS 字段。下面保留环境变量写法，供无界面部署和故障排查使用。
...
有界面部署可直接进入 **更多 → 应用插件 → 全双工 → 设置** 完成相同配置；禁用插件后，左侧“全双工”入口会隐藏，但管理项会保留以便重新启用。
...
通过全双工插件设置保存时无需重启；直接手工编辑 `.env` 后仍需重启 Gateway。
...
JoyAI 与 Qianwen 共用同一套打断流程：

```mermaid
flowchart LR
    User["用户开始说话"] --> Detect["前端检测语音活动"]
    Detect --> Stop["停止旧音频"]
    Stop --> Keep["保留已显示文字"]
    Keep --> Listen["继续采集麦克风"]
    Stop --> Cancel["取消当前输出"]
    Cancel -. "JoyAI" .-> JTTS["停止独立 TTS"]
    Cancel -. "Qianwen" .-> QCancel["发送 response.cancel"]
```

两条链路的用户体验和前端控制逻辑一致。底层协议仅有一处差异：JoyAI 停止独立 TTS 流，Qianwen 向 Realtime 会话发送原生 `response.cancel`。
