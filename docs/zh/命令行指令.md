...
`jiuwenswarm chat` 通过 Gateway 的 `/cli` WebSocket 路由调用 JiuwenSwarm 的运行时，与 TUI 使用相同的 MessageHandler 和 AgentServer 路径。
...
| `--gateway-url <url>` | `ws://127.0.0.1:19001/cli` | 自定义 Gateway 地址 |
| `--name <instance>` | — | 多实例隔离 |
...
`jiuwenswarm chat` 使用独立的 `/cli` 路由（`channel_id="cli"`），与 TUI 的 `/tui` 路由隔离，两者可以同时使用而不会互相干扰。终端 CLI 的第一版不追求完整复刻 TUI 的所有 slash command。
