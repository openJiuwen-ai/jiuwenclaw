...
> 💡 **说明**：`react.evolution.enabled` 是总开关（默认开启，决定自演进能力是否可用），`auto_scan` / `skill_create` 是其下的子开关（默认关闭，决定是否自动触发）。`Skill自演进.md` 中提到的 `evolution_auto_scan` 即对应这里的 `auto_scan` 子项。即使总开关开启，只要 `auto_scan` 关闭，系统也只会在你手动执行 `/evolve` 时才进行演进。
...
| `symphony.orchestration.top_k` | `3` | 编排时保留的候选路线数量上限 |
...
| `symphony.skill_retrieval.build.branching_factor` | `12` | 技能树拆分阈值基数，影响树的粗细 |
