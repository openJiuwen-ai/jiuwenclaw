...
> 💡 **TeamSkillsHub Token 说明**：`teamskills_user_token` 用于在 `/teamskills search`、`/teamskills install` 等命令中标识您的 TeamSkillsHub 账户身份，便于技能的安装记录、版本追踪与权限校验。不配置时，仍可浏览和搜索公开技能，但安装 / 同步私有技能、记录安装历史等功能将受限。配置方式：在 [TeamSkillsHub](https://teamskills.openjiuwen.com) 登录后于个人中心生成 Token，填入配置面板即可。
