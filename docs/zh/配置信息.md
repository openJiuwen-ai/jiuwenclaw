...
| `symphony.skill_retrieval.build.equivalence_enabled` | `false` | 是否在 taxonomy 终端分支内启用 Skill 等价群归一 |
| `symphony.skill_retrieval.build.equivalence_all_pairs_scope_limit` | `12` | 小终端分支全量枚举 Skill pair 的数量上限 |
| `symphony.skill_retrieval.build.equivalence_candidate_neighbors` | `8` | 大终端分支中每个 Skill 最多提议的候选邻居数 |
| `symphony.skill_retrieval.build.equivalence_max_pairwise_pairs` | `10000` | 单次构建允许验证的等价候选 pair 硬上限，超过时构建失败而不截断 |
