...
| **自动保存审批** | `react.evolution.auto_save` | `false` | 开启后演进审批无需用户点击即可自动通过 |
| **自动建议创建新技能** | `react.evolution.skill_create` | `false` | 开启后，系统可以在缺少合适 Skill 时提出自动创建建议。环境变量 `SKILL_CREATE` 优先 |
| **执行轨迹落盘目录** | `react.evolution.trajectory_dir` | `agent/evolution_trajectories` | 技能演进执行轨迹 JSONL 目录。相对路径相对工作区根目录；绝对路径直接使用。环境变量 `EVOLUTION_TRAJECTORY_DIR` 优先；均未设置时默认 `~/.jiuwenswarm/agent/evolution_trajectories` |

> 💡 **说明**：`react.evolution.enabled` 是产品总开关（默认开启）。挂载演进 Rail 后被动扫描始终开启，不再提供独立的 `auto_scan` / `signal_trigger` 子开关。`review_trigger` / `auto_save` / `skill_create` 为可选子开关；当 `skill_create` 关闭且未开启 `review_trigger` 时，系统仍会做被动扫描，并响应手动 `/evolve` 指令。
