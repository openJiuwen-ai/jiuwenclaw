...
> 💡 **说明**：`react.evolution.enabled` 是产品总开关（默认开启）。挂载演进 Rail 后被动扫描始终开启，不再提供独立的 `auto_scan` / `signal_trigger` 子开关。`review_trigger` / `auto_save` / `skill_create` 为可选子开关；当 `skill_create` 关闭且未开启 `review_trigger` 时，系统仍会做被动扫描，并响应手动 `/evolve` 指令。执行轨迹固定落盘到默认目录 `~/.jiuwenswarm/agent/evolution_trajectories`，不再通过配置项或环境变量覆盖。
