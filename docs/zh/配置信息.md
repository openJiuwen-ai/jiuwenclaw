...
- **配置项**：`context_engine.kv_release_enabled`
- **默认值**：`false`（关闭）
...
- **配置项**：`memory.filter_enabled`
- **默认值**：`true`（开启）
- **作用**：启用后，系统会在写入记忆前自动检测并过滤敏感信息
...
管理员可以直接在主配置中指定扩展目录，无需通过环境变量注入：

一个“扩展根目录”中应包含 `extension.py`，并可选包含 `extension.yaml`。例如：
...
多个条目使用分号分隔，每个条目既可以是扩展集合目录，也可以是单个扩展根目录。JiuwenSwarm 还会自动加入内置的 `jiuwenswarm/extensions` 目录。扩展通常在服务启动时加载，修改目录或扩展代码后应重启相应的 AgentServer。
