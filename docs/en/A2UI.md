...
A2UI is an optional generated UI capability in JiuwenAvatar, natively supported only by the Web channel. It lets an agent return standardized UI messages for forms, confirmations, structured details, comparisons, and similar workflows, then lets the Web frontend render those messages as interactive components.

## Module Location

Backend A2UI code lives under `jiuwenavatar/server/runtime/a2ui/`. The capability belongs to the AgentServer runtime response path and is not a top-level shared package under `jiuwenavatar/`.

Frontend rendering code lives under `jiuwenavatar/channels/web/frontend/src/features/a2ui/`. The Web frontend only enters A2UI code when rendering assistant messages or sending interaction events.
...
Default configuration lives in `jiuwenavatar/resources/config.yaml`:
...
- Environment variable: `JIUWENAVATAR_A2UI_ENABLED=false` or `true`.
...
Legacy `JIUWENCLAW_A2UI_*` environment variables are not supported after this PR update on June 4, 2026. Use the `JIUWENAVATAR_A2UI_*` prefix for all A2UI runtime overrides.
...
- A2UI channel support is centralized in `jiuwenavatar.server.runtime.a2ui.integration.is_a2ui_channel`; currently only `web` returns true.
- Backend host code should call only the thin `jiuwenavatar.server.runtime.a2ui.integration` facade, not protocol, repair, or schema internals.
