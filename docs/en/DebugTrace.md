...
JiuwenSwarm's debug tracing provides unified observability across **Agent, Code, and Team modes**, helping you reconstruct the full behaviour of a conversational run:
...
The `/debug` directive and its parsing primitive are shared across all three modes. The local-dump and OTel paths are independent yet cross-referenceable via `trace_id`.
...
| Unified across modes | Agent / Code / Team all support `/debug`, sharing one directive primitive |
...
All three modes support debug tracing; they differ only in the dump directory and the config block used:
...
| `team` / `code.team` | ✅ | `~/.jiuwenswarm/.agent_teams/traces/dump-team-<session>.txt` | `team_observability` |

> All three modes share the same OTel backend stack (OTel Collector + Langfuse). See [Section 5](#5-local-file-mode-offline-upload-to-langfuse) and `deploy/observability/`.
...
~/.jiuwenswarm/.agent_teams/traces/dump-team-<session_id>.txt    # Team mode
```
...
> The sections below are organised by *what you want to do*; all of them apply to Agent / Code / Team alike — there is no per-mode split.
...
> Team mode's local dump is controlled by `team_observability` and is written when `/debug` is triggered (internally passed via the `JIUWENSWARM_TEAM_STREAM_TRACE` switch). The `debug_trace` block applies only to Agent / Code.
...
Then configure (`agent_observability` for Agent/Code, `team_observability` for Team — same fields):
...
When you cannot (or don't want to) stay connected to a Collector, use the `file` exporter to write spans to local disk, then batch-upload them later with `deploy/observability/upload_traces_to_langfuse.py`. This is the "two-phase" approach offered by the deploy stack and **works for Agent / Code / Team alike**.
...
Edit `~/.jiuwenswarm/config/config.yaml` (Agent/Code and Team each use their own block; fields are identical):

```yaml
agent_observability:           # for Team mode use team_observability
  enabled: true
  exporter: file
...
| Team | `~/.jiuwenswarm/.agent_teams/traces/dump-team-<session_id>.txt` |
...
### 7.3 `team_observability` — Team structured OTel

Identical fields to `agent_observability`, applied to the Team runtime path. Team's local dump (`dump-team-<session>.txt`) is written by its internal logger when `/debug` is triggered:

```yaml
team_observability:
  enabled: true                     # Team observability master switch
  exporter: file                    # otlp_grpc / otlp_http / file / console
  endpoint: http://localhost:4317
...
> **How the three blocks relate**: `debug_trace` governs only Agent/Code local text dumps; `agent_observability` and `team_observability` govern OTel spans for Agent/Code and Team respectively. All three share the same `deploy/observability` backend stack. The single coupling point is `debug_trace.<mode>.otel_enabled` — it lets a `/debug` run temporarily pull up Agent/Code OTel.

### 7.4 Resolution rules (debug_trace)

```
debug_enabled = request has /debug  OR  debug_trace.enabled  OR  debug_trace.<mode>.enabled
...
- OTel has a single global `TracerProvider` per process: if Agent and Team observability are both enabled, whichever initialises first wins and the other reuses the same provider.
...
By mode, under `~/.jiuwenswarm/.agent/traces/`, `.code/traces/`, or `.agent_teams/traces/`. If `JIUWENSWARM_DATA_DIR` is set, the corresponding subdirectory under `<that dir>/`.
...
- [Modes](Modes.md) — `agent.*` / `code.*` / `team` mode descriptions and switching
