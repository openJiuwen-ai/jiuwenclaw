...
- **CLI TUI**: `jiuwenavatar/channels/tui/frontend/src/core/commands/` has a local registry; some commands call the backend via WebSocket, others are purely local.
...
- **Python side**: Implemented as `jiuwenavatar/gateway/slash_command.py` (controlled channel parsing, `CONTROL_MESSAGE_TEXTS`, first-batch command metadata `FIRST_BATCH_REGISTRY`). May evolve into `channel_control_slash.py` etc.
