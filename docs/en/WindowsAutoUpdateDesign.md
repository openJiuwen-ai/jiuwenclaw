...
This document describes the minimum viable auto-update solution for JiuwenAvatar on Windows desktop. The goal is to prioritize stability over seamless upgrades.
...
- Download artifact: Inno Setup installer `jiuwenavatar-setup-<version>.exe`
...
5. The backend downloads the installer to `%USERPROFILE%\\.jiuwenavatar\\.updates` in the background
...
updater:
  ...
  repo_name: jiuwenavatar
  release_api_url: ""
  asset_name_pattern: "jiuwenavatar-setup-{version}.exe"
  sha256_name_pattern: "jiuwenavatar-setup-{version}.exe.sha256"
  timeout_seconds: 20
```
...
4. After installation completes, it restarts `jiuwenavatar.exe`