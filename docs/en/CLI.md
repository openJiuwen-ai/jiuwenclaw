...
JiuwenAvatar supports **special prefix commands** to control sessions and modes. These commands are parsed by the Gateway's `MessageHandler` and **are not sent to the Agent**.
...
**Scope:** Terminal UI (`jiuwenavatar-tui`) only; parsed locally, not by the Gateway control pipeline.
...
- Stored as a single-line file: **`~/.jiuwenavatar/tui-workspace-dir`**.
