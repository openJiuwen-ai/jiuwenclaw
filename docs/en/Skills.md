...
A Skill is a module that extends JiuwenAvatar with specific capabilities. You can think of it as an **installable, manageable, reusable capability package**.
...
JiuwenAvatar supports multiple sources:
...
| **ClawHub** | Skill “app store” in the JiuwenAvatar ecosystem                                  | Search and install in the web UI; https://clawhub.ai/skills |
...
Built-in skills are skill resources packaged with JiuwenAvatar.
...
3. **Manual copy (optional)**
   ...
   ```text
   C:\Users\<username>\.jiuwenavatar\service_default\agent_default\jiuwenavatar_workspace\skills\
   ```
...
| Possible cause | Resolution |
|----------|----------|
| Service not restarted | Restart JiuwenAvatar |
...
You can let JiuwenAvatar help you generate it. **`YAML frontmatter` between the first `---` and second `---` declares metadata**; Markdown after the second `---` is the **skill body**—the instructions the Agent follows. Example:
...
1. **Modify an existing skill through JiuwenAvatar**

   Talk to JiuwenAvatar directly, for example: "Help me optimize the xxx skill and add xxx capability."
...
### Through chat with JiuwenAvatar: "Optimize the weather skill and add UV intensity display", the skill is updated
