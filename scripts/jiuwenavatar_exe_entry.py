...
# 在任何业务 import 之前，将 cwd 切换到用户数据目录 ~/.jiuwenavatar，
# 让 openjiuwen 的相对日志路径落到 <data>/logs/，与项目其它运行时数据同根。
if getattr(sys, "frozen", False):
    _ORIGINAL_CWD = os.getcwd()
    _data_dir_env = os.environ.get("JIUWENAVATAR_DATA_DIR")
    if _data_dir_env:
        _target_cwd = Path(_data_dir_env).expanduser()
    else:
        _target_cwd = Path(os.path.expanduser("~")) / ".jiuwenavatar"
    try:
...
_DESKTOP_RUN_BACKEND = "--desktop-run-backend"
...
_SYNC_BUILTIN_SKILLS = "--sync-builtin-skills"

_CHILD_FLAGS = {"--desktop-run-app", "--desktop-run-web", "--desktop-run-backend",
        _DESKTOP_RUN_AGENT, _DESKTOP_RUN_GATEWAY, _DESKTOP_INSTALL_UPDATE, _SYNC_BUILTIN_SKILLS}
...
def _acquire_single_instance_lock() -> bool:
    ...
    lock_path = Path.home() / ".jiuwenavatar" / ".desktop.lock"
    try:
...
def _show_already_running_message() -> None:
    msg = "JiuwenAvatar is already running. Please use the existing window."
    title = "JiuwenAvatar"
    try:
        if os.name == "nt":
...
def _write_child_error(exc: BaseException) -> None:
    """将子进程的未捕获异常写入日志文件。"""
    try:
        log_dir = Path(os.environ.get("JIUWENAVATAR_DATA_DIR", Path.home() / ".jiuwenavatar")) / "logs"
        log_dir.mkdir(parents=True, exist_ok=True)
        log_file = log_dir / "jiuwenavatar_exe_error.log"
...
def _dispatch() -> None:
    ...
    if len(sys.argv) >= 2 and sys.argv[1].lower() == "init":
        sys.argv.pop(1)
        from jiuwenavatar.init_workspace import main as init_main
        init_main()
        return
    # 子命令：CLI 命令分发
    if len(sys.argv) >= 2 and sys.argv[1].lower() == "acp":
        from jiuwenavatar.channels.acp.app_acp import main as acp_main
        acp_main()
        return
    if _pop_flag("--desktop-run-app"):
        from jiuwenavatar.app import main as app_main
        app_main()
        return
    if _pop_flag(_DESKTOP_RUN_BACKEND):
        from jiuwenavatar.channels.desktop.embedded_backend import main as backend_main
        backend_main()
        return
    if _pop_flag("--desktop-run-web"):
        from jiuwenavatar.channels.web.app_web import main as web_main
        web_main()
        return
    if _pop_flag(_DESKTOP_RUN_AGENT):
        from jiuwenavatar.server.app_agentserver import main as agent_main
        agent_main()
        return
    if _pop_flag(_DESKTOP_RUN_GATEWAY):
        from jiuwenavatar.gateway.app_gateway import main as gateway_main
        gateway_main()
        return
    if _pop_flag(_SYNC_BUILTIN_SKILLS):
        from jiuwenavatar.server.runtime.skill.skill_manager import SkillManager

        # Installer/update helper runs in its own process. Use a fresh manager so
        # the sync is based on persisted user skill state, not an in-memory app singleton.
        result = SkillManager().sync_builtin_skills_from_package(force=True)
        updated = result.get("updated") or []
        if updated:
            print(f"[jiuwenavatar] synced builtin skills: {', '.join(updated)}")
        errors = result.get("errors") or {}
        if errors:
            print(f"[jiuwenavatar] builtin skill sync errors: {errors}", file=sys.stderr)
            raise SystemExit(1)
        raise SystemExit(0)
    if _DESKTOP_INSTALL_UPDATE in sys.argv:
        from jiuwenavatar.channels.desktop.desktop_app import main as desktop_main
        desktop_main()
        return
    ...
    if "--browser-start-client" in sys.argv:
        ...
        sys.argv.pop(idx)
        from jiuwenavatar.agents.harness.common.tools.browser_start_client import main as browser_main
