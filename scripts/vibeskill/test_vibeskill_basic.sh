...
echo "运行: uv run python scripts/vibeskill/test_ws_send.py $SESSION_ID"
uv run python scripts/vibeskill/test_ws_send.py $SESSION_ID

echo ""
echo "========== 3. 中止 AI 处理 =========="
curl -sS -X POST "$API_BASE/api/v1/session/$SESSION_ID/abort"
echo ""

echo ""
echo "========== 4. 删除会话 =========="
curl -sS -X DELETE "$API_BASE/api/v1/session/$SESSION_ID"
echo ""
