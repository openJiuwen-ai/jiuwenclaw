...
add_resource_if_set() {
    local module=$1
    local file=$2
    local name="${DEPLOY_VARS["${module}_NAME"]}"
    local cond=".kind == \"Deployment\" and .metadata.name == \"${name}\""

    if [ -n "${DEPLOY_VARS["${module}_CPU_REQUEST"]:-}" ]; then
        yq eval "select(${cond}).spec.template.spec.containers[0].resources.requests.cpu = \"${DEPLOY_VARS["${module}_CPU_REQUEST"]}\"" -i "${file}"
    fi

    if [ -n "${DEPLOY_VARS["${module}_MEMORY_REQUEST"]:-}" ]; then
        yq eval "select(${cond}).spec.template.spec.containers[0].resources.requests.memory = \"${DEPLOY_VARS["${module}_MEMORY_REQUEST"]}\"" -i "${file}"
    fi

    if [ -n "${DEPLOY_VARS["${module}_CPU_LIMIT"]:-}" ]; then
        yq eval "select(${cond}).spec.template.spec.containers[0].resources.limits.cpu = \"${DEPLOY_VARS["${module}_CPU_LIMIT"]}\"" -i "${file}"
    fi

    if [ -n "${DEPLOY_VARS["${module}_MEMORY_LIMIT"]:-}" ]; then
        yq eval "select(${cond}).spec.template.spec.containers[0].resources.limits.memory = \"${DEPLOY_VARS["${module}_MEMORY_LIMIT"]}\"" -i "${file}"
    fi
}