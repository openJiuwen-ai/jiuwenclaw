...
Beta2 migration note (2026-09-03): the original menu/tag UI described above is superseded by plain text `/persist <first task>` on the new-conversation page. The creation-time metadata contract remains unchanged; no Web slash-command picker is included. See `docs/zh/persist-session-beta2-migration.md`.
