...
def load_settings() -> EffectivePolicyDatabaseSettings:
    ...
    db_type = (
        ...
        or _env("JIUWENSWARM_GATEWAY_DB_TYPE")
    ...
    sqlite_path = (
        ...
        or _env("JIUWENSWARM_GATEWAY_DB_PATH")
        or _env("JIUWENCLAW_GATEWAY_DB_PATH")
    ...
    return EffectivePolicyDatabaseSettings(
        ...
        mysql_host=(
            ...
            or _env("DB_HOST")
            or _env("JIUWENSWARM_GATEWAY_DB_HOST")
        ...
        mysql_user=(
            ...
            or _env("DB_USER")
            or _env("JIUWENSWARM_GATEWAY_DB_USER")
        ...
        mysql_password=(
            ...
            or _env("DB_PASSWORD")
            or _env("JIUWENSWARM_GATEWAY_DB_PASSWORD")
        ...
        mysql_database=(
            ...
            or _env("DB_NAME")
            or _env("JIUWENSWARM_GATEWAY_DB_NAME")
...
def enterprise_policy_enabled() -> bool:
    """需 AGENT_RUNTIME + 组网实例 id；MySQL 默认可用，SQLite 需可解析路径。"""
    # 企业版特性：仅 AGENT_RUNTIME 开启时生效
    if not os.getenv("AGENT_RUNTIME", "").strip():
        return False
    flag = (
        _env("JIUWENSWARM_ENTERPRISE_MODEL_POLICY").lower()
        or _env("JIUWENCLAW_ENTERPRISE_MODEL_POLICY").lower()
    )
    ...
    if flag in ("1", "true", "yes", "on"):
        return True
    instance_id = (
        _env("JIUWENSWARM_PROVISIONED_INSTANCE_ID")
        or _env("JIUWENCLAW_PROVISIONED_INSTANCE_ID")
        or _env("GATEWAY_INSTANCE_ID")
    )
    if not instance_id:
        return False
    ...
    if settings.db_type == "mysql":
        return True
    explicit = (
        _env("JIUWENSWARM_EFFECTIVE_POLICY_DB_PATH")
        or _env("JIUWENCLAW_EFFECTIVE_POLICY_DB_PATH")
        or _env("JIUWENSWARM_GATEWAY_DB_PATH")
        or _env("JIUWENCLAW_GATEWAY_DB_PATH")
    )
    if explicit:
