...
def _load_project_sessions(
    ...
    for session in collect_all_sessions_metadata():
        ...
        if _attribute_session_project(session, visible_project_ids) != project_id:
            continue
        # This preserves the legacy Web payload behavior. It is a metadata
        # filter only; directory isolation is still wholly process-scoped.
        session_owner = str(session.get("user_id") or "").strip()
