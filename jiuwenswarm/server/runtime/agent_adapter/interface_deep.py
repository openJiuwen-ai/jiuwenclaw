...
from jiuwenswarm.server.runtime.agent_adapter.trusted_web_search import (  # noqa: E402
...
class _AgentProfileRailSpecs:
    ...
    before_execution_context: tuple[_RailBuildInfo, ...] = ()
    before_stream_event: tuple[_RailBuildInfo, ...] = ()
    before_permission: tuple[_RailBuildInfo, ...] = ()
    after_permission: tuple[_RailBuildInfo, ...] = ()
...
class JiuWenSwarmDeepAdapter:
    ...
    async def _evict_failed_permission_session_adapter(
        ...
        if self._session_adapters.get(session_id) is not adapter:
            return
        if adapter._has_live_root_permission_owner(  # pylint: disable=protected-access
            session_id
        ):
    ...
    def _build_profile_rail_specs(
        ...
        *,
        ...
        before_execution_context: list[_RailBuildInfo] = []
        ...
        return _AgentProfileRailSpecs(
            ...
            before_stream_event=(
                ...
                _RailBuildInfo(
                    "_eternal_conversation_rail",
                    self._build_eternal_conversation_rail,
                ),
    ...
    def _validate_profile_rail_infos(
        rail_infos: tuple[_RailBuildInfo, ...],
    ) -> None:
        for info in rail_infos:
            if info.attr_name in _REQUIRED_AGENT_RAIL_ATTR_NAMES:
    ...
    def _build_excluded_profile_rails(
        ...
        *,
        ...
        rail_infos = (
            *specs.before_execution_context,
            *specs.before_stream_event,
            _RailBuildInfo("_stream_event_rail", lambda: JiuSwarmStreamEventRail()),
            *specs.before_permission,
            *specs.after_permission,
        )
        return self._instantiate_rails(list(rail_infos), config_base)
    ...
    def _build_agent_rails(
        ...
        *,
        ...
        rails_list = self._instantiate_rails(rail_infos, config_base)
    ...
    def _publish_root_permission_cutover(
        ...
        if isinstance(previous, _RootPermissionDispatchHandoff) and (
        ...
        *,
        ...
        async for chunk in stream:
            if hasattr(chunk, "type") and hasattr(chunk, "payload"):
                # Core represents terminal model failures as answer chunks.
                # Preserve develop's unary error contract instead of returning
                # a successful empty response for cron and other unary callers.
                if (
                    chunk.type == "answer"
            ...
            parsed = self._parse_stream_chunk(
                chunk,
                _parent_session_id=root_session_id,
            )
            if parsed is None:
                continue
