...
class SkillManager:
    ...
    def _normalize_online_search_item(source: str, item: dict[str, Any], rank: int) -> dict[str, Any]:
        if source == "skillnet":
            ...
            native_score = item.get("score", item.get("stars"))
    ...
    async def handle_skills_online_search(self, params: dict) -> dict:
        ...
        per_source_limit = min(max(limit, 20), 50)

        calls: list[tuple[str, Awaitable[dict[str, Any]]]] = [
            (
                "skillnet",
                self.handle_skills_skillnet_search(
                    {"q": query, "limit": per_source_limit, "mode": "keyword"}
                ),
            )
        ]
        ...
        if self._get_clawhub_token():
            calls.append(
                (
                    "clawhub",
                    self.handle_skills_clawhub_search({"q": query, "limit": per_source_limit}),
                )
            )
        else:
