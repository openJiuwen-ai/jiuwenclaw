...
def _require_i18n(value: Any, field: str) -> dict[str, str]:
    if not isinstance(value, dict):
        raise ValueError(f"manifest.json: {field} 必须是含 en/zh 的对象")
    en = str(value.get("en") or "").strip()
    zh = str(value.get("zh") or "").strip()
    if not en or not zh:
        raise ValueError(f"manifest.json: {field}.en 与 {field}.zh 均不能为空")
    return {"en": en, "zh": zh}
...
def _entry_from_manifest(
    manifest: dict[str, Any], package_id: str, *, version: str
) -> dict[str, Any]:
    card = manifest.get("agentCard") or {}
    card_id = str(card.get("id") or "").strip()
    if card_id != package_id:
        raise ValueError(
            f"manifest.json: agentCard.id ({card_id!r}) 必须等于包目录名 ({package_id!r})"
        )
    return {
        "id": package_id,
        "name": _require_i18n(manifest.get("displayName"), "displayName"),
        "description": _require_i18n(
            manifest.get("displayDescription"), "displayDescription"
        ),
        "version": version,
        "source": "local",
        "installed": False,
...
def register_package(pkg_dir: Path, *, bump: bool = False) -> dict[str, Any]:
    ...
    if manifest.get("packageType") != "agent_template":
        raise ValueError(
            f"packageType 必须是 agent_template，当前是 {manifest.get('packageType')!r}"
        )

    marketplace_path = get_marketplace_path(pkg_dir)
    plugins = _read_marketplace_plugins(marketplace_path)
    existing = next(
    ...
    # Resolve version before building the entry: bump mutates manifest.json
    # in place so the entry and the on-disk manifest stay consistent.
    effective_version = _maybe_bump_version(pkg_dir, manifest, bump=should_bump)
    entry = _entry_from_manifest(manifest, package_id, version=effective_version)
    ...
    final = next(item for item in plugins if item.get("id") == package_id)
...
def main() -> int:
    ...
    write_stdout(f"  Version:     {entry['version']}\n")
    write_stdout(
