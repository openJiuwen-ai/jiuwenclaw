...
- Rail 监听生命周期、维护 session 状态；Tool 读同一状态返回结构化结果
- 状态文件从运行态根派生：`get_workspace() or get_cwd()` 下按 `<agent-name>/<session_id>` 隔离；原子写（临时文件 + `os.replace`）
- Tool 从 `kwargs["session"].get_session_id()` 取 session；Rail 从 `ctx.session.get_session_id()` 取
