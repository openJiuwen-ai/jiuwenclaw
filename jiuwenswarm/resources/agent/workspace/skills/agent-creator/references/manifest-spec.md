...
| `packageType`    | string | 是  | 固定 `"agent_template"` |
| `agentCard`      | object | 是  | agent 名片，见下 |
...
| `displayName`    | `{en, zh}` | 是  | 专家列表/详情页标题 |
| `displayDescription` | `{en, zh}` | 是  | 能力介绍，中文约 40-50 字 |
...
| `defaultInitInput` | `{en, zh}` | 是  | 首次对话引导语，须与 `quickInputs` 第一条一致 |
| `tags`           | `[{en,zh}]` | 是  | 擅长领域标签，固定 3 个 |
| `quickInputs`    | `[{en,zh}]` | 是  | 推荐提示词，固定 3 个 |

> `persona` **必填**。`skills` / `tools` / `rails` 没生成对应文件就**整段省略**，不要留空数组 `[]`，也不要声明指向不存在的路径。

> **本 skill 禁止生成或声明**：`model`、`mcps`、`subagents`。

## agentCard 字段

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `id` | string | 是 | 唯一标识，**必须等于包目录名**（agent-name，kebab-case） |
| `name` | string | 是 | 展示名（人设花名，如「Kaka」或「卡卡」） |
| `description` | string | 是 | 一句话英文/中文描述 |
...
| `displayName` | `{en, zh}` | 是 | 卡片标题；优先与 `ToolCard.name` / rail 逻辑名一致（如 `content_analyzer`） |
| `displayDescription` | `{en, zh}` | 是 | 卡片简介，中英各一句，说明用途 |

```json
{
  "file": "tools/content_analyzer.py",
  "class": "ContentAnalyzer",
  "displayName": { "en": "content_analyzer", "zh": "content_analyzer" },
  "displayDescription": {
...
> **skills 不要写 `displayName` / `displayDescription`**：技能卡片名与简介来自各目录 `SKILL.md` frontmatter 的 `name` / `description`。
...
3. **`defaultInitInput` 与 `quickInputs[0]` 中英逐字一致**
4. **填充后不得残留 `[TODO]`**
