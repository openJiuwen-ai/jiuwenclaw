...
| 文件              | 参考                             | 要点                                                                                                                                   |
| --------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------ |
| `manifest.json` | `@references/manifest-spec.md` | 展示字段填实；`id` = 目录名；`name` / `description` 非空；`defaultInitInput` ≡ `quickInputs[0]`；禁止 `persona` / `agentCard` / `model` / `subagents` / `mcps` |
| `README.md`     | —                              | 描述插件**能力**与使用方式（配合专家装备），不写人设                                                                                                         |


### quickInputs 写法（插件特有）

- 第一条：插件核心入口（常与 `defaultInitInput` 相同），如建档 + 生成方案
...
| Tool  | `tools/<name>.py`      | `@references/tool-spec.md`  |
| Rail  | `rails/<name>.py`      | `@references/rail-spec.md`  |
| none  | —                      | 跳过                          |
...
- `tools[]` / `rails[]`：每项必填 `file` / `class` / `displayName` / `displayDescription`
