...
| `quickInputs`        | `[{en,zh}]` | 是   | 推荐提示词，固定 3 个；写**能力触发场景**                                                          |
...
| `displayName`        | `{en, zh}` | 是   | 卡片标题；优先与 `ToolCard.name` / rail 逻辑名一致          |
| `displayDescription` | `{en, zh}` | 是   | 卡片简介，中英各一句                                     |


> **skills 不要写** `displayName` **/** `displayDescription`：来自各目录 `SKILL.md` frontmatter 的 `name` / `description`。
...
1. `id` **= 包目录名**
2. `defaultInitInput` **与** `quickInputs[0]` **中英逐字一致**
