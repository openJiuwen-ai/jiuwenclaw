...
description: |
  ...
  触发词：创建插件、创建 plugin、做一个插件、plugin 脚手架、scaffold plugin、修改插件、编辑插件、更新插件、调整插件、create plugin package、插件包、帮我做一个 XX 插件、给 agent 加 XX 能力包。当用户说「帮我做一个 XX 插件」「创建一个能力扩展包」「改一下 XX 插件」时，务必使用本 skill。
---
...
3. 向用户确认要改什么（展示字段、quickInputs、增删 skill/tool/rail 等）
...
3. **推荐提问**：直接引用 manifest 的 `quickInputs`（3 条），强调是**能力触发语**而非人设介绍
4. **如何启用**：
