...
def register_package(pkg_dir: Path, *, bump: bool = False) -> dict[str, Any]:
    ...
    if manifest.get("packageType") != "plugin":
        raise ValueError(
            f"packageType 必须是 plugin，当前是 {manifest.get('packageType')!r}"
        )
...
def main() -> int:
    ...
    write_stdout(
        f"  installed:   {entry['installed']}  enabled: {entry['enabled']}\n"
    )
    write_stdout(
        "\nNEXT:   在插件中心安装并启用该插件（installed=true, enabled=true）后方可对话装备\n"
    )
    return 0
