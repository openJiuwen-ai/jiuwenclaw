...
# 与 ExtensionLoader 使用同一父包，便于扩展已加载时复用 ``__path__``。
...
def _iter_extension_dir_entries() -> list[str]:
    """``EXTENSION_DIRS`` 同时支持 ``;``（ExtensionManager）与 ``os.pathsep``。"""
    raw = os.getenv("EXTENSION_DIRS", "")
    if not raw.strip():
        return []
    normalized = raw.replace(";", os.pathsep)
    return [p.strip() for p in normalized.split(os.pathsep) if p.strip()]


def resolve_manager_ws_client_root() -> Path | None:
    """解析 ``manager_ws_client`` 扩展根目录（``EXTENSION_DIRS`` 或仓库内置路径）。"""
    for raw in _iter_extension_dir_entries():
...
def ensure_manager_ws_client_package(ext_root: Path | None = None) -> Path:
    """注册 ``jiuwenswarm.loaded_extension.manager_ws_client`` 包命名空间。"""
