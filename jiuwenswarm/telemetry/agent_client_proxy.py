...
_tracer = trace.get_tracer("jiuwenswarm.gateway.routing.agent_client")
GATEWAY_CLIENT_INSTRUMENTED_ATTR = "_jiuwenswarm_gateway_client_instrumented"

_EXTENSION_PROXY_MARKER = "_jiuwenswarm_agent_client_extension_proxy"
...
def wrap_agent_client(client: AgentServerClient, *, target_uri: str | None = None) -> AgentServerClient:
    ...
    import jiuwenswarm.telemetry as telemetry_mod
...
async def _send_request_with_telemetry(
    ...
    *,
    ...
    with _tracer.start_as_current_span(
        "jiuwenswarm.gateway.agent.request",
        ...
        try:
            ...
            span.set_attribute("jiuwenswarm.cancelled", True)
            raise
...
def _build_attrs(target_uri: str | None, envelope: E2AEnvelope, is_stream: bool) -> dict[str, Any]:
    attrs: dict[str, Any] = {
        "jiuwenswarm.request.id": envelope.request_id or "",
        "jiuwenswarm.channel.id": envelope.channel or "",
        "jiuwenswarm.session.id": envelope.session_id or "",
        "jiuwenswarm.req.method": envelope.method or "",
        "jiuwenswarm.stream": bool(is_stream),
    }
