...
def instrument_llm() -> None:
    ...
    try:
        from jiuwenswarm.agents.harness.common.react_agent import JiuClawReActAgent
    except ImportError:
        logger.debug("[Telemetry] JiuClawReActAgent not available, skipping llm instrumentor")
        return

    _original_call_llm = getattr(JiuClawReActAgent, "_call_llm")

    async def _traced_call_llm(self, messages, tools=None, session=None, chunk_threshold=10):
        model_name = getattr(getattr(self, "_config", None), "model_name", "unknown")
        ...
        model_cfg = getattr(getattr(self, "_config", None), "model_config_obj", None)
    ...
    setattr(JiuClawReActAgent, "_call_llm", _traced_call_llm)
