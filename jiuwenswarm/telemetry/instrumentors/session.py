...
Target: ``jiuwenclaw.agentserver.session_manager.SessionManager``
...
def instrument_session(
    ...
    def _patched_init(self, *args, **kwargs):
        ...
        setattr(self, "_session_task_start_times", {})
        setattr(self, "_stuck_reported", {})
        setattr(self, "_stuck_checker_task", None)
    ...
    async def _patched_ensure_session_processor(self, session_id: str) -> None:
        processors = getattr(self, "_session_processors", {})
        if session_id in processors and not processors[session_id].done():
            return

        # 创建新的优先级队列和计数器
        getattr(self, "_session_queues")[session_id] = asyncio.PriorityQueue()
        getattr(self, "_session_priorities")[session_id] = 0
        ...
        async def process_session_queue():
            ...
            queue = getattr(self, "_session_queues", {})[session_id]
            while True:
                try:
                    ...
                    # >>> 埋点: state=active
                    getattr(self, "_session_task_start_times")[session_id] = time.monotonic()
                    getattr(self, "_stuck_reported", {}).pop(session_id, None)
                    _emit_state(session_id, "active", "task_started")

                    getattr(self, "_session_tasks")[session_id] = asyncio.create_task(task_func())
                    try:
                        await getattr(self, "_session_tasks", {})[session_id]
                        ...
                        getattr(self, "_session_tasks")[session_id] = None
                        getattr(self, "_session_task_start_times", {}).pop(session_id, None)
                        queue.task_done()
            ...
            # 清理
            getattr(self, "_session_queues", {}).pop(session_id, None)
            getattr(self, "_session_priorities", {}).pop(session_id, None)
            getattr(self, "_session_tasks", {}).pop(session_id, None)
            getattr(self, "_session_processors", {}).pop(session_id, None)
            getattr(self, "_session_task_start_times", {}).pop(session_id, None)
            getattr(self, "_stuck_reported", {}).pop(session_id, None)
        ...
        getattr(self, "_session_processors")[session_id] = asyncio.create_task(process_session_queue())
    ...
    setattr(SessionManager, "_telemetry_patched", True)
...
def _ensure_stuck_checker(session_manager) -> None:
    ...
    setattr(session_manager, "_stuck_checker_task", asyncio.create_task(_check_stuck_sessions()))
