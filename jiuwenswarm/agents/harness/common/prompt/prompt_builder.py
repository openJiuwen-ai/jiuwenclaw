...
- **Use skill xiaoyi-web-search for search tasks**: Whenever a task involves web search, information retrieval, or looking up latest/real-time content (news, market quotes, prices, current events, recent releases, recent developments, or whether a given fact is still current), you MUST invoke skill `xiaoyi-web-search` to perform the search; do not answer directly by other means. Only when this skill is clearly unavailable or its invocation fails may you fall back to other available approaches.
