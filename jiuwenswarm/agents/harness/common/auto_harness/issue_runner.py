...
class GitCodeIssueRunner:
    ...
    RECONCILE_RECORD_STATUSES = IN_FLIGHT_RECORD_STATUSES | {"completed_without_pr", "failed"}
    ...
    def _find_issue_pr_in_repo_pulls(
        ...
        *,
        ...
        issue_pattern = re.compile(
            rf"(?:^|[^\d])#\s*{issue_number}(?:[^\d]|$)|"
            rf"\bissue\s*#?\s*{issue_number}\b",
            flags=re.IGNORECASE,
        )
        for pull in pulls:
            text = "\n".join(
        ...
        for record in self._state_store.list():
            ...
            if not pulls and not fallback_pr:
                try:
                    ...
                    logger.warning(
                        "[GitCodeIssueRunner] fallback PR list failed for #%s: %s",
                        number,
                        exc,
                    )
            if pulls or fallback_pr or repo_pr:
                first = pulls[0] if pulls else fallback_pr or repo_pr or {}
                updated = await self._state_store.update(
                    ...
                    number,
                    {
                        ...
                        "pr_source": (
                            "gitcode_issue_links"
                            if pulls
                            else ("auto_harness_log" if fallback_pr else "repo_pull_search")
                        ),
    ...
    async def watch_once(self, options: IssueWatchOptions) -> dict[str, Any]:
        ...
        if options.issue_numbers:
            issues = [
                self._client.get_issue(owner=options.owner, repo=options.repo, number=number)
