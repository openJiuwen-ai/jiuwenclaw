...
def deterministic_domain_route(
    ...
    *,
    ...
    if category == "browser":
        for uri in network.absolute_uris:
            if reason := absolute_uri_hard_block_reason(uri):
...
def _domain_operation(facts: ToolDecisionFacts) -> str:
    ...
    if category == "browser":
        ...
        if "download" in operation:
            return "download"
        if any(token in operation for token in ("snapshot", "screenshot", "inspect", "probe", "recall", "health", "wait_for")):
            return "inspect"
