...
class RootPermissionQueue:
    ...
    def consume_accepted_answer_if_quarantined(
        ...
        with self._lock:
            ...
            if current.state != "resuming":
                return False
            if current.key.root_session_id not in self._quarantined:
                return False
    ...
    def reconcile(
        ...
        *,
        ...
        if not isinstance(raw_state, list) or not isinstance(raw_ids, list):
    ...
    def snapshot_scope(
        ...
        with self._lock:
            ...
            if request_id is not None:
                matching = [key for key in matching if key.request_id == request_id]
