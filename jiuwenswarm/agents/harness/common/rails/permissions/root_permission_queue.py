...
class RootPermissionCard:
    ...
    owner_binding: AgentOwnerBinding
    tool_name: str
...
class RootPermissionQueue:
    ...
    def begin(
        ...
        *,
        ...
        with self._lock:
            ...
            owner = AgentOwnerBinding(runtime_mode=runtime_mode, agent_id=agent_id)
            key = ToolInvocationKeyV1(
    ...
    def consume_accepted_answer_if_quarantined(
        ...
        with self._lock:
            ...
            if (
                current is None
                or current != answer.card
                or current.state != "resuming"
    ...
    def reconcile(
        ...
        *,
        ...
        if (
            ...
            or not raw_state
            or len(raw_state) != len(raw_ids)
            or len(raw_state) > MAX_ROOT_PERMISSION_CARDS
        ):
    ...
    def snapshot_scope(
        ...
        with self._lock:
            return tuple(
                ...
                for card in self._cards.values()
                if card.key.root_session_id == root_session_id
                and (request_id is None or card.key.request_id == request_id)
            )
...
def _normalize_answer(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if type(approved) is not bool or type(auto_confirm) is not bool:
        raise RootPermissionQueueError("permission_queue_decision_invalid")
    if type(persist_allow) is not bool or not isinstance(feedback, str):
        raise RootPermissionQueueError("permission_queue_decision_invalid")
