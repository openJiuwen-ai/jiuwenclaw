...
class AutoPermissionBeforeToolMixin:
    ...
    async def _before_tool_call_impl(self, *args: Any, **kwargs: Any) -> Any:
        ...
        eligible_confirmation = bool(
            initial_confirmation_payload
            and "reviewer_override_token" not in initial_confirmation_payload
        )
        retired_confirmation = bool(
            initial_confirmation_payload
            and _contains_retired_task_scope_confirmation(
                initial_confirmation_payload
            )
        )
        if path_policy_requires_approval and eligible_confirmation and not retired_confirmation:
            human_send_scope = parse_human_approval_scope(initial_confirmation_payload)
