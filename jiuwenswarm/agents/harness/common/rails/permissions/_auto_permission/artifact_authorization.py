"""Migrated Auto Permission artifact authorization slice."""
...
from jiuwenswarm.agents.harness.common.rails.permissions._auto_permission.artifact_declaration import (
    _requested_file_paths,
)
...
_FILE_DELIVERY_TOOLS = frozenset(
    {"send_file_to_user", "upload_file", "upload_photo"}
)
...
def has_user_file_delivery_prohibition(text: str) -> bool:
    """Recognize the explicit English/Chinese send prohibition hard guard."""

    normalized = str(text or "").strip()
    return bool(
        normalized
        and any(pattern.search(normalized) for pattern in _FILE_DELIVERY_PROHIBITION_PATTERNS)
    )
...
def is_file_delivery_action(facts: ToolDecisionFacts) -> bool:
    ...
    if facts.tool_name == "send_message":
        return bool(
            facts.read_paths
            or _has_file_delivery_field(facts.untrusted_args)
        )
...
class AutoPermissionArtifactAuthorizationMixin:
    """Phase 8c artifact authorization behavior."""

    @staticmethod
    def _issue_send_file_authorization(
        facts: ToolDecisionFacts,
    ) -> str:
        if facts.tool_name != "send_file_to_user":
            return ""
