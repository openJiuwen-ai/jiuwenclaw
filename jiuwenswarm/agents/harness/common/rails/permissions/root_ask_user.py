...
def build_ask_user_metadata(
    *,
    ...
    if tool_name != ASK_USER_TOOL_NAME or not call_id or len(call_id) > 512 or not questions:
        return None
    return {
...
def _answer_values(value: Any, *, multi_select: bool) -> tuple[str, ...] | None:
    ...
    if not values or len(values) > _MAX_ANSWERS or (not multi_select and len(values) != 1):
        return None
