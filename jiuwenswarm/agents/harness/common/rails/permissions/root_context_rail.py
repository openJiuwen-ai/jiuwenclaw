...
class RootContextRail(DeepAgentRail):
    ...
    def __init__(
        ...
        *,
        ...
        expected_agent_id = str(expected_agent_id or "").strip()
        if not expected_agent_id:
            raise ValueError("expected agent id is required")
        if sandboxed is not None and type(sandboxed) is not bool:
    ...
    def _bind(
        ...
        *,
        ...
        root = bind_root_permission_request(
            ...
            runtime_mode="agent",
            agent_id=self._expected_agent_id,
...
def _owner_from_extra(extra: Any, channel_id: str) -> PermissionContext | None:
    ...
    if any(type(raw[name]) is not bool for name in ("group_digital_avatar", "enable_memory", "avatar_mode")):
        raise ValueError("invalid permission owner")
    if any(not isinstance(raw[name], str) for name in ("principal_user_id", "triggering_user_id", "avatar_principal_name")):
        raise ValueError("invalid permission owner")
