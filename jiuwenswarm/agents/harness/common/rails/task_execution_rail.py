...
async def lazy_init_workspace_baseline(
    tool_name: str,
    tool_args: Any,
    ...
    *,
    ...
    log_prefix: str,
    workspace_base: Path | None = None,
) -> tuple[WorkspaceSnapshot | None, bool]:
    """bash 类工具懒建基线：首个此类工具执行前建工作区快照。

    线程 + 超时 + cancel_event 三重防护，防 stat 阻塞 event loop；
    超时一次即禁用本会话基线路径，避免反复重试超时。invoke_tool
    间接调用时按解包出的内部工具名判定（与 after 路径
    detect_artifact_paths 的解包对齐），使 invoke_tool -> bash 场景
    也能在执行前懒建基线，而非降级文本提取。

    供 TaskExecutionRail 与 SkillTurboArtifactRail 的 before_tool_call
    共用。返回 (更新后的基线, 更新后的禁用标志)；无需建立时原样返回。
    ...
    if (
        effective_name not in CODE_EXEC_TOOL_NAMES
        or baseline is not None
        or baseline_disabled
    ):
        return baseline, baseline_disabled
    ...
    try:
        ...
        cancel_event.set()
        logger.warning(
            "%s baseline snapshot timed out (%.1fs), disable baseline diff "
            "for this session",
            log_prefix,
    ...
    if snapshot is not None:
        logger.info("%s baseline ready files=%d", log_prefix, len(snapshot))
        return snapshot, baseline_disabled
    return baseline, baseline_disabled


async def ensure_workspace_baseline(
    rail: Any,
    tool_name: str,
    tool_args: Any,
    *,
    ...
    供 TaskExecutionRail 与 SkillTurboArtifactRail 的 before_tool_call 共用：
    ...
    """
    if rail._workspace_baseline is not None or rail._baseline_disabled:
        return
    async with rail._baseline_init_lock:
        if rail._workspace_baseline is not None or rail._baseline_disabled:
            return
        rail._workspace_baseline, rail._baseline_disabled = (
            await lazy_init_workspace_baseline(
                tool_name,
                tool_args,
                rail._workspace_baseline,
                baseline_disabled=rail._baseline_disabled,
...
class TaskExecutionRail(DeepAgentRail):
    ...
    async def before_tool_call(self, ctx: AgentCallbackContext) -> None:
        ...
        if tool_name in self.ARTIFACT_DETECTION_TOOLS:
            ...
            # 与 ensure_workspace_baseline）。workspace_base 从 metadata
