...
class _ActiveAskRequest:
    """Per-agent active request context, recovered inside the agent runner."""
...
async def ask_user_question_request_scope(
    *,
    ...
    ``agent`` is the running agent instance; when provided, the request context
    is also published to a process-level store keyed by ``id(agent)`` so the
    AskUserQuestionContextRail (running inside the agent's own execution
    context) can re-establish these ContextVars before the tool fires — needed
    because dev-stable's single-agent runner executes tools in a context that
    does not inherit ContextVars set here in the request handler.
    """
    ...
    active: _ActiveAskRequest | None = None
    if agent is not None:
        active = _ActiveAskRequest(
            interactive_ask=bool(interactive_ask),
            session_id=sid,
            stream_request_id=stream_request_id or "",
            channel_id=channel_id or "",
            service_id=runtime_scope.service_id,
            agent_id=runtime_scope.agent_id,
        )
        reg.bind_active_request(agent, active)
    try:
        yield
    finally:
        ...
        if active is not None and agent is not None:
            reg.clear_active_request(agent)
...
class AskUserQuestionRegistry:
    ...
    def __init__(self) -> None:
        ...
        self._active_by_agent_id: dict[int, _ActiveAskRequest] = {}

    def bind_active_request(self, agent: Any, active: _ActiveAskRequest) -> None:
        self._active_by_agent_id[id(agent)] = active

    def get_active_request(self, agent: Any) -> _ActiveAskRequest | None:
        return self._active_by_agent_id.get(id(agent))

    def clear_active_request(self, agent: Any) -> None:
        self._active_by_agent_id.pop(id(agent), None)
