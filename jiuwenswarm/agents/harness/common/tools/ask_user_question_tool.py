...
async def _ask_user_question_impl(questions: Any, max_options: int | None = None) -> dict[str, Any]:
    ...
    if not stream_rid:
        logger.warning(
            "[ask_user_question] missing stream_request_id: interactive=%s session_id=%s "
            "channel_id=%s scope=%s — AskUserQuestionContextRail did not re-bind context "
            "(agent not in active store or rail not registered)",
            interactive, session_id, channel_id, scope,
        )
...
def get_ask_user_question_tool(agent: Any = None) -> Tool:
    """Build the ask_user_question LocalFunction.

    When ``agent`` is provided (per-agent registration after
    ``create_deep_agent``), the returned tool wraps the impl so that — when the
    ContextVars bound by ``ask_user_question_request_scope`` are not visible
    (dev-stable runs the turn in a background single-agent runner that does
    not inherit the request handler's context) — it recovers the active
    request from the process-level store keyed by ``id(agent)`` and
    re-establishes the ContextVars *inside the tool's own execution context*,
    right before delegating to the real impl.
    """
    if agent is None:
        return LocalFunction(card=_ASK_TOOL_CARD, func=_ask_user_question_impl)

    async def _agent_aware_impl(questions: Any, max_options: int | None = None) -> dict[str, Any]:
        if not _stream_request_id_cv.get():
            ...
            if active is not None:
                ...
                logger.info(
                    "[ask_user_question] recovered request context from active store: "
                    "stream_request_id=%s session_id=%s channel_id=%s",
                    active.stream_request_id, active.session_id, active.channel_id,
                )
        return await _ask_user_question_impl(questions, max_options)

    return LocalFunction(card=_ASK_TOOL_CARD, func=_agent_aware_impl)
