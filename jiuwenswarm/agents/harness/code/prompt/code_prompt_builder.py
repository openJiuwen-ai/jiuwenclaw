...
def _code_doing_tasks_prompt() -> PromptSection:
    content = (
        ...
        "you MUST verify it actually works using at least one tool call: "
        "run the test, execute the script, read the output file, "
        "or use bash/curl to confirm the behavior. "
        "You are NEVER allowed to end a session with result_type='answer' "
        "without first making a verification tool call in the same turn. "
