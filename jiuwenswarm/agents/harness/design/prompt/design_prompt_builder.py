...
def _design_product_fundamentals_prompt() -> PromptSection:
    content = (
        ...
        "via the image-generation skill and `invoke`.\n"
        "- **Video**: generate a finished short video (mp4, 4–15 seconds per "
        "clip) via the `seedance-video-gen` skill and `invoke`. A storyboard "
...
def _design_core_capabilities_prompt() -> PromptSection:
    content = (
        ...
        "**follow this mandatory workflow**:\n"
        "\n"
        "1. **Load the skill first**: Call `skill_tool` to load `seedance-video-gen` "
        "and read its SKILL.md. Never improvise a video API.\n"
        "2. **Duration**: each clip MUST be 4–15 seconds. If the user asks for "
        ...
        "4. **Deliver the mp4**: download `content.video_url` and send the file "
        "to the user.\n"
        "\n"
...
def _design_doing_tasks_prompt() -> PromptSection:
    content = (
        ...
        "the `ppt-creation` skill MUST be loaded first. For video tasks, the "
        "`seedance-video-gen` skill MUST be loaded first, then `invoke` — never "
...
def _design_tool_and_skill_principles_prompt() -> PromptSection:
    content = (
        ...
        "For video, always load `seedance-video-gen` first, then call `invoke` "
        "(seedanceMiniTask; poll via seedanceMiniTaskQuery until the mp4 is "
        ...
        "- For video tasks, call `skill_tool` to load `seedance-video-gen` as "
        "your FIRST action, then `invoke` to generate the clip. Never skip "
