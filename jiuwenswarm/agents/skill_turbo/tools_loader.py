...
设计目标：
- 以最小成本让 SkillTurbo 复用 jiuwenswarm + openjiuwen 已有工具；
- 不抽公共层、不改 DeepAgent、不改 openjiuwen；
- 工具一律「逐个 import + 一个 loader 函数」；
- 可用性判断与主 Agent ``_get_tool_cards`` 对齐（同一开关 / 同一 factory），
  本模块不引入新的可用性算法。
...
class ToolLoaderContext:
    ...
    字段命名与 ``JiuWenSwarmDeepAdapter._get_tool_cards`` 内部使用的开关保持对齐，
    便于与主 Agent 同一套可用性条件。
    """
...
async def load_all(ctx: ToolLoaderContext) -> list[Any]:
    ...
    # ─── jiuwenswarm / openjiuwen.harness 工具（对齐主 Agent）──────────────
    _register_group(tools, ctx, _load_jw_named_web, "named_web")
    _register_group(tools, ctx, _load_jw_paid_search, "paid_search")
    ...
    # ─── openjiuwen filesystem / shell / code ─────────────────────────────
...
def load_send_file_tools(ctx: ToolLoaderContext) -> list[Any] | None:
    """加载 send_file_to_user 工具（每次请求单独刷新，不走 load_all 一次性缓存）.

    Returns:
        工具列表；加载失败返回 None（调用方应保留旧工具，勿当作空列表清空）。
    """
    try:
...
# ────────────────────────── harness / jiuwenswarm loaders ──────────────────────────


def _load_jw_named_web(ctx: ToolLoaderContext) -> list[Any]:
    """Web 工具：web_search / fetch_webpage（对齐主 Agent）。"""
    from openjiuwen.harness.tools import WebFetchWebpageTool, WebFreeSearchTool

    return [
        WebFreeSearchTool(agent_id=ctx.agent_id),
        WebFetchWebpageTool(agent_id=ctx.agent_id),
    ]
...
def _normalize_tool_language(language: str) -> str:
    """Map ToolLoaderContext language → tool/rail language (cn|en)."""
    lang = str(language or "cn").strip().lower()
    if lang in ("cn", "zh", "zh-cn", "zh_cn"):
        return "cn"
    return "en"


def _load_jw_ask_user(ctx: ToolLoaderContext) -> list[Any]:
    """Ask user：对齐主 Agent 的 StructuredAskUserTool（工具名 ask_user）。

    主 Agent 由 StructuredAskUserRail.init 注册；turbo 节点内无该 rail 的
    ability_manager 路径，故在此直接装载同款工具。HITL 打断由
    SkillTurboExecutor 挂载的 StructuredAskUserRail.before_tool_call 负责。
    """
    ...
    return [
        StructuredAskUserTool(
            language=_normalize_tool_language(ctx.language),
            agent_id=ctx.agent_id,
        )
    ]


def _load_jw_paid_search(ctx: ToolLoaderContext) -> list[Any]:
    """付费搜索：对齐主 Agent ``is_paid_search_enabled`` + WebPaidSearchTool。

    与 named_web 的 free search 互补；未启用付费 key 时跳过。
    """
    try:
        ...
        try:
            ...
            logger.debug("[ToolsLoader] is_paid_search_enabled unavailable; skip paid_search")
            return []

    if not is_paid_search_enabled():
        return []

    from openjiuwen.harness.tools import WebPaidSearchTool
...
def _load_jw_send_file(ctx: ToolLoaderContext) -> list[Any]:
    """send_file_to_user：对齐 interface_deep 同款 channel 可用性判断。"""
...
def _load_oj_filesystem(ctx: ToolLoaderContext) -> list[Any]:
    """文件系统工具：与主 Agent 相同，需 sys_operation。"""
