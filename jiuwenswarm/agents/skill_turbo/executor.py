...
class SkillTurboExecutor:
    ...
    def reset_tool_call_counter(self) -> None:
        """重置 use_tool 计数器，供在线单节点执行前调用。

        在线模式每次 execute 只跑一个 PlanNode，且 parent_executor 按 session 缓存
        复用（counter 会跨节点累加）。若不重置，HITL 中断后 resume 重放同一节点时，
        中断前调用过的工具计数已 +1，``_next_tool_call_id`` 会产出与首次中断时不同的
        tool_call_id，导致 ``_consume_pending_resume_input`` 无法命中、审批回复丢失、
        反复弹审批。每次 execute 前清零 counter，使"每个节点从 0 计数"，首次中断与
        resume 重放生成相同 id。

        注意：这依赖单节点内工具按确定性顺序（同步串行）调用。若节点内用
        ``asyncio.gather`` 并发调用同名同参的审批类工具，顺序不定仍可能不稳定
        （不在本次修复范围）。
        """
    ...
    async def use_tool(self, tool_name: str, **kwargs: Any) -> Any:
        ...
        try:
            ...
            # build_tool_ctx / before_tool_call 等外层路径的 HITL 中断
            if trace_status == "ok":
                trace_status = "interrupted"
                trace_error = repr(e)
            raise
        except Exception as e:
            # build_tool_ctx / before_tool_call 等外层路径失败
            if trace_status == "ok":
                trace_status = "error"
                trace_error = repr(e)
            raise
    ...
    def _log_from_node(
        ...
        if log_level not in {"debug", "info", "warning", "error"}:
            logger.warning(
                "[PlanNodeLog] unsupported log level=%r node=%s; message discarded",
                level,
                node.plan_name,
            )
            return
        ...
        session = _session_var.get()
        # Session 通过 get_session_id() 暴露 id（无 .id 属性）；用 getattr(session, "id")
        # 会恒取到空串，导致节点日志无法关联 session。
        session_id = ""
        if session is not None:
            getter = getattr(session, "get_session_id", None)
            if callable(getter):
                try:
                    session_id = getter() or ""
                except Exception:
                    session_id = ""
        log_fn(
