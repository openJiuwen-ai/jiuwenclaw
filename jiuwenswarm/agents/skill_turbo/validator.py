    ...
    def generated_skill_code(cls) -> "CodeValidationPolicy":
        return cls(
            ...
            allowed_import_exact=(
                "jiuwenswarm.agents.skill_turbo.plan_node",
            ),
...
class PlanCodeValidator:
    ...
    def is_import_allowed(self, module: str) -> bool:
        """判定 import 是否通过策略校验（公开 API）.

        用于运行时 import 沙箱（sandbox.py），使 AST 静态校验与运行时拦截
        使用完全一致的策略（exact 白名单 + prefix + 黑名单）。

        Args:
            module: 模块名（绝对导入路径）

        Returns:
            True 表示允许 import，False 表示禁止
        """
        return self._is_import_allowed(module)

    @property
    def policy(self) -> CodeValidationPolicy:
        """访问校验策略（公开 API）.

        用于运行时 import 沙箱（sandbox.py）读取 deny_relative_import 配置。
        """
        return self._policy
    ...
    def _check_call(self, node: ast.Call, errors: list[str]) -> None:
        ...
        # 拦截 __builtins__['exec'](...) / __builtins__["open"](...) 下标绕过
        if isinstance(func, ast.Subscript):
            value = func.value
            if isinstance(value, ast.Name) and value.id == "__builtins__":
                key = self._literal_str_from_slice(func.slice)
                if key and key in self._policy.denied_call_names:
                    errors.append(
                        f"禁止通过 __builtins__ 调用危险函数: {key} (行 {node.lineno})"
                    )
            return

    @staticmethod
    def _literal_str_from_slice(slice_node: ast.AST) -> str | None:
        """从 Subscript.slice 提取字符串字面量键（兼容 py3.8 Index 与 py3.9+ Constant）."""
        if isinstance(slice_node, ast.Constant) and isinstance(slice_node.value, str):
            return slice_node.value
        # Python < 3.9: ast.Index(value=...)
        ...
        if index_cls is not None and isinstance(slice_node, index_cls):
            ...
            if isinstance(inner, ast.Constant) and isinstance(inner.value, str):
                return inner.value
            if isinstance(inner, ast.Str):  # pragma: no cover - py3.7 legacy
                return inner.s
        return None
    ...
    def _is_broad_handler(handler: ast.ExceptHandler) -> bool:
        ...
        if isinstance(exc_type, ast.Name) and exc_type.id in (
            ...
            return True
        # except (Exception, SomeError) 元组形式也视为宽泛捕获
