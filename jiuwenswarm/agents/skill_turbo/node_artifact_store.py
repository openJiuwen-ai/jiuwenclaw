...
设计原则：
    - 复用 checkpointer 持久化链路（pre_run → update_state → post_run），
      不引入第二份内存存储。
    - 与 ``permission_bridge.set_skill_turbo_id`` 范式一致，共用 checkpointer 隔离 key。
...
async def save_node_artifacts(
    ...
    *,
    ...
    # pre_run 成功后必须配对 post_run。skip_post_run 表示调用方随后会自行 post_run，
    # 因此本地一律不 post_run（无论 update 成功与否），避免 update 失败时本地仍 post_run
    # 与调用方形成重复 post_run / close_stream。
    post_run_locally = not skip_post_run
    update_ok = False
    if skip_post_run:
        logger.debug(
    ...
    try:
        ...
        logger.warning(
            "[SkillTurboArtifacts] save_node_artifacts update_state failed: sid=%s err=%s",
            sid,
            e,
        )

    if not post_run_locally:
        return
    ...
    try:
        ...
        if update_ok:
            logger.info(
                "[SkillTurboArtifacts] save_node_artifacts: persisted OK sid=%s", sid
            )
    except Exception as e:
        logger.error(
            "[SkillTurboArtifacts] save_node_artifacts post_run failed: sid=%s err=%s",
            sid,
            e,
        )
        # update_state 已成功但落盘失败：向上抛出，避免调用方误以为已持久化
        if update_ok:
            raise
