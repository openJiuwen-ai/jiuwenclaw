...
async def _run_node_stream_with_progress(
    ...
    - fallback 兜底：消费 ``fallback.result`` 事件中的 result（不修改 inputs）
    - 默认 _execute_stream：yield 原始 result dict（无元键），直接使用

    Args:
        node: 已加载并绑定 callbacks 的 PlanNode
        inputs: 节点输入（不会被 fallback 原地修改）
    ...
    async for chunk in node.run_stream(inputs):
        ...
        # fallback 事件：result 经 fallback.result 显式返回；其余事件不转发前端
        event_type = chunk.get("event_type")
        if event_type == "fallback.result":
            result = chunk.get("result")
            if isinstance(result, dict):
                node_outputs = {
                    k: v for k, v in result.items() if k not in {"node", "status"}
                }
            continue
        if event_type:
            continue
        ...
        # 对齐批量串行：不打 stream_source_id，由 TaskExecutionRail task.start
        # 的前端任务栈把内容归入当前 stage 子气泡。
...
async def skill_turbo_tool(
    ...
    node = await asyncio.to_thread(
        executor.load_node, turbo_face.turbo_dir, scenario, plan_name, schema
    )
    logger.info(
    ...
    # 重置 tool_call 计数器：在线一次 execute 只跑一个节点，但 parent_executor 按
    # session 缓存复用（counter 跨节点累加）。若不清零，HITL 中断后 resume 重放同一
    # 节点时 tool_call_id 会与中断时不一致，导致审批回复丢失/反复弹审批。
    if parent_executor is not None:
        parent_executor.reset_tool_call_counter()
    ...
    try:
        ...
        try:
            ...
            # Nested ask_user/permission HITL: 透传 ToolInterruptException 给上层
            # harness，由 handler._handle_tool_interrupt_exception 提交中断到前端弹窗。
            # ability_manager line 1315/1423 已 except ToolInterruptException: raise
            # 透传（注释 "Nested HITL e.g. skill_turbo ask_user"）。
            #
            # tool_call 置 None 让 handler line 151 `tc = tool_result.tool_call or
            # tool_call` 用外层 skill_turbo_tool 的 tool_call resume（重新执行
            # skill_turbo_tool），而非 subagent 的 ask_user tool_call（主 agent
            # ability_manager 不认识 subagent 的 tool_call_id，resume 会失败）。
            #
            # resume 时 skill_turbo_tool line 490-515 从 resume_user_input 提取
            # pending_tool_call_id（save_resume_ctx 保存的 subagent 的 id）+
            # user_input，调 parent_executor.set_pending_resume 让 subagent 跳过
            # ask_user 继续。
            #
            # 之前尝试返回结构化结果让主 agent 转发 invoke_tool(ask_user)，但
            # invoke_tool 内部直接调 ask_user.invoke（返回 {}），绕过 ability_manager
            # 的 HITL 拦截，导致不弹窗（progressive_tool_rail._invoke_target_tool
            # line 741 + ask_user_rail.invoke line 217-218）。
            ...
            if tic is not None:
                # 保存断点（subagent 的 ask_user tool_call_id，供 resume 时 set_pending_resume）
                pending_tool_call_id = tic.tool_call.id if tic.tool_call else ""
                if pending_tool_call_id and session is not None:
                    try:
                        ...
                        logger.error(
                            ...
                            "online resume will replay from skill start",
                            plan_name,
                            save_exc,
                        )
                logger.info(
                    "[skill_turbo_tool] nested HITL interrupt, propagate "
                    "ToolInterruptException: plan_name=%s tcid=%s",
                    plan_name,
                    pending_tool_call_id,
                )
                raise ToolInterruptException(tic.request, None) from abort_exc
            # 非 ask_user 的 AbortError（如 permission deny）：透传给上层 harness
            raise
...
def _has_pending_resume(executor: Any) -> bool:
    """判断 executor 是否正挂起等待 HITL 审批（有未消费的 _pending_resume）."""
    return getattr(executor, "_pending_resume", None) is not None


def _evict_one_locked() -> None:
    """从缓存淘汰一个条目（须在 _PARENT_EXECUTOR_CACHE_LOCK 内调用）.

    优先淘汰无挂起 HITL 审批（``_pending_resume is None``）的最旧条目；
    若所有条目都在等待审批，才淘汰绝对最旧者并打 error（这会丢失该 session 的
    resume 断点，属高并发下的兜底降级）。
    """
    for evicted_id, (cache_key, executor) in list(_PARENT_EXECUTOR_CACHE.items()):
        if not _has_pending_resume(executor):
            _PARENT_EXECUTOR_CACHE.pop(evicted_id)
            logger.warning(
                "[skill_turbo_tool] evicted stale executor (cache full): session=%s key=%s",
                evicted_id, cache_key,
            )
            return
    # 全部都在挂起 HITL：淘汰绝对最旧者兜底容量，但会丢失其 resume 断点
    evicted_id, (cache_key, _executor) = _PARENT_EXECUTOR_CACHE.popitem(last=False)
    logger.error(
        "[skill_turbo_tool] evicted ACTIVE HITL executor (cache full, all pending): "
        "session=%s key=%s; its resume checkpoint is lost",
        evicted_id, cache_key,
    )
...
def _bind_online_parent_session(parent_executor: Any, plan_name: str) -> Any:
    ...
    用 SubagentSessionProxy 过滤 answer/complete 等终态事件，但
    ``inject_stream_source_id=False``：对齐批量串行，节点内容靠
    TaskExecutionRail ``task.start`` 任务栈进当前 stage 子气泡，
    不再依赖 stream_source_id 透传。
    ...
    try:
        ...
        if parent_session is not None:
            proxy = SubagentSessionProxy(
                ...
                subagent_id=f"skill_turbo:{plan_name}",
                role_id="skill_turbo",
                inject_stream_source_id=False,
            )
