...
def _iter_candidate_skill_roots() -> list[Path]:
    """获取候选 skill 根目录列表（通过 jiuwenswarm utils）."""
    try:
        from jiuwenswarm.common.utils import get_agent_skills_dir

        skill_dir = get_agent_skills_dir()
        roots = [Path(skill_dir)] if skill_dir else []
        logger.info(
        ...
        logger.warning("[schema_loader] get_agent_skills_dir EXCEPTION: %s", exc, exc_info=True)
        return []
...
def discover_turbo_face(skill_root: str) -> TurboFace | None:
    ...
    # skill 目录直接位于 skill_root 下，无嵌套。若未来注册结构变深需调整此处遍历深度。
    candidates: list[Path] = []
    if (root_path / "turbo" / "SKILL_TURBO.md").exists():
        candidates.append(root_path)
    # 也检查子目录（仅一层）
    ...
    for skill_dir in candidates:
        ...
        try:
            scenario_count = sum(
                1 for _ in turbo_dir.glob("schema_*.json") if _.is_file()
            )
        except Exception as exc:
            logger.debug(
...
def discover_all_turbo_faces(skill_roots: list[str] | None = None) -> list[TurboFace]:
    ...
    # 缓存未命中或失效，重新扫描
    faces = _discover_all_turbo_faces_uncached(skill_roots)
    mtime_snapshot = _get_mtime_snapshot(roots)
    _TURBO_FACES_CACHE[cache_key] = (now, mtime_snapshot, faces)
    logger.info(
        "[schema_loader] turbo faces cached: %d faces from %d roots",
        len(faces), len(roots)
    )
    return faces
