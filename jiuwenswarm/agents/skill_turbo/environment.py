...
if TYPE_CHECKING:
    from jiuwenswarm.agents.skill_turbo.tools_loader import ToolLoaderContext
...
def _normalize_tool_output_data(result: Any) -> Any:
    """Normalize harness ToolOutput shapes for skill_turbo / plan_code parsers.

    agent-core BashTool (and similar tools) may return
    ``ToolOutput(data={"content": ...})`` without ``stdout`` / ``exit_code``.
    Skill turbo plan_code often only reads ``stdout`` first and short-circuits
    before any ``content`` fallback, so empty stdout looks like a failed command.

    Rules (non-destructive):
    - Only touch dict-like ``data`` payloads
    - Mirror ``content`` → ``stdout`` when stdout is missing or empty
    - Fill ``exit_code`` from ToolOutput.success when exit_code is absent
    - Never overwrite a non-empty existing ``stdout``
    """
    if result is None:
        return result
    ...
    stdout_empty = not isinstance(stdout, str) or not stdout
    if has_content and stdout_empty:
        normalized["stdout"] = content
        mutated = True

    if "exit_code" not in normalized and isinstance(success, bool):
        normalized["exit_code"] = 0 if success else 1
        mutated = True

    if not mutated:
        return result

    # Prefer in-place mutation for pydantic/BaseModel ToolOutput so callers
    # that keep the original object still see normalized fields.
    try:
        result.data = normalized
        return result
    except Exception:
        logger.debug(
            "[SkillTurboEnvironment] in-place ToolOutput.data mutate failed, fallback copy",
            exc_info=True,
        )
...
def _verify_skill_checksum(pptx_root: str, expected_checksum: str) -> bool:
    ...
    expected_checksum 为空时跳过校验（返回 True，过渡期 fail-open）。
    ...
    if not expected_checksum:
        logger.warning(
            "[SkillTurboEnvironment] skill_checksum 为空，跳过 SHA256 校验（过渡期 fail-open）"
        )
        return True
...
class SkillTurboEnvironment:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        # 没有 card 时 session.pre_run/post_run 会崩溃，影响 checkpointer 持久化
    ...
    def _resolve_skill_root(self) -> str:
        ...
        try:
            ...
            skill_dir = get_agent_skills_dir()
            if skill_dir:
                first_dir = str(Path(skill_dir).resolve())
                logger.info(
            ...
            logger.debug(
                "[SkillTurboEnvironment] get_agent_skills_dir unavailable: %s",
                exc,
            )
    ...
    def get_tool_function(
        ...
        if callable(invoke_fn):
            async def _invoke_adapter(**kwargs: Any) -> Any:
                ...
                raw = await invoke_fn(kwargs)
                return _normalize_tool_output_data(raw)

            return _invoke_adapter

        run_fn = getattr(tool, "run", None)
        if callable(run_fn):
            async def _run_adapter(**kwargs: Any) -> Any:
                raw = run_fn(**kwargs)
    ...
    async def register_tools(self, context: "ToolLoaderContext | None" = None) -> None:
        """装载 jiuwenswarm + openjiuwen 工具到 environment（对齐主 Agent 工具面）。

        - 静态工具仅在首次调用时加载，重复调用幂等；
        - ``send_file_to_user`` 每次请求刷新（依赖 request_id / session_id）；
        - 可用性判断与主 Agent 对齐（由 ``tools_loader`` 承接）；
        - 通过 ``ToolLoaderContext`` 透传 sys_operation / 模型配置 / 各类开关。
        """
        from jiuwenswarm.agents.skill_turbo.tools_loader import (
    ...
    def _refresh_send_file_tools(
        ...
        loader: Callable[[Any], list[Any] | None],
    ) -> None:
        """按请求刷新 send_file_to_user（先加载成功再替换旧实例）。"""
        # 先加载：失败时保留旧工具，避免进入「无 send_file」损坏态。
        try:
            tools = loader(ctx)
        except Exception as exc:
            logger.warning(
                "[SkillTurboEnvironment] load send_file tools failed, keep existing: %s",
                exc,
                exc_info=True,
            )
            return
        if tools is None:
            logger.warning(
                "[SkillTurboEnvironment] load send_file tools returned None, keep existing"
            )
            return
    ...
    def _refresh_skill_checksum_ok(self) -> None:
        """刷新外部 skill 目录 SHA256 校验结果（与扫盘缓存无关）."""
        if self._skill_root:
