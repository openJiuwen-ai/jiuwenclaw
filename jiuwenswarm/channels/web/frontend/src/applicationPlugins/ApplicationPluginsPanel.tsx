...
import { ApplicationPluginConfiguration } from './ApplicationPluginConfiguration';
...
export function ApplicationPluginsPanel({
  ...
  const uniquePlugins = plugins.filter((plugin, index) => plugins.findIndex(candidate => candidate.plugin_id === plugin.plugin_id) === index);
        ...
        {uniquePlugins.map(plugin => (
          <article className="application-plugins-panel__item" key={plugin.plugin_id}>
            <div className="application-plugins-panel__identity">
              <span className="application-plugins-panel__icon">
                <Boxes aria-hidden />
              </span>
              <div>
                <strong>{plugin.title_i18n_key ? t(plugin.title_i18n_key, plugin.title) : plugin.title}</strong>
                <span>
                  {plugin.plugin_id} · v{plugin.plugin_version}
                </span>
              </div>
            ...
            <ApplicationPluginConfiguration contribution={plugin} onManifestChanged={() => void onRefresh()} />
          </article>
        ))}
      </div>
    </section>
