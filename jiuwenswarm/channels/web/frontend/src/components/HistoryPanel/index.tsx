 ...
 * 数据源为 Web Pod 落库的会话历史（企业版 ChatHistoryStore），独立于 WS（AgentServer）链路。
 */
...
export function HistoryPanel() {
  ...
  const loadList = useCallback(async () => {
    ...
    setErrorList(null);
    try {
      // 多用户隔离：显式传入当前 user_id，确保只返回当前用户的会话
      // （defense-in-depth；即使后端已按认证过滤也显式传递）。
      const userId = getRuntimeScope().userId;
      setSessions(await fetchSessions(50, 0, userId));
    } catch (e) {
  ...
  const openDetail = useCallback(
    async (id: string) => {
      ...
      try {
        const d = await fetchSessionDetail(id, getRuntimeScope().userId);
        if (d === null) {
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        ...
        {sessions.map((s) => (
          <button
            ...
            className="w-full text-left rounded-lg border border-border bg-card/70 p-3 hover:border-[var(--color-border-accent)] hover:shadow-sm transition-colors"
          >
