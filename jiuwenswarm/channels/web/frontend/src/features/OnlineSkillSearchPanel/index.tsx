...
export function OnlineSkillSearchPanel({
  ...
  const installSkillNet = useCallback(
    async (item: OnlineSearchItem, force: boolean): Promise<InstallResponse> => {
  ...
  const handleInstall = useCallback(
    async (item: OnlineSearchItem) => {
      ...
      try {
        ...
        while (true) {
          if (item.source === 'skillnet') {
            const data = await installSkillNet(item, force);
            if (!data.success) {
    ...
    [installSkillNet, installingKey, onInstalled, t, withSession]
  );
