...
function normalizeTeamTaskStatus(
  ...
  return typeof status === "string" &&
    TEAM_TASK_STATUS_SET.has(status as TeamTaskStatus)
...
function statusFromTaskEventType(
  type: string,
  explicitStatus: unknown,
): TeamTaskStatus {
  if (type === "team.task.claimed")
    return normalizeTeamTaskStatus(explicitStatus, "claimed");
  if (type === "team.task.completed")
    return normalizeTeamTaskStatus(explicitStatus, "completed");
  if (type === "team.task.cancelled")
    return normalizeTeamTaskStatus(explicitStatus, "cancelled");
  if (type === "team.task.unblocked")
    return normalizeTeamTaskStatus(explicitStatus, "pending");
...
function getShutdownMemberFromToolResult(
  ...
  return (
    parseShutdownMemberName(toolResult.result) ||
    parseShutdownMemberName(toolResult.summary)
  );
}

function upsertTaskRecords(
  values: unknown,
  fallbackStatus: TeamTaskStatus = "pending",
) {
...
function applyTeamTaskToolCall(toolCall: ToolCall) {
  if (toolCall.name === "create_task") {
    upsertTaskRecords(
      Array.isArray(toolCall.arguments.tasks)
  ...
  if (toolCall.name === "update_task") {
    ...
    const existingStatus = taskId
      ? useSessionStore
          .getState()
          .teamTasks.find((task) => task.task_id === taskId)?.status
      : undefined;
...
function buildPersistedMediaFiles(
  mediaItems: MediaItem[],
...
function unsupportedEvolutionModeMessage(
  content: string,
  mode: AgentMode,
): string | null {
...
function isTeamTeammateMessagePayload(
  payload: Record<string, unknown>,
): boolean {
  return (
    typeof payload.role === "string" &&
    payload.role.trim().toLowerCase() === "teammate"
  );
}

function isHiddenTeamTeammateMessagePayload(
  mode: AgentMode,
  payload: Record<string, unknown>,
): boolean {
...
function getTeamPayloadMemberName(
  payload: Record<string, unknown>,
): string | undefined {
  return pickString(
    payload.member_name,
...
export function useWebSocket(options: UseWebSocketOptions): UseWebSocketReturn {
  ...
  const symphonyStatusTargetRef = useRef<
    Map<string, { messageId: string; baseContent: string }>
  >(new Map());
  ...
  const pendingTeamMemberContextCompressionStartRef = useRef<
    Map<string, PendingContextCompressionStart>
  >(new Map());
  ...
  const clearPendingTeamMemberContextCompressionStart = useCallback(
    (memberId: string) => {
      const pending =
        pendingTeamMemberContextCompressionStartRef.current.get(memberId);
      if (!pending) return;
      clearTimeout(pending.timer);
  ...
  const buildContextCompressionRuntimeState = useCallback(
    ...
    ): Omit<ContextCompressionRuntime, "status"> | null => {
  ...
  const findExistingTeamMemberId = useCallback(
    (memberName: unknown): string | null => {
  ...
  const handleTeamMemberContextCompressionState = useCallback(
    (payload: ContextCompressionStatePayload, memberId: string) => {
      ...
      if (status === "completed") {
        ...
        const current =
          useSessionStore.getState().teamMemberContextCompression[memberId]
            ?.summary;
        const nextSummary = {
      ...
      if (status === "started" || status === "running") {
        ...
        const pending: PendingContextCompressionStart = {
          ...
          timer: setTimeout(() => {
            if (
              pendingTeamMemberContextCompressionStartRef.current.get(
                memberId,
              ) !== pending
            )
              return;
        ...
        pendingTeamMemberContextCompressionStartRef.current.set(
          memberId,
          pending,
        );
        return;
  ...
  const sendMessage = useCallback(
    async (
      ...
      const unsupportedEvolutionMode = unsupportedEvolutionModeMessage(
        content,
        currentMode,
      );
    ...
    async (
      ...
      try {
        ...
        const effectiveSource =
          source ?? (pendingMatches ? pendingQuestion?.source : undefined);
        const approvalSchema = pendingMatches
          ? pendingQuestion?.approvalSchema
          : undefined;
        const evolutionMeta = pendingMatches
        ...
        const evolutionMetaPayload =
          evolutionMeta && typeof evolutionMeta === "object"
            ...
            : {};
        const approvalSchemaPayload = approvalSchema
          ? { approval_schema: approvalSchema }
          : {};
        const sourcePayload = effectiveSource
        ...
        if (
          ...
          (effectiveSource === "skill_evolution_approval" &&
            approvalTransport === "interrupt")
        ) {
          ...
          const action =
            answers[0]?.selected_options[0] === "拒绝" ? "reject" : "accept";
          const interactionId =
            requestId ||
            useHarnessStore.getState().activateInteraction?.interactionId ||
            "";
          if (!interactionId) {
        ...
        setConnectionStats({ lastError: webError.message });
        onErrorRef.current?.(
          webError.message || t("network.submitAnswerFailed"),
        );
      }
    },
  ...
  const shouldDropDuplicatedEvent = useCallback(
    (eventName: string, payload: Record<string, unknown>): boolean => {
      ...
      const dropped =
        lastSeen != null && now - lastSeen <= EVENT_DEDUP_WINDOW_MS;
  ...
  const shouldRecoverProcessingFromReasoning = useCallback(
    (payload: Record<string, unknown>): boolean => {
  ...
  const appendTeamMemberOutputDelta = useCallback(
    (payload: Record<string, unknown>, memberId: string, content: string) => {
      ...
      const existingContent =
        useSessionStore
          .getState()
          .teamMemberExecutionEvents.find((event) => event.id === id)
          ?.content || "";
  ...
  useEffect(() => {
    ...
    const isTeamPanelClearedForPayload = (payload: Record<string, unknown>) => {
      const sessionId =
        getPayloadSessionId(payload) || activeSessionIdRef.current || undefined;
      return Boolean(
        sessionId && clearedTeamPanelSessionRef.current === sessionId,
      );
    };
    ...
    const unsubs = [
      ...
      webClient.on("chat.delta", ({ payload }) => {
        ...
        if (
          !useChatStore.getState().isProcessing &&
      ...
      webClient.on("chat.final", ({ payload }) => {
        ...
        if (
          ...
          const sessionId =
            pickString(payload.session_id) || activeSessionIdRef.current || "";
        ...
        if (isHiddenTeamTeammateMessagePayload(currentMode, payload)) {
          ...
          if (memberId) {
            ...
            const outputEventId = takeTeamMemberOutputEventId(
              payload,
              memberId,
            );
            if (!content.trim()) {
        ...
        const proactiveType =
          typeof payload.proactive_type === "string"
            ? payload.proactive_type
            : "";
          ...
          updateMessage(streamId, {
            ...
            ...(isProactiveRecommendation
              ? {
                  ...
                  ...(proactiveType
                    ? {
                        proactiveType: proactiveType as
                          | "skill_recommend"
                          | "task_reminder"
                          | "need_exploration",
                      }
        ...
        if (content) {
          ...
          if (!isCronPlaceholderContent) {
            ...
            if (cronRunId) {
              const byRun = messages.find(
                (m) => m.id === `cron-placeholder-${cronRunId}`,
              );
          ...
          const isProactiveRecommendation =
          ...
          addMessage({
            ...
            ...(proactiveType
              ? {
                  proactiveType: proactiveType as
                    "skill_recommend" | "task_reminder" | "need_exploration",
                }
              : {}),
          });
          if (!content.includes("MEDIA:")) {
      ...
      webClient.on("chat.tool_call", ({ payload }) => {
        ...
        if (isHiddenTeamTeammateMessagePayload(currentMode, payload)) {
          if (
            currentMode === "team" &&
            !isTeamPanelClearedForPayload(payload)
          ) {
            applyTeamTaskToolCall(toolCall);
          }
          const memberId =
            getTeamPayloadMemberName(payload) || toolCall.memberName;
          if (memberId) {
            ...
            useSessionStore.getState().addTeamMemberExecutionEvent({
              ...
              kind: "tool_call",
              timestamp,
              title: t("team.process.execution.toolCallTitle", {
                tool: toolCall.name,
              }),
              content:
                toolCall.description ||
                toolCall.formatted_args ||
                stringifyCompact(toolCall.arguments),
        ...
        const toolRequestId =
          getPayloadRequestId(payload) || activeRequestIdRef.current;
        const { currentStreamId, messages } = useChatStore.getState();
      ...
      webClient.on("chat.tool_result", ({ payload }) => {
        ...
        const shutdownMemberId =
          (toolResult.toolCallId
            ...
            : undefined) || getShutdownMemberFromToolResult(toolResult);
        if (isHiddenTeamTeammateMessagePayload(currentMode, payload)) {
          const memberId =
            getTeamPayloadMemberName(payload) ||
            (toolResult.toolCallId
              ? teamToolCallMemberRef.current.get(toolResult.toolCallId)
              : undefined);
          if (memberId) {
            const timestamp = eventTimestampMs(payload);
            useSessionStore.getState().addTeamMemberExecutionEvent({
              ...
              kind: "tool_result",
              timestamp,
              title: t("team.process.execution.toolResultTitle", {
                tool: toolResult.toolName,
              }),
              content:
                toolResult.summary || stringifyCompact(toolResult.result),
      ...
      webClient.on("context.usage", ({ payload }) => {
        ...
        const contextMax =
          ...
          Number.isFinite(payload.context_max)
            ? payload.context_max
            : null;
        const tokensUsed =
          typeof payload.tokens_used === "number" &&
          Number.isFinite(payload.tokens_used)
            ? payload.tokens_used
            : null;
        const stats = {
          rate,
      ...
      webClient.on<ContextCompressionStatePayload>(
        ...
        ({ payload }) => {
          ...
          if (isTeamTeammateMessagePayload(payload)) {
            const memberId = findExistingTeamMemberId(
              getTeamPayloadMemberName(payload),
            );
            if (!memberId) return;
      ...
      webClient.on("chat.processing_status", ({ payload }) => {
        if (!shouldHandleSessionEvent(payload)) return;
        if (shouldDropDuplicatedEvent("chat.processing_status", payload))
          return;
        // 切换模式时忽略处理状态更新
      ...
      webClient.on("chat.symphony_status", ({ payload }) => {
        ...
        const targetMessage = cachedTarget
          ? chatState.messages.find(
              (message) => message.id === cachedTarget.messageId,
            )
          : [...chatState.messages]
              .reverse()
              .find(
      ...
      webClient.on("chat.notice", ({ payload }) => {
        ...
        if (!content) return;
        const noticeType =
          pickString(payload.notice_type, payload.type) || "notice";
      ...
      webClient.on("chat.error", ({ payload }) => {
        ...
        if (
          errorMsg.includes("invalid page_idx or session history not found")
        ) {
      ...
      webClient.on("security.alert", ({ payload }) => {
        ...
        const alertMsg =
          typeof payload.message === "string" ? payload.message : "安全警告";
      ...
      webClient.on("chat.ask_user_question", ({ payload }) => {
        ...
        const evolutionMeta =
          questionPayload.evolution_meta &&
          typeof questionPayload.evolution_meta === "object"
            ? (questionPayload.evolution_meta as Record<string, unknown>)
            : questionPayload._evolution_meta &&
                typeof questionPayload._evolution_meta === "object"
              ? (questionPayload._evolution_meta as Record<string, unknown>)
              : undefined;
        const questions = Array.isArray(questionPayload.questions)
        ...
        const planContent =
          typeof questionPayload.plan_content === "string"
            ...
            : undefined;
        const planLanguage =
          questionPayload.plan_language === "cn" ||
          questionPayload.plan_language === "en"
            ? questionPayload.plan_language
            : undefined;
        const normalizedPayload: AskUserQuestionPayload = {
          request_id:
            typeof questionPayload.request_id === "string"
              ? questionPayload.request_id
              : "",
          source:
            typeof questionPayload.source === "string"
              ? questionPayload.source
              : undefined,
          questions,
      ...
      webClient.on("proactive_recommendation", ({ payload }) => {
        ...
        const proactiveType =
          typeof payload.proactive_type === "string"
            ? payload.proactive_type
            : "";
        const proactiveTarget =
          typeof payload.proactive_target === "string"
            ? payload.proactive_target
            : "";
        const proactiveReason =
          typeof payload.proactive_reason === "string"
            ? payload.proactive_reason
            : "";
      ...
      webClient.on("team.member", ({ payload }) => {
        ...
        if (event) {
          ...
          if (e.type === "team.member.shutdown" && e.member_id) {
            ...
            activeSessionId &&
            clearedTeamPanelSessionRef.current === activeSessionId
          ) {
            return;
          } else if (
            e.type === "team.member.status_changed" &&
            e.member_id &&
            e.new_status
          ) {
            useSessionStore
              .getState()
              .updateTeamMemberStatus(e.member_id, e.new_status, e.timestamp);
          } else if (
            e.type === "team.member.execution_changed" &&
            e.member_id
          ) {
            const existingMember = useSessionStore
              .getState()
        ...
        if (Array.isArray(rawStages) && rawStages.length > 0) {
          ...
          for (const s of rawStages) {
            if (typeof s === "object" && s !== null) {
              ...
              if (slot)
                stages.push({ slot, display_name: displayName || slot });
            }
          }
          if (stages.length > 0)
            useHarnessStore.getState().setStageDefinitions(stages);
        }
      ...
      webClient.on("harness.stage_result", ({ payload }) => {
        ...
        const messages = Array.isArray(payload.messages)
          ? payload.messages.filter((m) => typeof m === "string")
          : [];
        const metrics =
        ...
        const extensionName =
          typeof payload.extension_name === "string"
            ? payload.extension_name
            : "";
        const extensionStage =
          typeof payload.extension_stage === "string"
            ? payload.extension_stage
            : "";
        const parentStage =
          typeof payload.parent_stage === "string" ? payload.parent_stage : "";
        const taskId =
        ...
        if (stage) {
          useHarnessStore.getState().updateStageResult({
            stage,
            status: status as
              "running" | "success" | "failed" | "timeout" | "pending",
            error,
            messages,
            metrics,
          });
          ...
          console.warn(
            "[harness.stage_result] No stage field in payload, skipping update",
          );
        }
      }),
      webClient.on("harness.extension_ready", ({ payload }) => {
        ...
        const extensionName =
          typeof payload.extension_name === "string"
            ...
            : "";
        const runtimePath =
          typeof payload.runtime_path === "string" ? payload.runtime_path : "";
        const sessionRuntimePath =
          typeof payload.session_runtime_path === "string"
            ? payload.session_runtime_path
            : runtimePath;
        const extensionRuntimePath =
          typeof payload.extension_runtime_path === "string"
            ? payload.extension_runtime_path
            : "";
        const configPath =
          typeof payload.config_path === "string" ? payload.config_path : "";
        const runtimeExtensions = Array.isArray(payload.runtime_extensions)
          ? payload.runtime_extensions
              ...
              .map((item) => {
                ...
                return {
                  extensionName:
                    typeof obj.extension_name === "string"
                      ? obj.extension_name
                      : "",
                  runtimePath:
                    typeof obj.runtime_path === "string"
                      ? obj.runtime_path
                      : "",
                  configPath:
                    typeof obj.config_path === "string" ? obj.config_path : "",
                };
              })
              .filter((item) => item.extensionName && item.runtimePath)
          : [];
        const verifyReport =
          typeof payload.verify_report === "object" &&
          payload.verify_report !== null &&
          !Array.isArray(payload.verify_report)
            ? (payload.verify_report as Record<string, unknown>)
            : {};
        const componentsSummary =
          typeof payload.components_summary === "object" &&
          payload.components_summary !== null &&
          !Array.isArray(payload.components_summary)
      ...
      webClient.on("harness.activate_interaction", ({ payload }) => {
        ...
        const runtimePath =
          typeof payload.runtime_path === "string" ? payload.runtime_path : "";
        const options: string[] = Array.isArray(payload.options)
          ? payload.options
          : ["accept", "reject"];
          ...
          questions: [
            {
              ...
              options: options.map((opt: string) => ({
                label:
                  opt === "accept" ? "激活" : opt === "reject" ? "拒绝" : opt,
                description: "",
              })),
            },
          ],
