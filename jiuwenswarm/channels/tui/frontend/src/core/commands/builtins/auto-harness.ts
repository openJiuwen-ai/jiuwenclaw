...
import {
  ...
  ISSUE_FIX_WATCH_INTERVAL_MS,
  ISSUE_FIX_WATCH_MAX_POLLS,
  type IssueFixTaskStatus,
  type IssueFixWatchItem,
  formatIssueFixStatusBlock,
  formatIssueWatchResult,
  isIssueFixWatchDone,
  issueFixTaskStatusToWatchItem,
...
const issueWatchOnceCommand: SlashCommand = {
  ...
  takesArgs: true,
  action: async (ctx, args) => {
    const parsed = parseIssueWatchArgs(args, resolvePipelineName);
...
const issueFixCommand: SlashCommand = {
  ...
  action: async (ctx, args) => {
    ...
    const watchItems = parsed.issue_numbers.map((issue) => {
      ...
      return taskId
        ? issueFixTaskStatusToWatchItem(issue, taskId, undefined, "queued")
        : issueFixTaskStatusToWatchItem(
            issue,
            undefined,
            undefined,
    ...
    for (let poll = 0; poll < ISSUE_FIX_WATCH_MAX_POLLS; poll++) {
      ...
      for (const issue of parsed.issue_numbers) {
        ...
        updatedItems.push(issueFixTaskStatusToWatchItem(issue, taskId, task, "queued"));
      }
