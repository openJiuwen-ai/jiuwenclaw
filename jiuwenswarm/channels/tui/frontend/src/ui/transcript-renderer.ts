...
export function buildTranscriptLines(
  ...
  const allLines: string[] = [...buildWelcomeLines(width, snapshot.connectionStatus, snapshot.modelInfo, snapshot.mode, snapshot.memoryWarnings, snapshot.preferredLanguage, snapshot.selectedAgentosModel)];
