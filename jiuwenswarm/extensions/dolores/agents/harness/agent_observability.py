...
This module handles observability for the single-agent (``mode=agent``) and
coding-agent runs. It is kept in a **separate file with its own state and
config section** on purpose, so the agent scenario stays self-contained.
...
``core/foundation/tool/base.py``) for *every* agent — so simply
...
LLM/tool span tracing.

Shared-provider caveat (important):
    OpenTelemetry allows exactly ONE global ``TracerProvider`` per process,
    and ``init_observability`` is a no-op if already initialized. We track
    ``_agent_owns_provider``: agent shutdown only tears down the provider
    when the agent actually created it.
"""
...
from jiuwenswarm.extensions.dolores.common.utils import get_user_workspace_dir
