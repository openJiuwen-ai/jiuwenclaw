...
if TYPE_CHECKING:
    ...
    from jiuwenswarm.extensions.dolores.server.runtime.agent_config_service import AgentDefinition
...
from jiuwenswarm.extensions.dolores.agents.harness.common.tools.cron.cron_runtime import CronRuntimeBridge
from jiuwenswarm.extensions.dolores.agents.harness.common.rails.interrupt.interrupt_helpers import (
    build_permission_rail,
    convert_interactions_to_ask_user_question,
)
from jiuwenswarm.extensions.dolores.agents.harness.common.tools.todo_compat import (
    CompatibleTodoModifyTool,
    install_todo_modify_compat_patch,
)
from jiuwenswarm.extensions.dolores.agents.harness.common.prompt.prompt_builder import build_agent_identity_prompt
from jiuwenswarm.extensions.dolores.agents.harness.common.rails import (
...
from jiuwenswarm.extensions.dolores.agents.harness.common.rails.execution_guard import (
    CircuitBreakerRail,
    CircuitBreakerConfig,
)
from jiuwenswarm.extensions.dolores.common.config import get_model_names
from jiuwenswarm.extensions.dolores.common.hooks_config import load_hooks_config
...
from jiuwenswarm.extensions.dolores.agents.harness.common.memory.config import (
    clear_config_cache,
    get_memory_mode,
    is_memory_enabled,
    is_proactive_memory,
)
from jiuwenswarm.extensions.dolores.agents.harness.common.memory.external_memory_config import is_builtin_memory_allowed
from jiuwenswarm.extensions.dolores.common.model_config_validation import is_placeholder_api_base
from jiuwenswarm.extensions.dolores.agents.harness.common.rails.permissions.tool_permission_context import TOOL_PERMISSION_CHANNEL_ID
from jiuwenswarm.extensions.dolores.server.runtime.session.session_metadata import build_server_push_message
from jiuwenswarm.extensions.dolores.server.runtime.session.session_history import append_history_record, load_history_records
from jiuwenswarm.extensions.dolores.server.runtime.skill.skill_manager import SkillManager
from jiuwenswarm.extensions.dolores.server.runtime.prompt_attachment_loader import PromptAttachmentLoader
from jiuwenswarm.extensions.dolores.server.runtime.prewarm import PrewarmConfig, PrewarmCoordinator, PrewarmRail
from jiuwenswarm.extensions.dolores.server.utils.stream_utils import parse_ask_user_question_payload
from jiuwenswarm.extensions.dolores.agents.harness.common.tools.multimodal_config import (
...
from jiuwenswarm.extensions.dolores.agents.harness.common.tools.video_tools import video_understanding
from jiuwenswarm.extensions.dolores.agents.harness.common.tools.image_tools import generate_image

from jiuwenswarm.extensions.dolores.agents.harness.common.tools import (
    SendFileToolkit,
    SkillRetrievalToolkit,
    SkillToolkit,
    is_skill_retrieval_enabled,
)
from jiuwenswarm.extensions.dolores.agents.harness.common.tools.wiki_tools import wiki_ingest, wiki_query, wiki_lint
from jiuwenswarm.extensions.dolores.agents.harness.common.tools.acp_output_tools import get_tools as get_acp_output_tools
from jiuwenswarm.extensions.dolores.agents.harness.common.tools.acp_chat import acp_chat
from jiuwenswarm.extensions.dolores.agents.harness.common.tools.xiaoyi_phone_tools import (
    get_user_location,
    create_note,
    search_notes,
    modify_note,
...
from jiuwenswarm.extensions.dolores.common.config import (
    get_config,
    get_default_models,
    get_sandbox_runtime,
...
from jiuwenswarm.extensions.dolores.common.mcp_config import (
...
from jiuwenswarm.extensions.dolores.agents.harness.common.plugins.rail_manager import get_rail_manager
...
from jiuwenswarm.extensions.dolores.common.schema.message import ReqMethod
...
class JiuWenSwarmDeepAdapter:
    ...
    def __init__(self) -> None:
        ...
        self._llm_retry_rail: ModelAnomalyDetectionRail | None = None
    ...
    def _build_configured_subagents(
        ...
        if headless:
            ...
            try:
                ...
                from jiuwenswarm.extensions.dolores.common.utils import get_user_workspace_dir as _get_ws
    ...
    def _inject_attribution_to_config(config: dict) -> None:
        ...
        from jiuwenswarm.extensions.dolores.common.openrouter_attribution import inject_attribution_to_config
    ...
    def _resolve_model_for_request(self, request: AgentRequest) -> Model:
        ...
        # 2/3) 预配模型按名选择 / 回退默认（委托 _resolve_model_by_name）
        requested = (params.get("model_name") or "").strip()
        model = self._resolve_model_by_name(requested)
        return model if model is not None else self._model
    ...
    def _build_tokenjuice_rail(config: dict[str, Any] | None = None) -> Any | None:
        ...
        try:
            from jiuwenswarm.extensions.dolores.agents.harness.common.rails.tokenjuice_rail import TokenJuiceRail
    ...
    def _build_avatar_rail() -> Any | None:
        ...
        try:
            from jiuwenswarm.extensions.dolores.agents.harness.common.rails.avatar_rail import AvatarPromptRail
    ...
    @staticmethod
    def _build_llm_retry_rail(config_base: dict[str, Any] | None = None) -> ModelAnomalyDetectionRail | None:
        try:
            ...
            logger.warning("[JiuWenSwarmDeepAdapter] ModelAnomalyDetectionRail create failed: %s", exc)
            return None
    ...
    def _resolve_enable_task_loop(
        ...
        """Resolve enable_task_loop considering skill_create rail requirements.

        SkillCreateRail follow-ups require task-loop mode (enable_task_loop=True)
        because they use AFTER_TASK_ITERATION events and enqueue_follow_up().
        When skill_create=True, we force enable_task_loop=True regardless of user
        config.
        ...
        if os.getenv("JIUWENSWARM_AGENT_KIND", "").strip().lower() == "dolores":
            ...
            self._instance = create_deep_agent(
                **common_kwargs,
                ...
                audio_model_config=self._audio_model_config,
                enable_read_image_multimodal=self._resolve_enable_read_image_multimodal(config),
                enable_model_anomaly_detection_rail=((config_base.get("execution_guard") or {}).get("llm_retry_rail") or {}).get(
                    "enabled", False
                ),
                completion_timeout=config.get("completion_timeout", 3600.0),
            )
    ...
    async def process_message_impl(
        ...
        try:
            ...
            from jiuwenswarm.extensions.dolores.agents.harness.agent_observability import (
    ...
    async def process_message_stream_impl(
        ...
        try:
            ...
            from jiuwenswarm.extensions.dolores.server.runtime.debug_trace.config import (
            ...
            if _run_span is not None:
                try:
                    ...
                    pass
            try:
                from jiuwenswarm.extensions.dolores.server.runtime.debug_trace.context import (
                    set_debug_trace_logger,
                )
                from jiuwenswarm.extensions.dolores.server.runtime.debug_trace.paths import debug_trace_file
                from jiuwenswarm.extensions.dolores.server.runtime.debug_trace.stream_logger import (
                    DebugTraceLogger,
                )
    ...
    def _parse_stream_chunk(
        ...
        *,
        ...
        Args:
            ...
            _stage: 当前阶段名称，用于 harness.message 事件
    ...
    def _build_external_memory_rail(self):
        from jiuwenswarm.extensions.dolores.agents.harness.common.memory.external_memory_builder import (
    ...
    async def _handle_external_memory_rail_by_config(self):
        ...
        """
        from jiuwenswarm.extensions.dolores.agents.harness.common.memory.external_memory_config import (
    ...
    async def generate_recap(self, session_id: str) -> dict[str, Any]:
        ...
        from jiuwenswarm.extensions.dolores.server.runtime.agent_adapter.recap_prompts import (
    ...
    async def compact_partial(
        ...
        """
        from jiuwenswarm.extensions.dolores.agents.harness.common.session_ops_service import (
            get_agent_sessions_dir,
        )
        from jiuwenswarm.extensions.dolores.server.runtime.session.session_history import _read_history
        from jiuwenswarm.extensions.dolores.server.runtime.agent_adapter.compact_partial_prompts import (
            NO_TOOLS_PREAMBLE,
    ...
    async def try_start_dreaming(self, busy_checker: Callable[[], bool] | None = None) -> None:
        if self._dreaming_started:
            return
        try:
            from jiuwenswarm.extensions.dolores.agents.harness.common.memory.dreaming import start_dreaming
    ...
    async def try_stop_dreaming(self) -> None:
        if not self._dreaming_started:
            return
        try:
            from jiuwenswarm.extensions.dolores.agents.harness.common.memory.dreaming import stop_dreaming
            mode = getattr(self, "_dreaming_mode", "agent")
...
def _load_custom_subagents(
    ...
    **kwargs: Any,
    ...
    """
    from jiuwenswarm.extensions.dolores.server.runtime.agent_config_service import AgentConfigService
...
# --- DoloresAgent: self-contained local fork of the DeepAdapter ---
DoloresAgent = JiuWenSwarmDeepAdapter
DoloresAdapter = JiuWenSwarmDeepAdapter
