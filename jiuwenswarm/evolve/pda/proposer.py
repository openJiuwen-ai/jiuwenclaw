...
"""PdaProposer — self-contained CLEAN→EVAL→DIAG→GOV→PROPOSE pipeline.
...
from jiuwenswarm.evolve.pda.proposer_prompts import PDA_PROPOSER_SYSTEM_PROMPT
...
class PdaProposer(ProposalGenerator):
    """PDA-style One-shot AHE — self-contained CLEAN->EVAL->DIAG->GOV->PROPOSE.
    ...
    async def generate(self, batch: TraceBatch) -> list[Proposal]:
        ...
        logger.info("PdaProposer: processing %d traces", len(trace_ids))
        ...
        for tid in trace_ids:
            try:
                ...
                logger.warning("PdaProposer: CLEAN failed for %s: %s", tid, exc)

        if not normalized_traces:
            logger.info("PdaProposer: no traces could be normalized")
            return []
        ...
        if not failed:
            logger.info("PdaProposer: no fail/uncertain traces, skipping")
            return []

        logger.info("PdaProposer: %d of %d traces are fail/uncertain",
        ...
        if diagnosis_result.budget_exceeded:
            logger.warning("PdaProposer: DiagnosisAgent budget exceeded")
        ...
        logger.info("PdaProposer: generated %d proposals", len(proposals))
        return proposals
    ...
    async def _call_llm_propose(
        ...
        try:
            ...
            logger.warning("PdaProposer: openjiuwen.llm not available")
            return []
        ...
        try:
            messages = [
                SystemMessage(content=PDA_PROPOSER_SYSTEM_PROMPT),
            ...
            logger.warning("PdaProposer._call_llm_propose failed: %s", exc)
            return []
    ...
    def _parse_proposals(
        ...
        for raw in raw_proposals:
            try:
                ...
                logger.warning("PdaProposer: failed to parse proposal: %s", exc)
        return proposals
    ...
    def _parse_llm_json(text: str) -> dict:
        ...
        logger.warning("PdaProposer: failed to parse JSON from LLM response")
        return {"proposals": []}
    ...
    async def _init_model(self):
        ...
        try:
            ...
            logger.warning("PdaProposer._init_model failed: %s", exc)
            return None
