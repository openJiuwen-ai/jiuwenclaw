...
"""PdaDecisionPolicy — RuleGate + LLMDecision two-phase judgment.
...
class PdaDecisionPolicy(DecisionPolicy):
    """PDA-style Decision — RuleGate + LLMDecision two-phase judgment.
    ...
    def __init__(
        ...
        super().__init__(name="pda_decision_policy", version="1.0")
    ...
    async def _llm_decision(self, proposal: Proposal) -> DecisionResult:
        ...
        if self._model is None:
            logger.warning("PdaDecisionPolicy: no model for LLM decision, using RuleGate result")
            return DecisionResult(
        ...
        try:
            ...
            logger.warning("PdaDecisionPolicy._llm_decision failed: %s", exc)
            return DecisionResult(
    ...
    async def _init_model(self):
        ...
        try:
            ...
            logger.warning("PdaDecisionPolicy._init_model failed: %s", exc)
            return None
