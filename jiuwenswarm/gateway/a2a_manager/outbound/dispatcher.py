...
class A2AOutboundDispatcher:
    ...
    async def find_agents(
        ...
        for agent in await self._repository.list_agents():
            ...
            search_parts = [agent.display_name, str(card.get("description") or "")]
            for skill in skills:
                skill_values = [
                    str(skill.get(key) or "")
                    for key in ("id", "name", "description")
                ]
                skill_values.extend(str(tag) for tag in skill.get("tags") or [])
                search_parts.append(" ".join(skill_values))
                for value in (skill_values[0], skill_values[1], *skill_values[3:]):
                    if value:
                        skill_tokens.add(value.lower())
        ...
        try:
            ...
            if create_task is not None:
                try:
                    ...
                    logger.warning(
                        "a2a.outbound dispatch record creation failed during cancellation",
                        exc_info=True,
                    )
        ...
        try:
            ...
            for item in parsed.supported_interfaces:
                if item.url != selected.url:
                    continue
                if (
                    item.protocol_binding.upper()
                    == selected.protocol_binding.upper()
                ):
    ...
    async def _settle_cancelled(
        ...
        if client is not None and remote.remote_task_id:
            try:
                ...
                logger.warning(
                    "a2a.outbound remote cancellation failed: dispatch_id=%s",
                    dispatch_id,
                    exc_info=True,
                )
        await self._repository.transition_dispatch(
