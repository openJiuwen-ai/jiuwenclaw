...
def normalize_chat_media_attachments(params: dict[str, Any], session_id: str | None) -> None:
    ...
    content_text = (
        ...
        else str(original_content or "").strip()
    )
    content = _append_image_instruction(content_text, stored)
...
def _append_image_instruction(content: str, items: list[dict[str, Any]]) -> str:
    lines = [
        content or "请分析我上传的图片。",
        "",
        "用户随消息上传了以下图片，"
        "请在需要理解图片内容时调用 visual_question_answering 工具读取这些本地路径：",
    ]
    for index, item in enumerate(items, start=1):
        lines.append(f"{index}. {item.get('filename')}: {item.get('path')}")
    return "\n".join(lines).strip()
