    ...
    *,
    ...
    if history_store is not None and history_store.available:
        ...
        if backend == "sqlite":
            ...
            settings = history_store._settings
        ...
        logger.error(
            "[jiuwenclaw-enterprise-web] 企业模式未配置 MySQL（缺少 WEB_DB_HOST），"
            "会话历史不会落盘；聊天不受影响（不回退 SQLite）"
        )
    try:
...
def main() -> None:
    ...
    if history_store.available:
        ...
        elif history_store.backend == "mysql" and history_store._settings is not None:
            s = history_store._settings
            logger.info(
                "[jiuwenclaw-enterprise-web] history backend=mysql %s:%s/%s",
