...
职责：
- ``ChatHistoryStore``：按部署形态分支存储 sessions / messages，幂等去重（由 ws loop 写）。
  - 单机（``deployment_mode=standalone``）：本机 SQLite 文件
  - 企业（``active-standby`` / ``distributed``）：MySQL 独立库 ``web``（``WEB_DB_*``）
...
def resolve_history_deployment_mode() -> str:
    """解析当前部署模式（与 gateway 配置对齐）。

    优先 ``DEPLOYMENT_MODE`` 环境变量，其次 ``config.yaml`` 的 ``gateway.deployment_mode``，
    非法/缺失时回退 ``standalone``。
    """
    env_mode = os.getenv("DEPLOYMENT_MODE", "").strip().lower()
    if env_mode in VALID_DEPLOYMENT_MODES:
        return env_mode
    try:
        ...
        logger.debug("[history] 读取 gateway.deployment_mode 失败，回退 standalone", exc_info=True)
    return MODE_STANDALONE


def default_history_sqlite_path() -> Path:
    """单机历史 SQLite 默认路径：``workspace_default/web_history.db``（可被环境变量覆盖）。"""
...
class ChatHistoryStore:
    """会话历史存储：单机 SQLite / 企业 MySQL；单测可用 memory=True。"""
    ...
    @classmethod
    def for_deployment_mode(cls, mode: str) -> ChatHistoryStore:
        """按部署模式构造：standalone → SQLite；企业模式 → MySQL（缺 WEB_DB_HOST 则不可用）。"""
        if history_storage_backend(mode) == "mysql":
    ...
    def from_env(cls) -> ChatHistoryStore:
        return cls.for_deployment_mode(resolve_history_deployment_mode())
    ...
    def _get_session_detail_blocking(
    ...
    async def list_sessions(self, *, limit: int = 20, offset: int = 0) -> list[dict[str, Any]]:
        return self._list_sessions_blocking(limit=limit, offset=offset, user=None)

    async def get_session_detail(self, session_id: str) -> dict[str, Any] | None:
        return self._get_session_detail_blocking(session_id, user=None)
...
def list_sessions_sync(
    ...
    *,
    ...
    if not st.available:
        return []
    return st._list_sessions_blocking(limit=limit, offset=offset, user=user)
...
def get_session_detail_sync(
    ...
    *,
    ...
    if not st.available:
        return None
    return st._get_session_detail_blocking(session_id, user=user)
