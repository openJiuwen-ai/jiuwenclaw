...
def apply_openjiuwen_safe_log_handler_patch() -> None:
    ...
    def _patched_init(
        ...
        **kwargs,
        ...
        if log_file_pattern:
            # 访问受保护成员是monkey patching的必要操作
            filename = self._format_filename(filename, log_file_pattern)  # pylint: disable=protected-access
    ...
    # 设置标记以标识已应用patch（monkey patching 必要操作）
    handler_cls._jiuwenclaw_chmod_tolerant = True  # pylint: disable=protected-access
    _OPENJIUWEN_LOG_HANDLER_PATCHED = True
