...
class SkillManager:
    ...
    def _remove_skill_directory(self, skill_name: str) -> None:
        name = str(skill_name or "").strip()
        if not name:
            return
        try:
    ...
    async def handle_skills_web_install(self, params: dict) -> dict:
        ...
        from jiuwenclaw.agentserver.installed_skill_ops import (
            commit_install,
            precheck_install,
        )
        from jiuwenclaw.agentserver.installed_skill import verify_skill_download_hmac
        ...
        with tempfile.TemporaryDirectory(prefix="jiuwenclaw_enterprise_install_") as tmpdir:
            ...
            commit = await commit_install(
                ...
                bot_id=bot_id,
                user_id=user_id,
                remove_skill_dir=self._remove_skill_directory,
            )
