...
set 成功后异步触发生效 (``_apply_sandbox_change``):
  - enabled: 热重载 agent, 不动沙箱/box-server (只影响新会话选 LOCAL/SANDBOX).
...
async def _apply_sandbox_change(kind: str) -> None:
    ...
    try:
        if kind == "enabled":
            ...
            logger.info("[sandbox] enabled 变更, 下轮 _create_sys_operation 读新值生效")
            return
        if kind == "startup_mode":
            ...
            if mode == "external" and runner._owns_process:
                logger.info(
        ...
        if kind in ("files", "network"):
            ...
            runner = JiuwenBoxRunner.instance()
            if not runner._owns_process or runner._process is None:
                logger.info(
            ...
            await runner.ensure_running(
                ...
                policy_path=runner._spawned_policy_path,
                timeout=120.0,
            )
            return
