...
if TYPE_CHECKING:
    from .store import EffectivePolicyStore
...
async def _lookup_mapping_by_part(
    ...
    *,
    ...
    if not key:
        return None
    if dim == "user":
        return await store.lookup_template_mapping_ref(
            ctx.jiuwenclaw_id, user_id=key
        )
    return await store.lookup_template_mapping_ref(
        ctx.jiuwenclaw_id, group_id=key
    )
...
async def resolve_model_slot_ref(
    ...
    *,
    ...
    for part in _OR_SPLIT_PATTERN.split(text):
        ...
        mapping_ref = await _lookup_mapping_by_part(part, ctx, store=store)
        if mapping_ref:
            return mapping_ref
...
def service_rule_matches(rule: dict[str, Any], ctx: RoutingContext) -> bool:
    """服务策略是否命中：仅 ``match_expr``；``service_id`` 不参与匹配。"""
