...
"""管理本地 jiuwenbox uvicorn 子进程 — 由 AgentServer boot / ensure_running 触发.

设计要点:
- 默认地址 ``http://127.0.0.1:8321``; 端口是否空闲由调用方在
...
- ``startup_mode='external'`` 时: 仅做健康检查; 不可达则直接失败, 提示用户
  自行启动 jiuwenbox-server。
- AgentServer 进程退出时调用 ``stop()`` 终止子进程, 避免悬挂.
"""
...
def _resolve_jiuwenbox_src_dir() -> Optional[Path]:
    """探测仓库内 ``jiuwenbox/src``; 若存在则供 PYTHONPATH 注入用.

    从 ``jiuwenclaw/agentserver/sandbox/`` 向上查找同级 ``jiuwenbox/src``。
...
class JiuwenBoxRunner:
    ...
    def is_owned_listener(self, host: str, port: int) -> bool:
        ...
        调用方在分配端口前用它判断"8321 是不是我自己刚才拉起的",
