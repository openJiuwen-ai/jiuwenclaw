...
def _strip_one_pass(text: str) -> str:
    ...
    while pos < n:
        ...
        if tag_pos != -1 and (func_pos == -1 or tag_pos <= func_pos):
            ...
            if inner_func == -1:
                # 有外层标签但无内层；尝试找闭合标签
                close_end = _find_close_tags(text, anchor)
                if close_end != -1:
                    # 整段删掉（开标签到闭标签结束）
...
def _find_close_tags(text: str, from_pos: int) -> int:
    """
    从 from_pos 起贪婪消费所有连续的闭合标签（标签间允许有空白），
    返回最后一个闭合标签结束后的位置；未找到任何闭合标签则返回 -1。
    """
    pos = from_pos
    found_any = False
    while True:
...
def _find_complete_protocol_end(text: str, anchor: int) -> int:
    ...
    如果协议不完整（截断/不匹配），返回 -1。
    如果锚点只是普通文本中恰好出现的协议样式前缀，则返回 -2。
    """
    n = len(text)
    incomplete = -1
    not_protocol = -2

    if any(text.startswith(tag, anchor) for tag in _OPEN_TAGS):
        # 外层标签路径
        inner_func = _find_func_sep(text, anchor)
        if inner_func == -1:
            close_end = _find_close_tags(text, anchor)
            return close_end if close_end != -1 else incomplete
        name_start = inner_func + len(_FUNC_SEP)
        m = _TOOL_NAME_RE.match(text, name_start)
        if not m:
            return incomplete if name_start >= n else not_protocol
        tool_name = m.group(0)
        if not _tool_name_allowed(tool_name):
            return incomplete if _tool_name_may_be_incomplete_prefix(tool_name) else not_protocol
        json_start = m.end()
        if json_start >= n:
            return incomplete
        if text[json_start] != "{":
    ...
    if text.startswith(_FUNC_SEP, anchor):
        ...
        if text[json_start] != "{":
            return not_protocol
        json_end = _find_balanced_json_end(text, json_start)
        return json_end if json_end != -1 else incomplete
