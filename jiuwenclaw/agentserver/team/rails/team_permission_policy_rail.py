...
class TeamPermissionPolicyRail(DeepAgentRail):
    ...
    async def before_model_call(self, ctx: AgentCallbackContext) -> None:
        ...
        cfg = get_config()
        perm_cfg = cfg.get("permissions") if isinstance(cfg, dict) else None
        effective_cfg = (
            perm_cfg if isinstance(perm_cfg, dict) and perm_cfg else self._permissions_config
        )

        permissions_text = format_base_permissions_for_desc(
            effective_cfg, lang=self._language,
        )
