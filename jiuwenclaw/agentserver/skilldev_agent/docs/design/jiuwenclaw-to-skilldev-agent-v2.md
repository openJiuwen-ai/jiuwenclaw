...
| `agentserver/interface.py` | `process_message_stream` 新增 `skilldev.chat` 路由 + `_get_skilldev_chat_handler` 懒初始化 | +20 行 | **否** |
...
**详细步骤：**
1. 实现 `adapter.py`：
   - 构造函数（workspace_dir, agent_id, service_id）
   - `_build_model(config)` → 从 `get_config()` 读取模型配置（参见 6.12 补充 3）
   - `create_instance()` → 构建 DeepAgent（Prompt + 工具 + Rails）
   - `process_message_stream_impl()` → **必须使用 `ask_user_question_request_scope` 包裹 Agent 执行**（参见 6.6）
   - `process_message_impl()` → 空实现（实际不使用）
   - `process_interrupt()` → cancel 支持
   - `handle_user_answer()` → **必须路由 `source=="ask_tool"` 到 `AskUserQuestionRegistry.resolve()`**（参见 6.7）
   - `handle_heartbeat()` → 返回 None
   - `is_working()` → 状态检查
   - `get_instance()` → 返回 DeepAgent（兼容 interface.py MCP/extension 注册）
   - `set_skill_manager()` → 接收 SkillManager
   - `_init_subagent_context()` → 初始化 ForkAgentExecutor + 上下文变量
   - TodoToolkit session_id 每次请求动态更新（参见 6.9）
...
2. 实现 `chat_handler.py`：
   ...
   - **per-task 工作区隔离**：每个 task 创建独立目录 `skilldev/{task_id}/`（参见 6.10）
   - 工作区初始化（创建子目录、写入上传文件、解压 skill 包）
   - **请求串行化锁**：防止同 task 并行执行（参见 6.8）
   - 构建 Agent inputs
   - 委托 adapter 执行
3. 修改 `interface.py`：
...
### 6.6 ask_user_question 上下文变量未设置

**风险（严重）：** `ask_user_question` 工具内部调用 `get_ask_request_context()` 获取 4 个 `ContextVar`：`interactive_ask`、`session_id`、`stream_request_id`、`channel_id`。这些变量由 `ask_user_question_request_scope()` 上下文管理器设置。

在现有 `JiuWenClawDeepAdapter.process_message_stream_impl()` 中（第 3678 行），Agent 执行被包裹在此上下文管理器内：
...
**如果新适配层不设置这些上下文变量**，`ask_user_question` 工具将无法获取 `stream_request_id`，导致无法向前端推送问题，直接返回错误 `"内部错误：缺少 stream_request_id"`。

**缓解（必须实现）：** `SkillDevDeepAdapter.process_message_stream_impl()` 中必须使用相同的上下文管理器：
...
async def process_message_stream_impl(self, request, inputs):
    ...
    interactive_ask = True  # SkillDev Agent 模式默认开启交互
...
### 6.7 handle_user_answer 必须路由 ask_tool 回答

**风险（严重）：** 用户回答 `ask_user_question` 的问题时，前端通过 `chat.answer`（`ReqMethod.CHAT_ANSWER`）发送回答。`interface.py` 将其路由到 `adapter.handle_user_answer(request)`。

现有 `JiuWenClawDeepAdapter.handle_user_answer()` 中关键逻辑：
```python
elif source == "ask_tool":
    resolved = AskUserQuestionRegistry.get_instance().resolve(request_id, answers)
```

**如果新适配层的 `handle_user_answer()` 没有此路由**，用户的回答将无法到达 `AskUserQuestionRegistry`，`ask_user_question` 工具的 `wait_for_answer()` 将一直阻塞直到超时。

**缓解（必须实现）：** `SkillDevDeepAdapter.handle_user_answer()` 必须包含完整的回答路由：
...
### 6.8 SessionManager 任务队列绕过

**风险（中）：** 现有流式路径通过 `self._session_manager.submit_task(session_id, run_stream_task)` 实现同 session 请求串行化。新的 `skilldev.chat` 路由在 `process_message_stream` 中直接 yield，**完全绕过了 SessionManager**。

这意味着如果用户快速连续发送两个 `skilldev.chat` 请求，可能导致两个 Agent 执行并行运行，产生文件写入冲突。

**缓解方案（推荐在 chat_handler 中实现）：**

方案 A：在 `chat_handler.py` 中引入简单的任务锁

```python
class SkillDevChatHandler:
    def __init__(self, ...):
        self._task_locks: dict[str, asyncio.Lock] = {}  # task_id -> lock
    
    async def handle_stream(self, request):
        task_id = params.get("task_id", "default")
        lock = self._task_locks.setdefault(task_id, asyncio.Lock())
        async with lock:
            async for chunk in self._do_handle(request):
                yield chunk
```

方案 B：在 `interface.py` 中复用 SessionManager（改动略大但更一致）

**推荐方案 A**，简单有效。

### 6.9 plan_todo_context session_id 设置

**风险（中）：** `TodoToolkit` 在未传入 `session_id` 参数时，通过 `_resolve_runtime_session_id()` → `plan_todo_context.PLAN_TODO_SESSION_ID` 获取。此上下文变量由 `JiuWenClawDeepAdapter` 在每次请求时设置。

**缓解：** 在 `build_skilldev_tools()` 中**显式传入 `session_id`**（已在 tools.py 骨架中预留了此参数）。在 `chat_handler.py` 或 `adapter.py` 中调用时传入：

```python
tools = build_skilldev_tools(session_id=session_id, todo_dir=workspace_dir)
```

由于 `TodoToolkit` 的构造函数支持显式 `session_id`，此方案不依赖上下文变量。但注意：**工具列表在 `create_instance` 时构建，而 `session_id` 在每次请求时不同**。解决方案：

- 方案 A：每次请求重建 TODO 工具实例（开销较小）
- 方案 B：利用 `TodoToolkit` 的 `set_session_id()` 方法在每次请求前动态更新
- 方案 C：不传 `session_id`，改为在 adapter 中正确设置 `plan_todo_context.PLAN_TODO_SESSION_ID` 上下文变量

**推荐方案 B**，最简单：

```python
async def process_message_stream_impl(self, request, inputs):
    session_id = inputs.get("conversation_id", "default")
    self._todo_toolkit.set_session_id(session_id)
    # ... 执行 Agent ...
```

### 6.10 工作区 per-task 隔离

**风险（中）：** 旧 Pipeline 为每个 task 创建独立的工作区目录（`skilldev/{task_id}/`），通过 `WorkspaceProvider` 管理。新 Agent 路径中，adapter 的 `workspace_dir` 在构造时固定。如果多个 skill 创建任务共用同一 workspace，文件会冲突。

**缓解：** `chat_handler.py` 在首次请求时创建 per-task 工作区，并在调用 adapter 前**动态设置 workspace**：

```python
async def handle_stream(self, request):
    task_id = self._get_or_create_task_id(params)
    task_workspace = self._base_workspace / "skilldev" / task_id
    task_workspace.mkdir(parents=True, exist_ok=True)
    
    # 动态更新 Agent 的 workspace
    self._adapter.update_workspace(task_workspace)
    # ... 继续处理 ...
```

或者更安全的做法：**每个 task 创建一个独立的 DeepAgent 实例**（类似 Pipeline 中 `create_stage_agent` 的模式）。这需要 adapter 支持 per-task 实例化。

### 6.11 前端事件兼容性

**风险（低-中）：** 旧 Pipeline 向前端发送特定事件类型（`skilldev.stage_start`、`skilldev.agent_thinking`、`skilldev.artifact_ready` 等），前端根据这些事件渲染 UI（如进度条、文件列表、评测面板）。

新 Agent 路径发送的是标准 chat 事件（`chat.delta`、`chat.tool_call` 等），前端可能无法正确渲染 Skill 开发特有的 UI 元素。

**缓解：**

- **短期**：新 Agent 先使用标准 chat 事件，前端以普通对话方式展示。功能可用但 UI 不如 Pipeline 精致。
- **中期**：在 `JiuClawStreamEventRail` 中增加逻辑，根据工具调用类型（如 `todo_create`、`ask_user_question`）自动映射为 SkillDev 专有事件。
- **长期**：前端适配新的事件格式，或 adapter 层增加事件转换逻辑。

### 6.12 Prompt 中遗漏的实现细节

**补充 1 - `todo_remove` 工具：** `TodoToolkit.get_tools()` 返回的工具列表包含 `todo_remove`，但 Prompt 和工具表中均未提及。应补充到 Prompt 第 3.2 节和工具表 3.3.1 中。

**补充 2 - 描述优化阶段（desc_optimize）：** 原 Pipeline 中有独立的 `desc_optimize_stage.py`，优化 SKILL.md 的 `description` 字段使 skill 更容易被触发。当前 Prompt 在 2.8（打包）之前缺少此步骤。应在 Prompt 中增加"描述优化"指引：

> 在打包前，审视 `description` 字段：
> - 是否清晰描述了 skill 的触发场景？
> - 是否包含了典型用户会使用的关键词？
> - 是否过于宽泛或过于狭窄？
> 如有必要，使用 `ask_user_question` 询问用户是否需要优化描述。

**补充 3 - `_build_model` 方法：** 适配层骨架中引用了 `self._build_model(config)` 但未定义实现。需参考 `interface_deep.py` 中的模型配置读取逻辑：
...
def _build_model(self, config=None):
    ...
    return Model(
        request_config=ModelRequestConfig(**default.get("model_config_obj", {})),
        client_config=ModelClientConfig(**default.get("model_client_config", {})),
    )
```
