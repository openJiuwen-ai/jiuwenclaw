...
设计 §8.5 代码加载：
``<skill_root>/turbo/`` 加入 ``sys.path`` → ``importlib`` 加载包
``turbo_codes_<scenario>.<module>``（包内相对 import 在包内解析）→
...
from jiuwenclaw.agentserver.skill_turbo.plan_node import PlanNode, AbortError
...
class SkillCodeExecutor:
    ...
    def _validate_source(self, source_path: str) -> None:
        ...
        if not path.is_file():
            logger.debug(
                "[SkillCodeExecutor] skip validate, source missing: %s", source_path,
            )
            return
        try:
    ...
    def _ensure_turbo_dir_on_path(self, turbo_dir: str) -> None:
        """把 turbo_dir 加入 sys.path（幂等去重），供 importlib 加载 turbo_codes 包。

        turbo_codes_<scenario> 是 turbo_dir 下的顶层包，需 turbo_dir 在 sys.path 中。
        """
        if not turbo_dir:
            return
    ...
    def _load_node_instance(
        ...
        self._ensure_turbo_dir_on_path(turbo_dir)
        try:
        ...
        *,
        ...
        if executor is not None:
            ...
            if callable(bind):
                ...
                logger.warning(
                    "[SkillCodeExecutor] executor 无 _bind_node_callbacks，节点回调未注入"
                )
        else:
            logger.warning(
