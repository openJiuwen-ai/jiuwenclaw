...
def _load_node_display_names(turbo_dir: str | None) -> dict[str, str]:
    ...
    try:
        ...
        if not result:
            ...
            if sibling.is_file():
                result.update(_extract_node_display_names(sibling, turbo_root=turbo_path))
    except Exception as exc:
        logger.debug("[OnlineTaskProgress] load NODE_DISPLAY_NAMES failed: %s", exc)

    # 简易上限：过多时淘汰最旧项（插入序）
    if len(_NODE_DISPLAY_CACHE) >= 64 and key not in _NODE_DISPLAY_CACHE:
        oldest = next(iter(_NODE_DISPLAY_CACHE))
        _NODE_DISPLAY_CACHE.pop(oldest, None)
    _NODE_DISPLAY_CACHE[key] = result
    return result


def _extract_node_display_names(
    path: Path, *, turbo_root: Path | None = None,
) -> dict[str, str]:
    """从 ppt_common.py 只读提取 NODE_DISPLAY_NAMES 常量（不做 exec_module）。"""
    import ast

    try:
        resolved = path.resolve()
        if turbo_root is not None:
            resolved.relative_to(Path(turbo_root).resolve())
    except ValueError:
        logger.warning(
            "[OnlineTaskProgress] display names path escapes turbo_dir: %s", path,
        )
        return {}
    try:
        source = path.read_text(encoding="utf-8")
    except OSError as exc:
        logger.debug("[OnlineTaskProgress] read display names failed %s: %s", path, exc)
        return {}
    try:
    ...
    for node in tree.body:
        ...
        for target in node.targets:
            if isinstance(target, ast.Name) and target.id == "NODE_DISPLAY_NAMES":
                try:
...
def prepare_resume_progress(ctx: TurboContext) -> None:
...
def _public_tasks(ctx: TurboContext) -> list[dict[str, Any]]:
    progress = getattr(ctx, "task_progress", None) or {}
    tasks = []
...
def _probe_emitter_closed(session: Any) -> bool:
    ...
    try:
        ...
        # 探测失败时保守视为已关闭，避免误计 emit_ok 抑制补偿
        return True
