...
def _release_turbo_active_body(parent_session: Any, skill_name: str) -> None:
...
def _resolve_skill_root_for_turbo(skill_name: str, env: Any) -> str:
    ...
    if env_skill_root:
        candidate = Path(env_skill_root) / skill_name
        if candidate.is_dir():
            return str(candidate.resolve())
        # 也许 env.skill_root 本身就是 skill 根
        if Path(env_skill_root).name == skill_name and Path(env_skill_root).is_dir():
            return str(Path(env_skill_root).resolve())
...
async def skill_turbo_tool(
    ...
    env = skill_turbo_inst._env
    executor = skill_turbo_inst._executor
...
async def _execute(
    ...
    # 候选集校验（next_candidates 可能 when-skip 并写 skip_defaults；早退前须落盘 — F11）
    candidates = flow_scheduler.next_candidates(schema, ctx)
    await task_progress.sync_from_completed(
        ctx, schema, parent_session, request_id=req_id,
    )
    ...
    # 逐个执行（并行批用 asyncio.gather，当前 create_ppt 全串行）
    single = executor_single.SkillCodeExecutor(
        env, request_id="", channel_id="",
    )
    ...
    for i, result in enumerate(results):
        ...
        if isinstance(result, dict) and result.get("success") is False:
            ...
            if fallback_policy.should_task_fallback(
                ...
                await context_store.save_online_context(parent_session, ctx)
                context_store.mark_pending_clear_online_context()
                _release_turbo_active_body(parent_session, skill_name)
...
async def _param_validation_fallback(
    ...
    *,
    ...
    try:
        node = single._load_node_instance(
            ctx.turbo_dir, ctx.scenario, plan_name, schema,
        )
        bind = getattr(executor, "_bind_node_callbacks", None)
        if callable(bind):
            bind(node)
