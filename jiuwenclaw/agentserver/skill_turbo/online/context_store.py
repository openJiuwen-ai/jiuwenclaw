...
_pending_clear_online_ctx: ContextVar[bool] = ContextVar(
    "skill_turbo_pending_clear_online_ctx", default=False,
)


def mark_pending_clear_online_context() -> None:
    """标记：StreamEventRail.after_tool_call flush 后应 clear ctx。"""
    _pending_clear_online_ctx.set(True)


def consume_pending_clear_online_context() -> bool:
    """读取并清除 pending clear 标记。"""
    if not _pending_clear_online_ctx.get():
        return False
    _pending_clear_online_ctx.set(False)
    return True
    ...
    try:
        if persist:
            try:
                ...
                logger.warning(
                    "[OnlineCtx] clear_online_context pre_run failed: sid=%s err=%s",
                    sid, exc,
                )
                return
