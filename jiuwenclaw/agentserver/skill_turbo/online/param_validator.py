...
职责（设计 §5.5 + 优化修复 F1）：
1. 执行入参：始终传 accumulator **深拷贝全量副本**，避免节点变异嵌套结构污染 ctx
...
def assemble_node_inputs(
    ...
    统一策略（优化修复 F1）：
    - ``node_inputs = copy.deepcopy(accumulator)`` 深拷贝全量副本（隔离嵌套可变对象）
    ...
    if task is None:
        ...
        if not isinstance(required, list):
            # fail-closed：畸形 schema 不静默跳过必填校验
            return (copy.deepcopy(accumulator) if accumulator else {}), ["__schema_inputs_invalid__"]
        if not isinstance(optional, list):
            optional = []

    node_inputs: dict[str, Any] = copy.deepcopy(accumulator) if accumulator else {}
...
def validate_node_inputs(
    ...
    if not isinstance(required, list):
        return ValidationResult(
            ok=False,
            missing_keys=["__schema_inputs_invalid__"],
            hint=f"节点 {plan_name} schema inputs 类型异常（期望 list）",
        )
