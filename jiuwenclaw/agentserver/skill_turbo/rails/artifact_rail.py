...
class SkillTurboArtifactRail:
    """复用 DeepAgent artifact_emitter，在 SkillTurbo 工具调用后检测产物。"""
    ...
    async def after_tool_call(self, ctx: AgentCallbackContext) -> None:
        ...
        logger.info(
            "[SkillTurboArtifact] Detect done: tool=%s session_id=%s emitted=%s elapsed_ms=%d",
            ctx.inputs.tool_name,
            ctx.session.get_session_id(),
            emitted,
