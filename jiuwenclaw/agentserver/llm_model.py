...
from jiuwenclaw.extensions.sdk.llm_base_model_client import LlmBaseModelClient, LlmModelClientDelegate
...
class LlmModel:
    ...
    def _create_model_client(self, client_config: ModelClientConfig) -> BaseModelClient:
        ...
        if issubclass(client_class, LlmBaseModelClient):
            reg_mod = sys.modules.get("jiuwenclaw.extensions.registry")
            if reg_mod is not None and hasattr(reg_mod, "ExtensionRegistry"):
                try:
                    full_config = reg_mod.ExtensionRegistry.get_instance().get_config()
                    client = client_class(self.model_config, client_config)
                    if hasattr(client, "set_full_config"):
                        getattr(client, "set_full_config")(full_config)
                    return LlmModelClientDelegate(client)
                except Exception as e:
                    logger.error(f"get registry had Exception = {e}")
