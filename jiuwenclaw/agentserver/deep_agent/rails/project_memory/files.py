...
def discover_and_load_memory_files(
    *,
    ...
    trusted_roots = _build_trusted_include_roots(
        workspace=Path(workspace_key),
        additional_directories=normalized_additional_dirs,
    )
...
def _build_trusted_include_roots(
    *,
    workspace: Path,
...
def _load_path(
    ...
    *,
    ...
    if not any(
        _is_relative_to(resolved_path, Path(root))
        for root in trusted_roots
    ):
        logger.warning(
            "[project_memory] skipped memory file outside trusted roots: %s",
            resolved,
        )
        return
    ...
    *,
    ...
    for line in body.splitlines():
        ...
        if include_path is None:
            continue
        resolved_include = Path(_safe_resolve(include_path))
        if not any(
            _is_relative_to(resolved_include, Path(root))
            for root in trusted_roots
        ):
            logger.warning(
                "[project_memory] skipped @include outside trusted roots: %s",
                include_spec,
            )
            continue
