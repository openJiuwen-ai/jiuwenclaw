...
class PlanApprovalInterruptRail(ConfirmInterruptRail):
    ...
    async def resolve_interrupt(
        ...
        if tool_call is not None and not self._is_plan_mode(ctx):
            ...
            message = (
                ...
                if language == "en"
                else "exit_plan_mode 只能在 plan 模式下使用。"
            )
            ctx.extra["_plan_rejected"] = True
            return self.reject(tool_result={"error": message})
        ...
        if user_input is None and tool_call is not None:
            ...
            self._mark_approval_state(ctx, decision)
            return decision
        ...
        try:
            ...
            self._mark_approval_state(ctx, None)
            raise
    ...
    @staticmethod
    def _mark_approval_state(
        ctx: AgentCallbackContext,
        decision: Any,
    ) -> None:
        """Persist approval state; anything except explicit approval fails closed."""
        if isinstance(decision, ApproveResult):
            ctx.extra["_plan_approved"] = True
            ctx.extra.pop("_plan_rejected", None)
            ctx.extra.pop("_plan_pending", None)
            return
        ctx.extra["_plan_approved"] = False
        ctx.extra["_plan_rejected"] = True
        ctx.extra["_plan_pending"] = isinstance(decision, InterruptResult)
    ...
    def _is_plan_mode(self, ctx: AgentCallbackContext) -> bool:
        ...
        try:
            ...
            return getattr(getattr(state, "plan_mode", None), "mode", None) == "plan"
        except Exception as exc:  # noqa: BLE001 - approval must fail closed
            logger.warning(
                "[PlanApprovalInterruptRail] Failed to validate plan mode: %s",
                exc,
            )
            return False
    ...
    def _read_plan_content(self, ctx: AgentCallbackContext) -> str:
        ...
        try:
            with plan_path.open("r", encoding="utf-8") as plan_file:
                return plan_file.read(_MAX_PLAN_PREVIEW_CHARS + 1).strip()
