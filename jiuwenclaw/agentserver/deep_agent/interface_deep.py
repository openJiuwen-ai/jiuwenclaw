    ...
    def _init_skill_turbo_tool(self) -> None:
        ...
        try:
            ...
            if isinstance(skill_turbo_config, dict):
                ...
                if execution_mode not in ("online", "batch"):
                    logger.warning(
                        "[JiuWenClawDeepAdapter] invalid skill_turbo_execution_mode=%r, "
                        "fallback to online",
                        execution_mode,
                    )
                    execution_mode = "online"
    ...
    async def _try_skill_turbo_online_resume(
        ...
        if interrupt_state is None:
            ...
            try:
                ...
                logger.warning(
                    "[JiuWenClawDeepAdapter] online resume probe post_run failed: %s",
                    exc,
                )
            return None
    ...
    def _make_skill_turbo_online_resume_stream(
        ...
        async def _resume_impl() -> AsyncIterator[AgentResponseChunk]:
            ...
            try:
                ...
                # 4. 加载 schema + 单节点重放（输入走 assemble 深拷贝，勿直传 accumulator）
                schema = schema_loader.load_schema(ctx.turbo_dir, ctx.scenario)
                from jiuwenclaw.agentserver.skill_turbo.online import param_validator

                node_inputs, missing = param_validator.assemble_node_inputs(
                    plan_name, schema, ctx.accumulator,
                )
                if missing:
                    logger.warning(
                        "[JiuWenClawDeepAdapter] online resume missing inputs "
                        "plan=%s missing=%s",
                        plan_name, missing,
                    )
                ...
                if complete:
                    ...
                    # 仍在可写会话窗口内清理，再统一 post_run 落盘（避免 post_run 后再 clear）
                    await clear_online_context(session, persist=True, skip_post_run=True)
                else:
                ...
                try:
                    ...
                    logger.warning(
                        "[JiuWenClawDeepAdapter] online resume post_run failed: %s",
                        exc,
                    )

                # 6. 构造结果摘要（candidates 已在落盘前算好）
                ...
                if complete:
                    from jiuwenclaw.agentserver.skill_turbo.online.skill_turbo_tool import (
                        release_turbo_active_body,
                    )
                    release_turbo_active_body(session, ctx.skill_name)
                    yield AgentResponseChunk(
                        request_id=rid,
                        channel_id=cid,
                        payload={
