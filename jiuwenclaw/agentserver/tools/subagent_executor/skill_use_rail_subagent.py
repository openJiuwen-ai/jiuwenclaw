...
_SUBAGENT_SKILL_PROMPT_CN = (
    ...
    "- 给定路径 `.../<Y>/<X>/SKILL.md`，**`<skill_name>` 必须取 `<Y>/<X>`**"
    "（含父路径 `<Y>` 与末段 `<X>` 的命名空间形式），**禁止**只用裸名 `<X>`。\n"
    "- 即便 `<Y>` 本身也注册为同名 skill，**本次任务的指令集仍是 `<X>` 里的 SKILL.md**，"
    "用 `<Y>/<X>` 是为区分不同父 skill 下同名的 `<X>`，`<Y>` 只是 umbrella 不含本次具体规则。\n"
...
_SUBAGENT_SKILL_PROMPT_EN = (
    ...
    "- Given a path `.../<Y>/<X>/SKILL.md`, **`<skill_name>` must be `<Y>/<X>`** "
    "(namespaced form with parent `<Y>` and leaf `<X>`), **not** the bare name `<X>` alone.\n"
    "- Even if `<Y>` itself is also registered as a same-named skill, "
    "**the instructions for the current task are still in `<X>`'s SKILL.md**, "
    "use `<Y>/<X>` to disambiguate same-named `<X>` under different parent skills, "
    "`<Y>` is just an umbrella and does not contain the specific rules for this task.\n"
