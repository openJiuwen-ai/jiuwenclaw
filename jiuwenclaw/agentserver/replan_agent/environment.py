...
"""RePlanEnvironment —— 灵魂、工具、模型、技能配置环境。"""
...
from jiuwenclaw.agentserver.replan_agent.validator import PlanCodeValidator

if TYPE_CHECKING:
    from jiuwenclaw.agentserver.replan_agent.tools_loader import ToolLoaderContext  # noqa: F401
...
_DEFAULT_SKILL_CODE_IMPORT_PACKAGE = (
    "jiuwenclaw.agentserver.replan_agent.skill_codes"
)
...
def _verify_skill_checksum(pptx_root: str, expected_checksum: str) -> bool:
    ...
    if not expected_checksum:
        logger.info("[RePlanEnvironment] skill_checksum 为空，跳过 SHA256 校验")
        return True

    actual = _compute_dir_checksum(pptx_root)
    if actual == expected_checksum:
        logger.info("[RePlanEnvironment] skill_checksum 校验通过: %s", actual)
        return True

    logger.warning(
        "[RePlanEnvironment] skill_checksum 校验失败！期望=%s 实际=%s pptx_root=%s",
...
class RePlanEnvironment:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        # 类型为 RePlanFallbackHandler | None，延迟 import 避免循环依赖。
        ...
        # 允许安全标准库和 replan_agent 内部模块 import，禁止 os/subprocess 等危险模块，
    ...
    def _resolve_skill_root(self) -> str:
        ...
        2. DeepAgent 的 skills 目录 — 通过 get_agent_registered_skill_dirs() 获取
           （RePlanAgent 由 interface_deep.py 调用，此时 DeepAgent 已初始化，
        ...
        if self._skills_dir:
            ...
            if skills_path.is_dir():
                logger.info(
                    "[RePlanEnvironment] skill_root from skills_dir: %s",
                    self._skills_dir,
                )
        ...
        try:
            ...
            if skill_dirs:
                ...
                logger.info(
                    "[RePlanEnvironment] skill_root from DeepAgent skills_dir: %s",
                    first_dir,
                )
                return first_dir
        except Exception as exc:
            logger.debug(
                "[RePlanEnvironment] get_agent_registered_skill_dirs unavailable: %s",
                exc,
            )

        logger.info("[RePlanEnvironment] skill_root not resolved")
        return ""
    ...
    def _resolve_sys_operation(self) -> Any:
        ...
        Runner.resource_mgr 中，RePlanAgent 可直接获取，无需 interface_deep.py
        显式传入。
        ...
        try:
            ...
            if sys_ops is not None:
                ...
                if isinstance(sys_ops, list):
                    if sys_ops:
                        logger.info(
                            "[RePlanEnvironment] sys_operation from Runner.resource_mgr: %s",
                            sys_ops[0],
                        )
                        return sys_ops[0]
                else:
            ...
            logger.debug(
                "[RePlanEnvironment] sys_operation from Runner.resource_mgr unavailable: %s",
                exc,
            )

        logger.info("[RePlanEnvironment] sys_operation not resolved")
        return None
    ...
    def get_tool_function(
        ...
        logger.warning(
            "[RePlanEnvironment] tool=%s has no invoke/run callable", tool_name
        )
        return None
    ...
    def register_skill(self, skill: Skill) -> None:
        ...
        if not skill or not skill.name:
            logger.warning("[RePlanEnvironment] register_skill ignored: invalid skill")
            return
        if skill.name in self._skills:
            logger.info(
                "[RePlanEnvironment] register_skill overwrite skill=%s", skill.name
            )
    ...
    def reload(self) -> None:
        ...
        logger.info("[RePlanEnvironment] reload skills")
        self._skills.clear()
    ...
    def register_tool(self, tool_card: Any) -> None:
        ...
        if tool_card is None:
            logger.warning("[RePlanEnvironment] register_tool ignored: None card")
            return
        name = getattr(tool_card, "name", None)
        if not name:
            logger.warning(
                "[RePlanEnvironment] register_tool ignored: card without name"
            )
            return
        if name in self._tools:
            logger.info("[RePlanEnvironment] register_tool overwrite tool=%s", name)
    ...
    async def register_tools(self, context: "ToolLoaderContext | None" = None) -> None:
        ...
        """
        from jiuwenclaw.agentserver.replan_agent.tools_loader import (
        ...
        if not self._tools_loaded:
            ...
            logger.info(
                "[RePlanEnvironment] register_tools static done loaded=%d tool_names=%s",
                len(tools),
    ...
    def build_tool_loader_context(
        ...
        *,
        ...
        from jiuwenclaw.agentserver.replan_agent.tools_loader import ToolLoaderContext

        return ToolLoaderContext(
            agent_id=str(self._config.get("agent_id") or "replan"),
    ...
    def _refresh_send_file_tools(
        ...
        for name in list(self._tools.keys()):
            ...
            if tool_id:
                try:
                    ...
                    logger.warning(
                        "[RePlanEnvironment] remove send_file tool failed id=%s: %s",
                        tool_id,
                        exc,
                    )
            removed.append(name)
        ...
        for tool in tools:
            ...
            if tool_id and Runner.resource_mgr.get_tool(tool_id) is not None:
                try:
                    ...
                    logger.debug(
                        "[RePlanEnvironment] remove stale send_file tool ignored id=%s: %s",
                        tool_id,
                        exc,
                    )
            try:
                ...
                logger.warning(
                    "[RePlanEnvironment] register send_file tool failed: %s",
                    exc,
                    exc_info=True,
                )
                continue
        ...
        if tools or removed:
            logger.info(
                "[RePlanEnvironment] send_file refresh removed=%s registered=%d",
                removed,
                len(tools),
            )
    ...
    def _scan_skills_dir(self) -> None:
        ...
        if not base.is_dir():
            logger.warning(
                "[RePlanEnvironment] _scan_skills_dir config path not found: %s",
            ...
            if not default_path.is_dir():
                logger.info(
                    "[RePlanEnvironment] _scan_skills_dir skipped: both config and default paths not found"
                )
                return
            base = default_path
            logger.info(
                "[RePlanEnvironment] _scan_skills_dir fallback to default path: %s",
                base,
            )
        ...
        if cached is not None and cached[0] == current_mtime and current_mtime > 0.0:
            ...
            logger.debug(
                "[RePlanEnvironment] _scan_skills_dir cache hit: dir=%s skills=%d",
                cache_key,
                len(cached[1]),
            )
            return
        ...
        for skill_dir in base.iterdir():
            ...
            try:
                ...
                if root_file is None:
                    logger.debug(
                        "[RePlanEnvironment] _scan_skills_dir skip skill=%s: no root file found",
                        skill_name,
                    )
                    continue
                ...
                logger.info(
                    "[RePlanEnvironment] _scan_skills_dir registered skill=%s root_file=%s",
                    skill_name,
                    root_file.name,
                )
            except Exception as e:
                logger.warning(
                    "[RePlanEnvironment] _scan_skills_dir failed to register skill=%s: %s",
                    skill_name,
                    e,
                )
    ...
    def _compute_skill_codes_mtime(base: Path) -> float:
        ...
        try:
            ...
            logger.debug(
                "[RePlanEnvironment] _compute_skill_codes_mtime failed dir=%s error=%s",
                base,
                e,
            )
            return 0.0
    ...
    def _validate_skill_code_dir(self, skill_name: str, skill_dir: Path) -> bool:
        ...
        if not skill_dir.is_dir():
            logger.warning(
                "[RePlanEnvironment] skill_code validation failed: skill=%s dir not found: %s",
                skill_name,
                skill_dir,
            )
            return False
        ...
        for code_file in sorted(skill_dir.rglob("*.py")):
            ...
            try:
                ...
                logger.warning(
                    "[RePlanEnvironment] skill_code read failed: skill=%s file=%s error=%s",
                    skill_name,
                    code_file,
                    e,
                )
            ...
            if errors:
                logger.warning(
                    "[RePlanEnvironment] skill_code validation failed: skill=%s file=%s errors=%s",
                    skill_name,
                    code_file,
                    errors,
                )
        ...
        if not success:
            ...
            logger.warning(
                "[RePlanEnvironment] skip skill registration due to validation errors: %s",
                skill_name,
            )
