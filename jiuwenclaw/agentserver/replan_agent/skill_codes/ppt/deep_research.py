...
class SearchFilterNode(PlanNode):
        ...
        for page in pages:
            ...
            if coverage == "covered":
                ...
                for q in research_queries:
                    queries.append({"page": page["page_number"], "query": q})
                for d in page.get("data_needs", []):
                    queries.append({"page": page["page_number"], "query": d})
    ...
    async def _score_and_filter_sources(
        ...
        prompt = (
            ...
            '以 JSON 对象输出，key 为来源完整URL，value 为评分等级（A+/A/A-/B+/B/C）。\n'
            '例如：{"https://example.com/report": "A", "https://blog.com/post": "C"}\n'
        ...
        try:
            ...
            scores = self.extract_json(result, expected_type=dict)
            ...
            for page_key, sources in page_sources.items():
                ...
                for s in sources:
                    if s.get("from_existing"):
                        ...
                        continue
                    url = s.get("url", "")
                    grade = str(scores.get(url, "")).strip().upper()
            ...
            logger.info("[P6.2] 来源评分完成，合格 %d / %d", len(scores), len(all_sources))
            return filtered
        except (ValueError, TypeError) as e:
    ...
    async def _backfill_search(
        ...
        for page in gap_pages:
            for q in page.get("research_queries", []):
                backfill_queries.append({"page": page["page_number"], "query": f"{q} report 白皮书"})
            for d in page.get("data_needs", []):
                backfill_queries.append({"page": page["page_number"], "query": f"{d} 官方数据"})
...
class FetchValidateNode(PlanNode):
    ...
    async def _fetch_and_validate(
        ...
        page_extractions = await self._batch_fetch(pages, page_sources)
        ...
        for page in pages:
            ...
            if extra_urls and extra_urls.get(page_key):
                for url in extra_urls[page_key]:
                    if url not in {s.get("url") for s in top_sources}:
                        top_sources.append({"url": url})

            data_needs_desc = "；".join(page.get("data_needs", [])) or page.get("title", "")
            prompt_text = (
                f"从本文提取关于「{data_needs_desc}」的信息，仅输出以下结构化内容，禁止输出全文或无关内容：\n"
                "1. 关键事实（具体数据点、统计数字，带年份）\n"
                "2. 核心观点（1-2句结论性陈述）\n"
                "3. 案例信息（具体公司/产品/实施情况，含名称）\n"
    ...
    async def _validate_data_sufficiency(
        ...
        cross_validation_rule = {
            "L3": "L3：关键数据需 ≥3 独立来源",
            "L2": "L2：关键数据需 ≥2 独立来源（单源时需标注）",
            "L1": "L1：允许单源，但需标注",
        }.get(research_depth, "L2：关键数据需 ≥2 独立来源（单源时需标注）")
        ...
        for page in pages:
            ...
            if not extractions:
                gap_pages.append((page, ["无任何抓取内容"]))
                continue

            combined = "\n---\n".join(e.get("content", "") for e in extractions)
            ...
            prompt = (
                ...
                f"独立来源数：{source_count}\n\n"
                f"抓取内容：\n{combined[:3000]}\n\n"
                "校验项（4 项）：\n"
                "1. 证据密度：每页 ≥3 条 key_findings 且关键数据点 ≥5 条\n"
                "2. 数据类型覆盖：≥2 种数据类型（绝对值/百分比/排名/增长率）\n"
                "3. 时序/对比数据：trend/data/comparison/technology 页需有结构化表格数据"
                "（时序 ≥3 时间点；对比 ≥2 对象 × ≥2 维度）\n"
                f"4. 交叉验证：{cross_validation_rule}\n\n"
                '输出 JSON：{"pass": true/false, "missing": ["缺失类别1", "缺失类别2"]}\n'
                "missing 字段必须从以下受控词汇表中选取（可多选）：\n"
                "- \"证据密度不足\"：key_findings <3 或 关键数据点 <5\n"
                "- \"数据类型单一\"：仅有 1 种数据类型（绝对值/百分比/排名/增长率）\n"
                "- \"缺时序数据\"：trend/data 页未提取到 ≥3 时间点\n"
                "- \"缺对比数据\"：comparison/technology 页未提取到 ≥2 对象 × ≥2 维度\n"
                "- \"来源不足\"：未达到当前研究深度的来源数要求\n"
                "通过则输出 missing: []。\n"
                "只输出 JSON，不要输出其他内容。"
            )
            try:
                ...
                if isinstance(check, dict) and not check.get("pass", False):
                    ...
                    if isinstance(raw_missing, list):
                        ...
                        missing = ["未说明"]
                    logger.info("[P6.3] 页面 %s 数据不充分: %s", page_key, missing)
                    gap_pages.append((page, missing))
            except (ValueError, TypeError) as e:
    ...
    async def _backfill_fetch(
        ...
        if extra_urls:
            backfill = await self._batch_fetch(gap_page_list, page_sources, extra_urls=extra_urls)
            for page_key, extractions in backfill.items():
                page_extractions.setdefault(page_key, []).extend(extractions)

        still_gap = await self._validate_data_sufficiency(gap_page_list, page_extractions, research_depth)
...
class WriteResearchNode(PlanNode):
    ...
    async def _execute(self, inputs: dict[str, Any]) -> dict[str, Any]:
        ...
        research_md = await self._write_research(
            pages=pages,
            page_extractions=page_extractions,
            source_material=source_material,
    ...
    async def _write_research(
        ...
        for page in pages:
            ...
            extractions = page_extractions.get(page_key, [])
            if extractions:
                extraction_summary += f"\n### P{page['page_number']}: {page.get('title', '')}\n"
                for ext in extractions:
                    extraction_summary += f"来源: {ext['url']}\n{ext['content']}\n\n"

        material_section = ""
        if source_material:
            material_limit = 12000 if config.search_mode == "no_search" else 3000
            truncated = source_material[:material_limit]
            note = ""
            if len(source_material) > material_limit:
                note = f"\n\n[素材已截断，原长 {len(source_material)} 字，仅展示前 {material_limit} 字]"
            material_section = f"\n\n用户素材：\n{truncated}{note}"

        min_words = _WORD_COUNT_MAP.get(config.research_depth, 2000)
        if config.search_mode == "no_search":
            min_words = _WORD_COUNT_NO_SEARCH_MAP.get(config.research_depth, 1200)

        pages_list_text = "\n".join(
            f"- P{p['page_number']}: {p.get('title', '')} | type={p.get('page_type', p.get('type', ''))}"
            for p in pages
        )
        expected_page_nums = sorted(int(p["page_number"]) for p in pages)
        expected_pages_text = "、".join(f"P{n}" for n in expected_page_nums)

        prompt = (
            "你是一位深度内容研究员。请根据以下信息撰写 PPT 深度研究报告，写入 research.md。\n\n"
            f"主题：{config.topic}\n"
            f"搜索模式：{config.search_mode}\n"
            f"研究深度：{config.research_depth}\n"
            f"最低字数要求：{min_words} 字\n\n"
            "### 需要研究的页面列表（必须逐页输出，编号不可更改）\n"
            f"{pages_list_text}\n\n"
            "### 页面编号硬规则（违反将导致下游解析失败）\n"
            "1. research.md 中每个章节标题必须使用上方列表中的原始页码：`### P{page_number}: {title}`\n"
            f"2. 必须且只能输出以下页码的章节：{expected_pages_text}\n"
            "3. 禁止按主题重新编号（如把 P3/P4/P5 合并重排为 P1/P2/P3）\n"
            "4. 禁止把多个页面合并为一个研究章节，即使主题相近也必须分别输出\n"
            "5. 章节数量必须等于上方列表的页面数量，不得多写也不得少写\n"
            "6. 不在列表中的页面（如封面/目录/总结等 ❌ 页面）不要输出\n\n"
            "### 严格格式要求\n"
            "```\n"
        ...
        result = await self.stream_llm_collect(
            prompt=prompt,
            system_prompt="你是深度内容研究员，直接输出 research.md 的完整 Markdown 内容，不要输出解释。",
        )
