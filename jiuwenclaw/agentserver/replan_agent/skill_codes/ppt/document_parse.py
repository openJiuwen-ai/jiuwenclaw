...
class DocumentParseNode(PlanNode):
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            instruction=(
                "条件文档解析：has_documents 时按 doc_paths 逐个读取附件；"
                "文本类用 read_file，图片类优先 image_ocr、其次 visual_question_answering；"
                "合并写入 {output_dir}/doc_raw.md 并校验非空；"
                "若用户未指定 topic，则 call_llm 结合用户请求与 doc_raw 推断主题。"
                "不在 ctx 中传递 doc_content，下游从 doc_raw_path 读取。"
            ),
        )
