...
class P41NormalizeNode(PlanNode):
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            instruction=(
                "读取 doc_raw_path 作为 source_material；call_llm 评估 material_richness；"
                "按 search_mode × 素材充裕度规则表计算 p4_should_search。"
            ),
        )
...
class P42QuickResearchNode(PlanNode):
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            instruction=(
                "p4_should_search 为 true 时：LLM 生成固定批次 query，并行 web_search，"
                "将搜索结果存入 search_results 供 P4.3 使用；否则跳过。"
            ),
        )
...
class P43OutlineGenNode(PlanNode):
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            instruction=(
                "读取 search_results（如有）与素材；"
                "按 source_type 策略 call_llm 生成大纲，"
                "并通过 write_file 落盘 {output_dir}/outline.md。"
            ),
        )
...
class P44ValidateNode(PlanNode):
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            instruction=(
                "通过 read_file 读取 outline.md，"
                "规则校验：`# 大纲：`、`## 页面规划`、✅ 页研究查询/数据需求；"
                "搜索模式下含 `## 已搜索来源` 章节。"
            ),
        )
...
class ContentPlanNode(PlanNode):
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            instruction=(
                "内容策划：P4.1 标准化与素材评估；P4.2 条件快速调研；"
                "P4.3 生成大纲；P4.4 产物校验。"
            ),
            sub_plans=[
                P41NormalizeNode(),
