...
class PageGenerateNode(PlanNode):
    """P8.1 — 分批并发调用 LLM 生成 N 页 HTML。"""

    def __init__(self) -> None:
        super().__init__(
            plan_name="p8_1_page_generate",
            instruction=(
                "## P8.1 分批并发生成 HTML\n"
                "\n"
                "### 前置条件\n"
                ...
                "- `outline_text` / `research_text` / `style_text`（透传给后续密度检查复用）\n"
                "\n"
                "### 执行流程\n"
                "1. 一次性读取 outline.md / research.md / style_file_path（任一失败 → missing_pages = 全部）\n"
                "2. 按 `### P{N}:` 章节拆分 outline 和 research，每页只取对应片段；拆分失败时回退全文\n"
                "3. 为 1..N 每页构造 prompt，包含：\n"
                ...
                "\n"
                "### 失败兜底\n"
                "- 读资料失败：missing_pages = 全部页码，直接返回\n"
                "- LLM 调用 raise / 返回空 / HTML 校验失败：进 missing_pages\n"
                "- write_file 异常：进 missing_pages\n"
                "- 重试后仍缺失：透传给根节点\n"
            ),
        )
    ...
    async def _execute(self, inputs: dict[str, Any]) -> dict[str, Any]:
        ...
        missing_inputs = [name for name, val in required_inputs if not val]
        if page_count <= 0 or missing_inputs:
            logger.error("[P8.1] 必填输入缺失，跳过生成")
            return {
                "page_files": [],
                "missing_pages": list(range(1, page_count + 1)) if page_count > 0 else [],
                "outline_text": "",
                "research_text": "",
                "style_text": "",
            }
        ...
        if not outline_text or not research_text or not style_text:
            logger.error(
                "[P8.1] 资料读取失败 outline=%d research=%d style=%d",
                len(outline_text),
                len(research_text),
                len(style_text),
            )
        ...
        if not outline_pages:
            logger.warning("[P8.1] outline.md 未拆分到任何页面章节，回退全文")
        if not research_pages:
            logger.warning("[P8.1] research.md 未拆分到任何页面章节，回退全文")

        all_pages = list(range(1, page_count + 1))
        missing_pages = await self._generate_pages(
            all_pages,
            pages_dir=pages_dir,
            style_id=style_id,
            style_text=style_text,
        ...
        logger.info(
            "[P8.1] 生成完成 success=%d/%d missing=%s",
    ...
    async def _execute_stream(self, inputs: dict[str, Any]) -> AsyncIterator[dict[str, Any]]:
        ...
        yield {
            **result,
            "node": self.plan_name,
            "status": "ok" if not result.get("missing_pages") else "warning",
            "message": (
                f"已生成 {len(result.get('page_files', []))} 页，"
                f"缺失 {len(result.get('missing_pages', []))} 页"
            ),
        }


class DensityCheckNode(PlanNode):
    """P8.2 — 内容密度检查 + 1 轮重写。"""

    def __init__(self) -> None:
        super().__init__(
            plan_name="p8_2_density_check",
            instruction=(
                "## P8.2 内容密度检查与重写\n"
                "\n"
                "### 前置条件\n"
                ...
                "- 上游已落盘 page-*.pptx.html\n"
                "\n"
                "### 输入\n"
                "- `pages_dir` / `page_files` / `style_id`\n"
                "- `outline_text` / `research_text` / `style_text`（来自上游）\n"
                ...
                "### 执行流程\n"
                "1. 并发读所有 page HTML\n"
                "2. 并发调 LLM 做 8 项密度检查（受控 JSON 输出），检查项：\n"
                "   - 数据可视化：≥1 个 ECharts 图表 或 ≥3 个数据卡片（no_search 模式且页面为'数据有限'时可降至 2 个数据卡片）\n"
                "   - 核心要点：6-10 个列表项或卡片\n"
                "   - 装饰图标：≥3 个 FontAwesome 图标（class 含 `fa-`）\n"
                "   - 空白率：估算 < 30%\n"
                "   - 数据来源：页脚有标注（机构名 / 资料名）\n"
                "   - 无大段文字：无连续 > 100 字段落\n"
                ...
                "   c. 将搜索结果 + 原有素材 + 不通过项提示词构造重写 prompt，调 LLM 重新生成 HTML\n"
                "   d. 若无需搜索（如缺装饰图标/大段文字/视觉层级/布局错误），直接用原有素材 + 不通过项提示词重写\n"
                "4. 重试 density_retry_round 轮（默认 1），仍不通过进 low_density_pages\n"
                "\n"
                "### 数据转换规则（搜索补充后）\n"
                "| 获取内容 | 转换方式 |\n"
                "|---|---|\n"
                ...
                "\n"
                "### 失败兜底\n"
                "- 检查 LLM 调用失败：保守视为通过（避免假阳性触发无意义重写）\n"
                "- web_search 不可用或搜索失败：跳过搜索补充，仅用原有素材重写\n"
                "- 重写 LLM 调用失败：保留当前 HTML，进 low_density_pages\n"
                "- 1 轮后仍不通过：保留当前 HTML 供人工排查\n"
            ),
        )

    async def _execute(self, inputs: dict[str, Any]) -> dict[str, Any]:
        pages_dir = str(inputs.get("pages_dir") or "").strip()
        page_files: list[str] = list(inputs.get("page_files") or [])
        style_id = str(inputs.get("style_id") or "").strip()
        outline_text = str(inputs.get("outline_text") or "")
        research_text = str(inputs.get("research_text") or "")
        ...
        if not pages_dir or not page_files:
            ...
            return {"low_density_pages": [], "density_report": {}}

        outline_pages = _split_md_pages(outline_text) if outline_text else {}
        research_pages = _split_md_pages(research_text) if research_text else {}
        has_rewrite_material = bool(outline_text and research_text and style_text)
        can_search = self.has_tool("web_search")
        if not has_rewrite_material and not can_search:
            logger.warning("[P8.2] 缺少重写素材且无搜索能力，不通过页直接进 low_density_pages")

        page_numbers = sorted({_extract_page_number(f) for f in page_files if _extract_page_number(f) > 0})
        ...
        for round_idx in range(retry_round):
            if not retry_queue:
                break
            if not has_rewrite_material and not can_search:
                logger.warning("[P8.2] 缺少重写素材且无搜索能力，跳过重写")
                break
            logger.info(
                "[P8.2] 第 %d 轮重写，待修复页: %s",
                round_idx + 1,
            ...
            for page_num in retry_queue:
                ...
                density_report[page_num] = check
                if not check.get("pass", True):
                    new_queue.append(page_num)
            retry_queue = new_queue

        low_density_pages = retry_queue
        logger.info(
            "[P8.2] 密度检查完成 low_density=%s",
            low_density_pages,
        )
        return {
    ...
    async def _check_one_page(
        ...
        html = await self._read_file(f"{pages_dir}/page-{page_num}.pptx.html")
        if not html:
            return {"pass": False, "failed_items": ["文件读取失败"], "reason": "html_empty"}
    ...
    async def _rewrite_pages(
        ...
        *,
        ...
        if self.has_tool("web_search"):
            search_pages = [
                p for p in pages
                if set(density_report.get(p, {}).get("failed_items") or []) & self._SEARCH_NEEDED_ITEMS
            ]
            if search_pages:
                search_supplements = await self._search_supplement(
                    search_pages, density_report=density_report, research_pages=research_pages, topic=topic,
                )

        original_htmls: dict[int, str] = {}
    ...
    async def _search_supplement(
        ...
        *,
        ...
        for page_num in pages:
            failed_items = set(density_report.get(page_num, {}).get("failed_items") or [])
            search_items = failed_items & self._SEARCH_NEEDED_ITEMS
            if not search_items:
                continue

            page_research = research_pages.get(page_num, "")
            page_keywords = self._extract_page_keywords(page_research) if page_research else []

            page_snippets: list[str] = []
            for item in search_items:
                templates = self._SEARCH_QUERY_TEMPLATES.get(item, [])
                queries = self._build_search_queries(templates, topic=topic, page_keywords=page_keywords)
                if not queries:
                    continue
                for query in queries[:2]:
                    try:
                        raw = await self.call_tool("web_search", query=query)
                        snippet = self._extract_search_text(raw)
                        if snippet:
                            page_snippets.append(f"[{item}] 搜索 \"{query}\" 结果：\n{snippet}")
                            break
                    except Exception as e:
                        if isinstance(e, AbortError):
                            raise
                        logger.warning("[P8.2] 页面 %d 搜索补充失败 item=%s query=%s: %s", page_num, item, query, e)

            if page_snippets:
                supplements[page_num] = "\n\n".join(page_snippets)
    ...
    def _extract_page_keywords(self, research_page: str) -> list[str]:
    ...
    def _extract_search_text(self, result: Any) -> str:
        if result is None:
            return ""
        ...
        if isinstance(result, dict):
            for key in ("results", "items", "data"):
                ...
                if isinstance(v, list):
                    return self._extract_search_text(v)
            content = result.get("content") or result.get("text") or ""
            if isinstance(content, str):
                return content[:2000]
        return str(result)[:2000]

    def _build_rewrite_hint(self, failed_items: list[str]) -> str:
        if not failed_items:
            return ""
        actions = {
            "缺数据可视化": (
        ...
        for item in failed_items:
            action = actions.get(item, "针对性优化该项")
    ...
    async def _read_file(self, path: str) -> str:
        ...
        try:
            ...
            if isinstance(e, AbortError):
                raise
            logger.warning("[P8.2] 读取文件失败 %s: %s", path, e)
            return ""
    ...
    async def _write_file(self, path: str, content: str) -> bool:
        ...
        try:
            ...
            if isinstance(e, AbortError):
                raise
            logger.error("[P8.2] 写入文件失败 %s: %s", path, e)
            return False
    ...
    async def _execute_stream(self, inputs: dict[str, Any]) -> AsyncIterator[dict[str, Any]]:
        ...
        yield {
            **result,
            "node": self.plan_name,
            "status": "ok" if not low else "warning",
            "message": f"密度检查完成，低密度页 {len(low)} 个",
        }
...
class PPTPageGenNode(PlanNode):
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            instruction=(
                ...
                "2. 全程统一并发（asyncio.gather + 分批），不区分单 Agent 模式\n"
                "3. 三阶段：生成 → 密度检查与重写 → QA 与自动修复\n"
                "\n"
                "### 输入\n"
                ...
                "2. 调用 P8.1 PageGenerateNode → page_files / missing_pages / 共享文本\n"
                "3. 调用 P8.2 DensityCheckNode → low_density_pages\n"
    ...
    async def _execute(self, inputs: dict[str, Any]) -> dict[str, Any]:
        ...
        gen_result = await self.execute_subplan(self.sub_plans[0], inputs)
        if isinstance(gen_result, dict):
            inputs.update(gen_result)
        missing_pages = list(gen_result.get("missing_pages") or []) if isinstance(gen_result, dict) else []
        page_files = list(gen_result.get("page_files") or []) if isinstance(gen_result, dict) else []

        density_result = await self.execute_subplan(self.sub_plans[1], inputs)
        low_density_pages = (
            list(density_result.get("low_density_pages") or [])
            if isinstance(density_result, dict)
            else []
        )

        qa_result = await self.execute_subplan(self.sub_plans[2], inputs)
        qa_status = "ok"
        final_page_files = page_files
        fix_report = ""
