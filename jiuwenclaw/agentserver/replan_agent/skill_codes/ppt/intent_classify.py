...
class IntentClassifyNode(PlanNode):
    ...
    说明：PPT 任务是否进入本 workflow 由 RePlanAgent / Planner 在入口判定；
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            instruction=(
                "workflow 内文档检测：优先读取 attachments + files，并用 LLM 从 query "
                "提取额外路径；无附件且 query 无路径时，同一 LLM 调用提取槽位信息 "
                "供 P2 快捷使用；写入 has_documents、doc_paths 及 slots_from_query。"
            ),
        )
