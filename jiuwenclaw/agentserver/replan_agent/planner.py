...
"""RePlanPlanner —— 任务匹配 skill 并返回 plan_code。"""
...
if TYPE_CHECKING:
    from jiuwenclaw.agentserver.replan_agent.environment import RePlanEnvironment, Skill

logger = logging.getLogger(__name__)

_SKILL_CODES_PACKAGE = "jiuwenclaw.agentserver.replan_agent.skill_codes"
...
class RePlanPlanner:
    ...
    async def plan(self, task: str, context: dict[str, Any] | None = None) -> str | None:
        ...
        无匹配或 skill 无预规划代码时返回 None，由 RePlanAgent 降级处理。
        """
        task = (task or "").strip()
        if not task:
            logger.info("[RePlanPlanner] empty task, skip planning")
            return None

        logger.info(
            "[RePlanPlanner] planning task=%s context_keys=%s skill_names=%s",
            task,
            list((context or {}).keys()),
            list(self._env.skills.keys()),
        )

        skill = await self._match_skill(task, context)
        if skill is None:
            logger.info("[RePlanPlanner] no skill routed")
            return None

        plan_code = self._build_skill_plan_code(skill)
        if not plan_code:
            logger.warning(
                "[RePlanPlanner] matched skill but no plan_code resolved "
                "skill=%s skill_dir=%s",
                skill.name,
                self._skill_codes_dir() / skill.name,
            )
            return None

        logger.info(
            "[RePlanPlanner] matched skill=%s plan_code_len=%d",
            skill.name,
            len(plan_code),
        )
        return plan_code
    ...
    async def _match_skill(
        ...
        if not registered_skills:
            logger.info("[RePlanPlanner] no registered skills")
            return None

        client = getattr(self._env, "model_client", None)
        if client is None:
            logger.warning("[RePlanPlanner] model_client is not configured")
            return None
        ...
        logger.info("[RePlanPlanner] route skills detail: %s", skills_payload)
        ...
        try:
            ...
            logger.warning(
                "[RePlanPlanner] LLM route failed error_type=%s",
                type(e).__name__,
            )
            return None
        ...
        logger.info(
            "[RePlanPlanner] LLM route result skill=%s confidence=%s",
            skill_name,
            confidence,
        )
        ...
        if confidence < _MIN_ROUTE_CONFIDENCE:
            logger.info(
                "[RePlanPlanner] route confidence too low skill=%s confidence=%s",
                skill_name,
                confidence,
            )
            return None
        ...
        if skill is None:
            logger.warning("[RePlanPlanner] routed unknown skill: %s", skill_name)
            return None
        return skill
...
5. 在已生成的 PPT 内容基础上迭代优化文稿
6. 以已有 PPT 文件内容为参考素材，重新生成全新版本 PPT
...
7. context 中存在 "__replan_prior_artifacts__" 字段时，需对比该字段记录的已完成产物信息
   与当前任务意图：
...
""".strip()
    ...
    def _build_skill_plan_code(self, skill: Skill) -> str | None:
        ...
        if root_file is not None:
            ...
            logger.info(
                "[RePlanPlanner] use discovered skill root skill=%s file=%s module=%s",
                skill.name,
                root_file,
                module,
            )
        ...
        if skill.plan_code:
            logger.info("[RePlanPlanner] use configured plan_code skill=%s", skill.name)
