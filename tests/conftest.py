...
def _wrap_init_for_extra_kwargs(cls: type) -> None:
    ...
    if getattr(original, "_jiuwenswarm_kwargs_compat", False):
        return

    def _compat(self, *args, **kwargs):
        return original(self, *args, **_filter_unsupported_kwargs(original, kwargs))

    _compat._jiuwenswarm_kwargs_compat = True  # type: ignore[attr-defined]
    cls.__init__ = _compat  # type: ignore[method-assign]


def _install_evolution_rail_kwargs_compat() -> None:
    """CI rails may lack signal_trigger / trajectory_span_processor parameters."""
    try:
        rails = import_module("openjiuwen.harness.rails")
    except ModuleNotFoundError:
        return
...
def pytest_configure() -> None:
    ...
    _install_evolution_rail_kwargs_compat()
