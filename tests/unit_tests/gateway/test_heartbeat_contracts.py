...
import jiuwenswarm.gateway.app_gateway as app_gateway_module
...
async def test_health_check_writes_new_internal_protocol_and_relay() -> None:
    ...
    assert "run" not in envelope.params
...
def test_gateway_config_migration_failure_preserves_channel_config(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    expected = {
        "health_check": {"every": 30},
        "channels": {"wecom": {"enabled": True}},
    }

    def fail_migration() -> bool:
        raise OSError("config lock unavailable")

    class Handler:
        def __init__(self) -> None:
            self.received = None

        def update_evolution_auto_save(self, full_cfg) -> None:  # noqa: ANN001
            self.received = full_cfg
    ...
    monkeypatch.setattr(config_module, "get_config", lambda: expected)
    handler = Handler()

    full_cfg, health_check_cfg, channels_cfg = (
        app_gateway_module._load_gateway_runtime_config(handler)
    )

    assert full_cfg == expected
    assert health_check_cfg == {"every": 30}
    assert channels_cfg == {"wecom": {"enabled": True}}
    assert handler.received == expected


def test_gateway_evolution_config_failure_preserves_channel_config(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    expected = {
        "health_check": {"every": 30},
    ...
    class Handler:
        @staticmethod
        def update_evolution_auto_save(_full_cfg) -> None:  # noqa: ANN001
            raise RuntimeError("evolution config unavailable")
...
async def test_heartbeat_scheduler_start_failure_is_nonfatal_to_gateway() -> None:
    class Scheduler:
        async def start(self) -> None:
            raise ValueError("corrupt heartbeat store")
...
def test_wecom_health_check_session_uses_configured_fallback_chat(
    session_id: str,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        config_module,
        "get_config",
        lambda: {"channels": {"wecom": {"last_chat_id": "fallback-chat"}}},
    )
    channel = object.__new__(WecomChannel)
    msg = Message(
        id="healthcheck-relay-probe",
        type="event",
        channel_id="wecom",
    ...
    assert channel._extract_chatid(msg) == "fallback-chat"
