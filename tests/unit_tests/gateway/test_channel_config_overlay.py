...
def test_overlay_enabled_standalone():
    with patch(
        "jiuwenswarm.gateway.channel_config_overlay._gateway_deployment_mode",
...
def test_deployment_mode_env_fallback_when_config_raises(monkeypatch: pytest.MonkeyPatch):
    ...
    with patch("jiuwenswarm.common.config.get_config", side_effect=_boom):
...
async def test_trigger_reload_serializes_concurrent_calls():
    ...
    with patch(
        "jiuwenswarm.gateway.channel_config_overlay.channel_config_overlay_enabled",
        return_value=True,
    ):
...
async def test_fetch_active_rows_delegates_to_db_loader():
    ...
    with patch(
        "jiuwenswarm.gateway.channel_config_overlay.load_active_channel_config_rows",
