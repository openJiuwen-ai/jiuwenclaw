...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    ("video_live_mode", "realtime_provider", "expected"),
    [
        ("joyai", "qwen_omni", "joyai"),
        ("realtime", "qwen_omni", "qwen_omni"),
        ("realtime", "minicpm", "minicpm"),
        ("", "", "minicpm"),
    ],
)
async def test_config_get_returns_effective_video_live_provider(
    monkeypatch, video_live_mode, realtime_provider, expected
):
    channel = FakeWebChannel()
    monkeypatch.setenv("VIDEO_LIVE_MODE", video_live_mode)
    monkeypatch.setenv("VIDEO_REALTIME_PROVIDER", realtime_provider)
    ...
    await channel.methods["config.get"](
        object(),
        "req-get-video-provider",
        {},
        "sess-get-video-provider",
    )

    assert channel.responses[-1]["ok"] is True
    assert channel.responses[-1]["payload"]["video_live_provider"] == expected


@pytest.mark.asyncio
async def test_config_set_switches_video_live_provider_and_saves_runtime_fields(
    ...
    await channel.methods["config.set"](
        ...
        "sess-set-video-provider",
    )

    response = channel.responses[-1]
    assert response["ok"] is True
    assert response["payload"]["updated"] == [
        "video_live_provider",
        "joyai_api_base",
        "joyai_model",
        "voice_protocol",
    ...
    assert "VIDEO_REALTIME_PROVIDER=" in persisted
    assert 'JOYAI_API_BASE="http://127.0.0.1:8070/v1"' in persisted
    assert 'JOYAI_MODEL_NAME="jdopensource/JoyAI-VL-Interaction"' in persisted
    assert 'VOICE_PROTOCOL="native_ws"' in persisted
    assert 'VOICE_ASR_ENDPOINT="ws://127.0.0.1:8994/ws/asr"' in persisted
    assert 'VOICE_TTS_ENDPOINT="ws://127.0.0.1:8992/ws/tts"' in persisted
    assert "VOICE_API_KEY=" in persisted
    assert 'VOICE_TTS_VOICE="vivian"' in persisted


@pytest.mark.asyncio
async def test_config_set_rejects_unknown_video_live_provider(monkeypatch):
    channel = FakeWebChannel()
    monkeypatch.setattr(app_web_handlers, "get_config", lambda: {})
    _register_web_handlers(WebHandlersBindParams(channel=channel))
    ...
    await channel.methods["config.set"](
        object(),
        "req-invalid-video-provider",
        {"video_live_provider": "unknown"},
        "sess-invalid-video-provider",
    )
...
@pytest.mark.asyncio
async def test_config_set_rejects_unknown_voice_protocol(monkeypatch):
    channel = FakeWebChannel()
    monkeypatch.setattr(app_web_handlers, "get_config", lambda: {})
    _register_web_handlers(WebHandlersBindParams(channel=channel))
    ...
    await channel.methods["config.set"](
        object(),
        "req-invalid-voice-protocol",
        {"voice_protocol": "unknown"},
        "sess-invalid-voice-protocol",
    )
...
@pytest.mark.asyncio
async def test_config_get_migrates_legacy_voice_fields_to_unified_schema(monkeypatch):
    channel = FakeWebChannel()
    for key in (
        "VOICE_PROTOCOL",
        "VOICE_ASR_ENDPOINT",
    ...
    monkeypatch.setattr(
        "jiuwenswarm.extensions.registry.ExtensionRegistry.get_instance",
        lambda: type("Registry", (), {"get_crypto_provider": lambda self: None})(),
    )
    monkeypatch.setenv("JOYAI_VOICE_PROVIDER", "siliconflow")
    monkeypatch.setenv("ASR_API_BASE", "https://asr.example/v1")
    monkeypatch.setenv("ASR_API_KEY", "shared-key")
    monkeypatch.setenv("ASR_MODEL_NAME", "asr-model")
    monkeypatch.setenv("ASR_API_MODE", "transcription")
    monkeypatch.setenv("TTS_API_BASE", "https://tts.example/v1")
    monkeypatch.setenv("TTS_MODEL_NAME", "tts-model")
    monkeypatch.setenv("TTS_VOICE", "anna")
    monkeypatch.setattr(app_web_handlers, "get_config_raw", lambda: {})
    monkeypatch.setattr(app_web_handlers, "get_config", lambda: {})
    _register_web_handlers(WebHandlersBindParams(channel=channel))

    await channel.methods["config.get"](
        object(), "req-get-legacy-voice", {}, "sess-get-legacy-voice"
    )

    payload = channel.responses[-1]["payload"]
    assert payload["voice_protocol"] == "openai_http"
    assert payload["voice_asr_endpoint"] == "https://asr.example/v1/audio/transcriptions"
    assert payload["voice_tts_endpoint"] == "https://tts.example/v1/audio/speech"
    assert payload["voice_api_key"] == "shared-key"
    assert payload["voice_asr_model"] == "asr-model"
    assert payload["voice_tts_model"] == "tts-model"
    assert payload["voice_tts_voice"] == "anna"
