...
def test_compose_full_access_network_ignores_user_hosts(isolated_data_dir: Path) -> None:
    from jiuwenswarm.agents.harness.common.rails.permissions.permission_compose import (
        PermissionModeController,
    )

    result = PermissionModeController().compose(
        {"enabled": True, "mode": "full_access"},
        {
            "network": {
                "hosts": [{"pattern": "evil.example", "action": "deny"}],
            }
        },
    )
    network = result.permissions["network"]
    assert network["ignore_user_host_rules"] is True
    assert network["defaults"] == "allow"
    assert network.get("hosts") in (None, [])


def test_compose_strict_injects_path_tool_allows(isolated_data_dir: Path) -> None:
    from jiuwenswarm.agents.harness.common.rails.permissions.permissions_layers import (
        compose_host_effective_permissions,
    )

    _write_global(isolated_data_dir, {"enabled": True, "mode": "strict"})
    eff = compose_host_effective_permissions()
    tools = eff.get("tools") or {}
    assert tools.get("read_file") == "allow"
    assert tools.get("write_file") == "allow"
    assert eff["defaults"]["*"] == "ask"
    assert eff["file_guard"]["enabled"] is True
    assert eff["permission_mode"] == "strict"


def test_resolve_sysop_placement_matrix() -> None:
    from jiuwenswarm.agents.harness.common.rails.permissions.permissions_layers import (
        resolve_sysop_placement,
    )

    assert resolve_sysop_placement("required", enabled=False, available=True) == ("sandbox", False)
    assert resolve_sysop_placement("required", enabled=True, available=False) == ("host", True)
    assert resolve_sysop_placement("optional", enabled=True, available=True) == ("sandbox", False)
    assert resolve_sysop_placement("optional", enabled=False, available=True) == ("host", False)
    assert resolve_sysop_placement("optional", enabled=True, available=False) == ("host", False)
...
def test_build_permission_rail_mounts_for_legacy_full_access(isolated_data_dir: Path) -> None:
    ...
    cfg = getattr(rail, "_static_config", None) or {}
    if cfg:
        assert cfg.get("defaults", {}).get("*") == "allow"
        assert cfg.get("file_guard", {}).get("enabled") is False


def test_build_permission_rail_composes_auto_not_fail_closed(isolated_data_dir: Path) -> None:
    """Product auto must be baked before the rail; unbaked mode=auto would Fail-Closed to ask."""
    from jiuwenswarm.agents.harness.common.rails.interrupt.interrupt_helpers import (
        build_permission_rail,
    )
    ...
    cfg = getattr(rail, "_static_config", None) or {}
    assert cfg.get("defaults", {}).get("*") == "allow"
    assert cfg.get("file_guard", {}).get("enabled") is True
    assert cfg.get("file_guard", {}).get("workspace", {}).get("write") == "allow"
    assert getattr(rail, "sandbox_intent", None) == "required"


def test_build_permission_rail_composes_strict_path_tools(isolated_data_dir: Path) -> None:
    from jiuwenswarm.agents.harness.common.rails.interrupt.interrupt_helpers import (
        build_permission_rail,
    )

    rail = build_permission_rail({"permissions": {"enabled": True, "mode": "strict"}})
    assert rail is not None
    cfg = getattr(rail, "_static_config", None) or {}
    assert cfg.get("defaults", {}).get("*") == "ask"
    tools = cfg.get("tools") or {}
    assert tools.get("read_file") == "allow"
    assert tools.get("write_file") == "allow"
    assert cfg.get("file_guard", {}).get("workspace", {}).get("write") == "ask"
    assert getattr(rail, "sandbox_intent", None) == "required"


@pytest.mark.filterwarnings("ignore::SyntaxWarning")
def test_update_permission_rail_bakes_legacy_full_access(
    isolated_data_dir: Path,
) -> None:
    ...
    adapter._model = None
    adapter._update_permission_rail({"permissions": {"enabled": False}})

    adapter._permission_rail.update_config.assert_called_once()
    baked = adapter._permission_rail.update_config.call_args[0][0]
    assert baked["enabled"] is True
    assert baked["mode"] == "full_access"
    assert baked["defaults"]["*"] == "allow"
    assert baked["file_guard"]["enabled"] is False
    assert baked["sandbox_intent"] == "optional"
