...
from jiuwenavatar.agents.harness.common.prompt.prompt_builder import build_agent_identity_prompt
from jiuwenavatar.agents.harness.common.rails import runtime_prompt_rail as runtime_prompt_rail_module
from jiuwenavatar.agents.harness.common.rails.runtime_prompt_rail import RuntimePromptRail
...
def test_deep_adapter_subagents_includes_optional_browser_and_configured_research():
    ...
    adapter.set_workspace_dir("/tmp/jiuwenavatar-workspace")
    model = object()
    config = {
        "max_iterations": 9,
        "subagents": {
    ...
    with (
        ...
        patch(
            "jiuwenavatar.server.runtime.agent_adapter.interface_deep.build_research_agent_config",
            return_value="research_spec",
        ) as mock_research,
        patch(
            "jiuwenavatar.server.runtime.agent_adapter.interface_deep.build_browser_agent_config",
    ...
    mock_research.assert_called_once_with(
        model,
        workspace="/tmp/jiuwenavatar-workspace",
        language="cn",
        max_iterations=9,
    )
