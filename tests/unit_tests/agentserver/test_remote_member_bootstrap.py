...
_spec = importlib.util.spec_from_file_location(
    ...
    _root / "jiuwenavatar" / "agents" / "harness" / "team" / "remote_member_bootstrap.py",
)
...
def _mock_a2x_precheck_reservation(monkeypatch) -> SimpleNamespace:
    ...
    monkeypatch.setattr(
        "jiuwenavatar.agents.harness.team.a2x.a2x_registry_runtime.reserve_blank_teammate_agent",
...
@pytest.mark.asyncio
async def test_spawn_member_wrapper_forces_unstarted_after_bootstrap_delivered(monkeypatch):
    from openjiuwen.agent_teams.schema.status import MemberStatus
    from openjiuwen.agent_teams.schema.team import TeamRole
    from openjiuwen.core.runner import Runner
...
@pytest.mark.asyncio
async def test_spawn_member_wrapper_blocks_before_db_when_precheck_fails(monkeypatch):
    from openjiuwen.agent_teams.schema.team import TeamRole
    from openjiuwen.core.runner import Runner
    from openjiuwen.harness.tools.base_tool import ToolOutput
...
@pytest.mark.asyncio
async def test_spawn_member_wrapper_fail_fast_when_bootstrap_not_delivered(monkeypatch):
    from openjiuwen.agent_teams.schema.status import MemberStatus
    from openjiuwen.agent_teams.schema.team import TeamRole
    from openjiuwen.core.runner import Runner
    from openjiuwen.harness.tools.base_tool import ToolOutput
...
async def test_replace_teammate_card_after_direct_bootstrap_uses_local_a2x_state(monkeypatch):
    ...
    monkeypatch.setattr(
        "jiuwenavatar.agents.harness.team.a2x.a2x_registry_runtime.replace_teammate_agent_card_after_bootstrap",
...
async def test_team_destroy_stops_dynamic_member_runtime(monkeypatch):
    ...
    monkeypatch.setattr(
        "jiuwenavatar.agents.harness.team.team_manager.get_team_manager",
        lambda channel_id: SimpleNamespace(destroy_team=destroy_team),
    )
    monkeypatch.setattr("jiuwenavatar.common.config.get_config", lambda: {})
    monkeypatch.setattr(
        "jiuwenavatar.agents.harness.team.a2x.a2x_registry_runtime.restore_teammate_blank_agent_on_destroy",
