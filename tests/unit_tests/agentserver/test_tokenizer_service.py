...
def _config(
    ...
    *,
    ...
    if proxy is not None:
        context_config["tokenizer_proxy"] = proxy
    return {
        "react": {
...
async def test_tokenizer_service_warms_new_model_only_once(tmp_path, monkeypatch):
    ...
    first = await service.warm(_config(tmp_path / "cache", models=first_models), reason="startup")
    second = await service.warm(_config(tmp_path / "cache", models=second_models), reason="model config change")
...
@pytest.mark.asyncio
async def test_tokenizer_service_fallback_is_degraded_and_retryable(tmp_path, monkeypatch):
    calls: list[str] = []
...
@pytest.mark.asyncio
async def test_tokenizer_service_retries_remote_warmup_after_transient_failure(tmp_path, monkeypatch):
...
@pytest.mark.asyncio
async def test_tokenizer_service_forwards_hidden_proxy_to_core(tmp_path, monkeypatch):
    import openjiuwen.core.context_engine as context_engine

    manager_kwargs: list[dict] = []
    ...
    assert result["warmed"] == 1
    assert manager_kwargs[0]["proxy"] == "http://127.0.0.1:7890"


@pytest.mark.asyncio
async def test_configured_model_metadata_is_case_insensitive_and_deduplicated(tmp_path, monkeypatch):
    calls: list[str] = []
...
@pytest.mark.asyncio
async def test_model_variants_share_one_registered_tokenizer_warmup(tmp_path, monkeypatch):
    calls: list[str] = []
