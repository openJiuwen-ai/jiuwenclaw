...
"""Exact send grants are consumed without changing the download owner."""
...
# Test-only relative path returned by a mocked download owner; it is not a
# routable URL and no network request is made.
_TEST_DOWNLOAD_URL = "/test-only-non-routable-download"
...
def _toolkit(*, require_authorization: bool) -> SendFileToolkit:
...
@pytest.mark.asyncio
async def test_auto_permission_send_consumes_exact_grant_and_existing_download(
    tmp_path,
) -> None:
    ...
    download_info = {
        ...
        "download_url": _TEST_DOWNLOAD_URL,
    ...
    with patch(
        ...
        result = await _toolkit(require_authorization=True).send_file(
            source.as_posix(),
            target_channels=("web",),
        )
    ...
    build_download.assert_called_once_with(
        source.as_posix(),
        source.name,
        "session-a",
        user_id="user-a",
    ...
    assert payload["download_url"] == _TEST_DOWNLOAD_URL
    assert payload["download_token"] == "existing-token"
