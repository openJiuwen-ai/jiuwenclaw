...
def test_jiuwenswarm_get_registered_tools_catalog_member() -> None:
    ...
    swarm = JiuWenSwarm()
    with patch.object(swarm, "get_instance", return_value=_DeepInstance()):
        tools = swarm.get_registered_tools_catalog()
    assert len(tools) == 1
