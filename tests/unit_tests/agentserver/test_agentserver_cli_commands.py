...
class AgentWebSocketServerHarness(agent_ws_server_module.AgentWebSocketServer):
    ...
    async def handle_command_autofix_pr_for_test(self, ws, request, send_lock):
...
@pytest.mark.asyncio
async def test_handle_command_autofix_pr_returns_prompt(server, fake_ws):
    """/autofix-pr 无参数时返回五阶段修复 prompt，且不追加 PR number 行。"""
    request = AgentRequest(
        request_id="req-autofix",
        channel_id="tui",
        req_method=ReqMethod.COMMAND_AUTOFIX_PR,
        params={},
    )

    await server.handle_command_autofix_pr_for_test(fake_ws, request, asyncio.Lock())

    assert len(fake_ws.sent) == 1
    msg = fake_ws.sent[0]
    assert msg["response_id"] == "req-autofix"
    assert msg["ok"] is True
    prompt = msg["payload"]["prompt"]
    # Phase structure — Phase 0 detects the platform, then the shared 1..5
    for phase in ("Phase 0", "Phase 1", "Phase 2", "Phase 3", "Phase 4", "Phase 5"):
        assert phase in prompt
    assert "git remote get-url origin" in prompt  # platform auto-detection
    # GitHub branch: reads the failure signal through gh, including the log
    assert "gh pr view" in prompt
    assert "gh pr checks" in prompt
    assert "--log-failed" in prompt
    assert "--comments" in prompt
    # GitCode branch (verified against real jiuwenswarm PRs): the pipeline status lives on
    # web-api's v2 pipeline-check endpoint behind a WAF, so the curl carries browser headers;
    # the failure log is unreachable, so diagnosis falls back to running checks locally.
    assert "web-api.gitcode.com/api/v2/projects/pipeline-check" in prompt
    assert "type=report_pipeline" in prompt
    assert "commit_id=<HEAD_SHA>" in prompt
    # Must pass the exact head sha — omitting it returns the whole (mixed-status) history
    assert "head.sha" in prompt
    assert "WHOLE run history" in prompt
    assert 'Origin: https://gitcode.com' in prompt  # WAF bypass header
    assert "/pulls/<number>/comments" in prompt  # v5 comments (review + CI bot table)
    # GitCode failure detail is reachable via the CI bot comment's report links, verified
    # against real jiuwenswarm PR #3949 (UT report on public OBS gave the failing tests).
    assert "report" in prompt and "--log-failed" in prompt
    assert "encoding='utf-8'" in prompt  # Windows cp1252 gotcha reading GitCode JSON
    # An empty pipeline result (repo without remote CI) must fall to local checks, not "green"
    assert "EMPTY result" in prompt and "NOT a pass" in prompt
    # Safety guardrail: only touch the PR for the branch you are on; never fetch other forks
    assert "Never pull someone" in prompt
    assert "never patch a PR you are not on" in prompt
    # Traceability trailer, mirroring Claude Code's Claude-Session trailer.
    # A live run showed the model silently skipping a prose-only "append the trailer"
    # instruction, so the prompt now hands it the literal command, makes it verify the
    # trailer landed, and repairs it by amending before the push.
    assert "--trailer \"Auto-Fixed-By: jiuwenswarm /autofix-pr\"" in prompt
    assert "git log -1 --format=%B" in prompt
    assert "git commit --amend --trailer" in prompt
    # Co-authored-by trailer marks the machine while keeping the human's author identity
    # (decision C, mirroring Claude Code's Co-authored-by attribution).
    assert "--trailer \"Co-authored-by: jiuwenswarm-autofix <noreply@openjiuwen.com>\"" in prompt
    assert "do not `--author`" in prompt  # keep the human author; only co-author marks the bot
    # And it is restated as a hard rule, not just a step
    assert "must carry BOTH the `Auto-Fixed-By: jiuwenswarm /autofix-pr`" in prompt
    # Guardrails: never fake green, never force-push, hand back when unsure
    assert "NEVER make CI pass by deleting or weakening tests" in prompt
    assert "force-push" in prompt
    assert "hand the PR back to the human" in prompt
    # No PR number line when none was given
    assert not prompt.rstrip().endswith("PR number:")
    assert "PR number: " not in prompt


@pytest.mark.asyncio
async def test_handle_command_autofix_pr_with_arg_appends_pr_number(server, fake_ws):
    """/autofix-pr 带参数时把 PR 号追加到 prompt 末尾（两侧空白被裁掉）。"""
    request = AgentRequest(
        request_id="req-autofix-arg",
        channel_id="tui",
        req_method=ReqMethod.COMMAND_AUTOFIX_PR,
        params={"pr_arg": "  123  "},
    )
    ...
    msg = fake_ws.sent[0]
    assert msg["ok"] is True
    assert msg["payload"]["prompt"].rstrip().endswith("PR number: 123")


@pytest.mark.asyncio
async def test_handle_command_autofix_pr_platform_hint_is_accepted(server, fake_ws):
    """显式 platform=gitcode 不再报错（GitCode 现已支持），而是作为跳过 Phase 0 的提示。

    单一 prompt 同时覆盖两个平台，Agent 按 git remote 自检；给出 platform 只是让它省掉检测。
    """
    request = AgentRequest(
        request_id="req-autofix-gitcode",
        channel_id="tui",
        req_method=ReqMethod.COMMAND_AUTOFIX_PR,
        params={"platform": "gitcode"},
    )
    ...
    assert msg["ok"] is True
    prompt = msg["payload"]["prompt"]
    assert "Detected platform: gitcode" in prompt
    # Still the same dual-platform prompt (GitCode read path present)
    assert "web-api.gitcode.com/api/v2/projects/pipeline-check" in prompt


@pytest.mark.asyncio
async def test_handle_command_autofix_pr_unknown_platform_degrades_to_autodetect(server, fake_ws):
    """未知 platform 不报错，退化为 Agent 自检（不追加 Detected platform 行）。"""
    request = AgentRequest(
        request_id="req-autofix-weird",
        channel_id="tui",
        req_method=ReqMethod.COMMAND_AUTOFIX_PR,
        params={"platform": "bitbucket"},
    )
    ...
    msg = fake_ws.sent[0]
    assert msg["ok"] is True
    assert "Detected platform:" not in msg["payload"]["prompt"]
