...
class TestContextWindowPopAdaptsToCoreField:
    ...
    @staticmethod
    def test_core_has_context_window_field_true_on_current_core():
        # 本地 agent-core 已声明 context_window 正式字段。
        assert core_has_context_window_field() is True

    @staticmethod
    def test_agentos_context_window_kept_when_core_has_field():
    ...
    @staticmethod
    def test_agentos_context_window_in_model_request_config_when_core_has_field():
        entries = get_default_models(_config(agentos=[_agentos_block()]))
        agentos = next(e for e in entries
                       if e.get("model_config_obj", {}).get("_source") == "agentos")
...
class TestContextWindowPrecedence:
    ...
    def test_deep_agent_uses_global_before_model_request_window():
        ...
        assert deep_agent._resolve_context_window_tokens() == 131072
