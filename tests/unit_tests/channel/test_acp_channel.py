...
from jiuwenavatar.gateway.channel_manager.protocol.acp.acp_connect import AcpChannel, AcpChannelConfig
...
def _import_acp_channel_entry(monkeypatch: pytest.MonkeyPatch):
    import jiuwenavatar.gateway.channel_manager.protocol.acp.acp_connect as existing_module

    fake_config_module = types.ModuleType("jiuwenavatar.common.config")

    def get_default_config():
        return {}

    fake_config_module.get_config = get_default_config
    monkeypatch.setitem(sys.modules, "jiuwenavatar.common.config", fake_config_module)
...
def test_main_passes_explicit_agent_server_url(monkeypatch: pytest.MonkeyPatch):
    import jiuwenavatar.gateway.channel_manager.protocol.acp.acp_connect as module

    captured = {}
    original_stdout = sys.stdout

    def parse_args(_self):
        return Namespace(agent_server_url="ws://127.0.0.1:29001")
    ...
    assert captured["url"] == "ws://127.0.0.1:29001"
    assert captured["result"] == "ws://127.0.0.1:29001"
...
async def test_jsonrpc_initialize_and_session_new(monkeypatch):
    ...
    monkeypatch.setattr("jiuwenavatar.gateway.channel_manager.protocol.acp.acp_connect._ACP_STDOUT", fake_stdout)
    ...
    assert agent_info.get("name") == "jiuwenavatar"
...
async def test_jsonrpc_session_prompt_merges_session_context(monkeypatch):
    ...
    monkeypatch.setattr("sys.stdout", fake_stdout)
    monkeypatch.setattr("jiuwenavatar.gateway.channel_manager.protocol.acp.acp_connect._ACP_STDOUT", fake_stdout)
    monkeypatch.setattr("jiuwenavatar.gateway.channel_manager.protocol.acp.acp_connect._STDIN_EOF_GRACE_SECONDS", 0.01)
...
async def test_jsonrpc_session_prompt_does_not_end_turn_from_chat_final_before_late_tool_result(monkeypatch):
    ...
    monkeypatch.setattr("sys.stdout", fake_stdout)
    monkeypatch.setattr("jiuwenavatar.gateway.channel_manager.protocol.acp.acp_connect._ACP_STDOUT", fake_stdout)
    monkeypatch.setattr("jiuwenavatar.gateway.channel_manager.protocol.acp.acp_connect._PROMPT_IDLE_FINALIZE_SECONDS",
                        0.01)
    monkeypatch.setattr("jiuwenavatar.gateway.channel_manager.protocol.acp.acp_connect._STDIN_EOF_GRACE_SECONDS", 0.01)
...
async def test_jsonrpc_session_prompt_does_not_auto_finalize_from_delta_only(monkeypatch):
    ...
    monkeypatch.setattr("jiuwenavatar.gateway.channel_manager.protocol.acp.acp_connect._PROMPT_IDLE_FINALIZE_SECONDS",
                        0.01)
    monkeypatch.setattr("jiuwenavatar.gateway.channel_manager.protocol.acp.acp_connect._STDIN_EOF_GRACE_SECONDS", 0.05)
