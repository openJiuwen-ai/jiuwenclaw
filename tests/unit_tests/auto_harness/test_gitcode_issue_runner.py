...
@pytest.mark.asyncio
async def test_watch_once_creates_one_task_for_eligible_issue(tmp_path: Path):
    ...
    result = await runner.watch_once(IssueWatchOptions(owner="openJiuwen", repo="jiuwenswarm"))
...
@pytest.mark.asyncio
async def test_watch_once_skips_issue_with_existing_open_pr(tmp_path: Path):
    client = FakeGitCodeClient([_issue(2)], pulls={2: [{"number": 9, "state": "open"}]})
    service = FakeHarnessService()
    runner = GitCodeIssueRunner(
        client=client,
        state_store=IssueStateStore(tmp_path),
    ...
    assert result["skipped"][0]["reason"] == "linked pull request already exists"
    assert service.queries == []


@pytest.mark.asyncio
async def test_watch_once_dry_run_does_not_create_task(tmp_path: Path):
    ...
    result = await runner.watch_once(IssueWatchOptions(owner="openJiuwen", repo="jiuwenswarm", dry_run=True))
...
@pytest.mark.asyncio
async def test_watch_once_does_not_duplicate_existing_task(tmp_path: Path):
...
@pytest.mark.asyncio
async def test_watch_once_accepts_explicit_issue_numbers_without_required_label(tmp_path: Path):
    ...
    result = await runner.watch_once(
        IssueWatchOptions(
            owner="openJiuwen",
            repo="jiuwenswarm",
            issue_numbers=(10, 11),
...
@pytest.mark.asyncio
async def test_watch_once_staggers_multiple_started_tasks(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    sleeps: list[float] = []
...
@pytest.mark.asyncio
async def test_watch_once_skips_explicit_issue_with_existing_open_pr(tmp_path: Path):
    client = FakeGitCodeClient([_issue(12, labels=("bug",))], pulls={12: [{"number": 20, "state": "open"}]})
    service = FakeHarnessService()
    runner = GitCodeIssueRunner(
        client=client,
        state_store=IssueStateStore(tmp_path),
