...
def test_plan_calls_compose_rpc_once_without_language():
    """Dolores: plan() should not pass language to compose RPC."""
    registry = _registry()
    calls = []
...
def test_plan_preserves_progress_callback_on_extension_request():
    """Dolores: progress callback should be forwarded via extension request."""
    registry = _registry()
    events = []
...
def test_plan_reports_missing_rpc_handler():
    """Dolores: plan() should report when RPC handler is not registered."""
    _registry()
...
def test_get_tools_describes_fast_and_beam_without_language():
    """Dolores: compose tool should describe fast/beam modes without language param."""
    compose_tool = next(
        tool
