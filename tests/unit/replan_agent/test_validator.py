...
REPLAN_SKILL_CODE_PREFIXES = ["jiuwenclaw.agentserver.replan_agent.skill_codes."]
...
class TestGeneratedSkillCodeValidator:
    ...
    def test_allow_plan_node_import(self, validator: PlanCodeValidator) -> None:
        code = "from jiuwenclaw.agentserver.replan_agent.plan_node import PlanNode\n"
        assert validator.validate(code) == []
...
class TestBuiltinSkillCodeValidator:
    @pytest.fixture
    def validator(self) -> PlanCodeValidator:
        return PlanCodeValidator.for_builtin_skill_code(REPLAN_SKILL_CODE_PREFIXES)
