...
This module MUST be imported BEFORE any other jiuwenavatar modules,
because it sets JIUWENAVATAR_DATA_DIR environment variable that affects
path resolution in jiuwenavatar.utils.

Usage in entry point files:
    from jiuwenavatar.dotenv_early import parse_dotenv_early
    parse_dotenv_early()

    # Now safe to import other jiuwenavatar modules
    from jiuwenavatar.common.utils import ...

The parsing happens before any jiuwenavatar imports:
...
- JIUWENAVATAR_DATA_DIR is injected into os.environ
...
_early_logger = logging.getLogger("jiuwenavatar.early")
...
def parse_dotenv_early(component_name: str = "jiuwenavatar") -> Path | None:
    """Parse --dotenv/--name arguments and load env before jiuwenavatar imports.
    ...
    - But JIUWENAVATAR_DATA_DIR is set BEFORE module-level code executes
    ...
    Args:
        component_name: Name for warning messages (e.g., "jiuwenavatar-app")
    ...
    Usage:
        from jiuwenavatar.dotenv_early import parse_dotenv_early
        parse_dotenv_early("jiuwenavatar-app")
...
def _load_bootstrap_by_name_early(name: str, component_name: str) -> Path | None:
    ...
    This is called before any jiuwenavatar imports, so it needs to:
    ...
    user_home = os.environ.get("JIUWENAVATAR_HOME") or Path.home()
    yaml_path = Path(user_home) / ".jiuwenavatar" / "instances.yaml"

    if not yaml_path.exists():
        _early_error(component_name, f"instances.yaml not found: {yaml_path}")
        _early_error(component_name, f"Run 'jiuwenavatar-init --name {name}' to create it.")
        return None
    ...
    try:
        ...
        if workspace_str:
            ...
            instances_dir = Path(user_home) / ".jiuwenavatar-instances"
    ...
    if bootstrap_env.exists():
        ...
        from jiuwenavatar.instance_manager.bootstrap import _create_basic_bootstrap_env
...
_component_name: str = "jiuwenavatar"
...
def set_component_name(name: str) -> None:
    ...
    Call this before importing the module if you want custom warnings:
        from jiuwenavatar import dotenv_early
        dotenv_early.set_component_name("jiuwenavatar-app")
...
def load_instance_bootstrap_by_name(name: str) -> Path | None:
    ...
    jiuwenavatar.instance_manager.bootstrap.load_instance_bootstrap_by_name.

    NOTE: This function is deprecated. Use the function from
    jiuwenavatar.instance_manager.bootstrap directly for new code.
    ...
    Returns:
        Path to loaded .env if successful, None otherwise
    """
    from jiuwenavatar.instance_manager.bootstrap import (
