...
from jiuwenavatar.gateway.channel_manager.base import RobotMessageRouter, BaseChannel
from jiuwenavatar.gateway.channel_manager.im_platforms.platform_adapter.message import MessageStore
from jiuwenavatar.common.schema.message import Message, ReqMethod, EventType
from jiuwenavatar.gateway.channel_manager.im_platforms.feishu.feishu_file_service import (
    FeishuFileService,
    is_image_file,
    is_audio_file,
    is_video_file,
...
class FeishuChannel(BaseChannel):
    ...
    async def _handle_message(
        ...
        Args:
            inbound: Feishu 入站消息
        """
        from jiuwenavatar.gateway.routing.interaction_context import PendingInteraction
    ...
    def _is_control_message(content: str) -> bool:
        from jiuwenavatar.gateway.message_handler.command_parser.slash_command import is_control_like_for_im_batching
    ...
    def _detect_workspace_files(self, text: str) -> list[str]:
        ...
        支持两种模式：
        1. 完整绝对路径：/home/xxx/.jiuwenavatar/agent/workspace/xxx.docx
        ...
        # 支持 Linux (/home/xxx/.jiuwenavatar/...) 和 Windows (C:\Users\xxx\.jiuwenavatar\...)
    ...
    async def _on_message(self, data: "P2ImMessageReceiveV1") -> None:
        ...
        try:
            ...
            if self.channel_id == self.name:
                try:
                    from jiuwenavatar.common.config import update_channel_in_config
                ...
                try:
                    from jiuwenavatar.common.config import update_channel_subsection_in_config
