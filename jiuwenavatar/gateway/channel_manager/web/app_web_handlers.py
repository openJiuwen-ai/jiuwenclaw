...
def _decrypt_sensitive_config_value(key: str, value: Any) -> str:
    text = "" if value is None else str(value).strip()
    if not text or not ("api_key" in key.lower() or "token" in key.lower()):
        return text
    try:
        ...
        if crypto_provider:
            decrypted = crypto_provider.decrypt(text)
            return str(decrypted).strip() if decrypted is not None else ""
    except Exception:  # noqa: BLE001
        return ""
    return text


def _read_platform_env_value(env_key: str) -> str:
    """读取平台 .env 中的真实配置值；用于区分平台配置与宿主环境变量。"""
    try:
        raw = dotenv_values(_ENV_FILE).get(env_key)
    except Exception:  # noqa: BLE001
        raw = None
    if raw is None:
        return ""
    return _decrypt_sensitive_config_value(env_key, raw)


def _sync_platform_env_to_process(env_keys: tuple[str, ...]) -> None:
    for env_key in env_keys:
        os.environ[env_key] = _read_platform_env_value(env_key)
...
_FORWARD_NO_LOCAL_HANDLER_METHODS = frozenset({
    "initialize",
    # "session.create" 移除：需要本地 handler 执行 init_session_metadata 写入 avatar_id
    "session.switch",
    "acp.tool_response",
    "team.delete",
    "browser.start",
...
def _register_web_handlers(bind: WebHandlersBindParams) -> None:
    ...
    def _apply_config_payload(params: dict[str, Any]) -> tuple[dict[str, str], list[str]]:
        ...
        if "GITCODE_TOKEN" in env_updates:
            try:
                from jiuwenavatar.common.gitcode_config import sync_gitcode_token_to_skill_config

                sync_gitcode_token_to_skill_config(env_updates["GITCODE_TOKEN"])
            except Exception as exc:  # noqa: BLE001
                logger.warning("[config.set] sync gitcode-repo.json failed: %s", exc)
        if yaml_updated:
    ...
    def _build_models_defaults_from_frontend(raw_models: Any) -> list[dict[str, Any]]:
        ...
        from jiuwenavatar.common.config import _infer_is_default
    ...
    async def _heartbeat_get_path(ws, req_id, params, session_id):
        ...
        from jiuwenavatar.common.utils import get_deepagent_heartbeat_path, get_agent_root_dir
    ...
    async def _channel_wechat_get_login_ui(ws, req_id, params, session_id):
        from jiuwenavatar.gateway.channel_manager.im_platforms.wechat.wechat_connect import (
    ...
    # 数字分身 — Persona / Avatar / Trigger / Report（Web 网关直连）
    async def _personas_list(ws, req_id, params, session_id):
        ...
        try:
            ...
            logger.exception("[personas.list] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _personas_get(ws, req_id, params, session_id):
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            persona_id = (params or {}).get("persona_id", "")
            manager = PersonaManager.get_instance()
            result = await manager.handle_personas_get(persona_id=persona_id)
            ok = "error" not in result
            await channel.send_response(ws, req_id, ok=ok, payload=result, error=result.get("error") if not ok else None)
        except Exception as e:
            logger.exception("[personas.get] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _personas_create(ws, req_id, params, session_id):
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            manager = PersonaManager.get_instance()
            result = await manager.handle_personas_create(**(params or {}))
            ok = "error" not in result
            await channel.send_response(ws, req_id, ok=ok, payload=result, error=result.get("error") if not ok else None)
        except Exception as e:
            logger.exception("[personas.create] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _personas_update(ws, req_id, params, session_id):
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            payload = dict(params or {})
            persona_id = payload.pop("persona_id", "")
            manager = PersonaManager.get_instance()
            result = await manager.handle_personas_update(persona_id=persona_id, **payload)
            ok = "error" not in result
            await channel.send_response(ws, req_id, ok=ok, payload=result, error=result.get("error") if not ok else None)
        except Exception as e:
            logger.exception("[personas.update] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _personas_delete(ws, req_id, params, session_id):
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            persona_id = (params or {}).get("persona_id", "")
            cascade_avatars = bool((params or {}).get("cascade_avatars"))
            manager = PersonaManager.get_instance()
            result = await manager.handle_personas_delete(
                persona_id=persona_id,
                cascade_avatars=cascade_avatars,
            )
            ok = "error" not in result
            await channel.send_response(ws, req_id, ok=ok, payload=result, error=result.get("error") if not ok else None)
        except Exception as e:
            logger.exception("[personas.delete] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _personas_duplicate(ws, req_id, params, session_id):
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            manager = PersonaManager.get_instance()
            result = await manager.handle_personas_duplicate(**(params or {}))
            ok = "error" not in result
            await channel.send_response(ws, req_id, ok=ok, payload=result, error=result.get("error") if not ok else None)
        except Exception as e:
            logger.exception("[personas.duplicate] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _personas_generate(ws, req_id, params, session_id):
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            manager = PersonaManager.get_instance()
            result = await manager.handle_personas_generate(**(params or {}))
            ok = "error" not in result
            await channel.send_response(ws, req_id, ok=ok, payload=result, error=result.get("error") if not ok else None)
        except Exception as e:
            logger.exception("[personas.generate] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _coding_engines_status(ws, req_id, params, session_id):
        from jiuwenavatar.server.runtime.coding import list_coding_engine_selectability
    ...
    async def _coding_cli_retry_install(ws, req_id, params, session_id):
        """重试 CLI 安装（用户手动触发）."""
        from jiuwenavatar.server.runtime.coding.bootstrap import start_cli_install_background
        
        try:
            engine_kind = (params or {}).get("engine_kind", "")
            if not engine_kind:
                await channel.send_response(ws, req_id, ok=False, error="engine_kind is required", code="INVALID_PARAMS")
                return
            
            detail = start_cli_install_background(engine_kind)
            await channel.send_response(ws, req_id, ok=True, payload={"detail": detail})
        except Exception as e:
            logger.exception("[coding.cli.retry_install] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _avatars_list(ws, req_id, params, session_id):
        from jiuwenavatar.server.runtime.persona import PersonaManager
    ...
    async def _avatars_get(ws, req_id, params, session_id):
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            avatar_id = (params or {}).get("avatar_id", "")
            manager = PersonaManager.get_instance()
            result = await manager.handle_avatars_get(avatar_id=avatar_id)
            ok = "error" not in result
            await channel.send_response(ws, req_id, ok=ok, payload=result, error=result.get("error") if not ok else None)
        except Exception as e:
            logger.exception("[avatars.get] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _avatars_create(ws, req_id, params, session_id):
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            _sync_platform_env_to_process(("ANTHROPIC_API_KEY", "ANTHROPIC_BASE_URL", "ANTHROPIC_MODEL", "OPENAI_API_KEY"))
            manager = PersonaManager.get_instance()
            result = await manager.handle_avatars_create(**(params or {}))
            ok = "error" not in result
            await channel.send_response(ws, req_id, ok=ok, payload=result, error=result.get("error") if not ok else None)
        except Exception as e:
            logger.exception("[avatars.create] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _avatars_update(ws, req_id, params, session_id):
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            _sync_platform_env_to_process(("ANTHROPIC_API_KEY", "ANTHROPIC_BASE_URL", "ANTHROPIC_MODEL", "OPENAI_API_KEY"))
            payload = dict(params or {})
            avatar_id = payload.pop("avatar_id", "")
            manager = PersonaManager.get_instance()
            result = await manager.handle_avatars_update(avatar_id=avatar_id, **payload)
            ok = "error" not in result
            await channel.send_response(ws, req_id, ok=ok, payload=result, error=result.get("error") if not ok else None)
        except Exception as e:
            logger.exception("[avatars.update] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _avatars_delete(ws, req_id, params, session_id):
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            avatar_id = (params or {}).get("avatar_id", "")
            manager = PersonaManager.get_instance()
            result = await manager.handle_avatars_delete(avatar_id=avatar_id)
            ok = "error" not in result
            await channel.send_response(ws, req_id, ok=ok, payload=result, error=result.get("error") if not ok else None)
        except Exception as e:
            logger.exception("[avatars.delete] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _triggers_list(ws, req_id, params, session_id):
        from jiuwenavatar.gateway.trigger import TriggerEngine

        try:
            engine = TriggerEngine.get_instance()
            result = await engine.handle_triggers_list(**(params or {}))
            await channel.send_response(ws, req_id, ok=True, payload=result)
        except Exception as e:
            logger.exception("[triggers.list] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _triggers_get(ws, req_id, params, session_id):
        from jiuwenavatar.gateway.trigger import TriggerEngine
        ...
        try:
            ...
            logger.exception("[triggers.get] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _triggers_create(ws, req_id, params, session_id):
        from jiuwenavatar.gateway.trigger import TriggerEngine

        try:
            engine = TriggerEngine.get_instance()
            result = await engine.handle_triggers_create(**(params or {}))
            ok = "error" not in result
            await channel.send_response(ws, req_id, ok=ok, payload=result, error=result.get("error") if not ok else None)
        except Exception as e:
            logger.exception("[triggers.create] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _triggers_update(ws, req_id, params, session_id):
        from jiuwenavatar.gateway.trigger import TriggerEngine

        try:
            payload = dict(params or {})
            trigger_id = payload.pop("trigger_id", "")
            engine = TriggerEngine.get_instance()
            result = await engine.handle_triggers_update(trigger_id=trigger_id, **payload)
            ok = "error" not in result
            await channel.send_response(ws, req_id, ok=ok, payload=result, error=result.get("error") if not ok else None)
        except Exception as e:
            logger.exception("[triggers.update] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _triggers_delete(ws, req_id, params, session_id):
        from jiuwenavatar.gateway.trigger import TriggerEngine
        ...
        try:
            ...
            logger.exception("[triggers.delete] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")
    ...
    async def _missions_list(ws, req_id, params, session_id):
        ...
        try:
            ...
            logger.exception("[missions.list] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")
    ...
    async def _missions_get(ws, req_id, params, session_id):
        ...
        try:
            ...
            logger.exception("[missions.get] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    async def _missions_cancel(ws, req_id, params, session_id):
        from jiuwenavatar.gateway.trigger import TriggerEngine

        try:
            mission_id = (params or {}).get("mission_id", "")
            engine = TriggerEngine.get_instance()
            result = await engine.cancel_mission(mission_id)
            ok = "error" not in result
            await channel.send_response(ws, req_id, ok=ok, payload=result, error=result.get("error") if not ok else None)
        except Exception as e:
            logger.exception("[missions.cancel] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")
    ...
    async def _missions_delete(ws, req_id, params, session_id):
        ...
        try:
            ...
            logger.exception("[missions.delete] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")
    ...
    async def _reports_list(ws, req_id, params, session_id):
        ...
        try:
            ...
            logger.exception("[reports.list] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")
    ...
    async def _reports_get(ws, req_id, params, session_id):
        ...
        try:
            ...
            logger.exception("[reports.get] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")
    ...
    async def _report_read_state_get(ws, req_id, params, session_id):
        ...
        try:
            ...
            logger.exception("[report_read_state.get] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")
    ...
    async def _report_read_state_set(ws, req_id, params, session_id):
        ...
        try:
            ...
            logger.exception("[report_read_state.set] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")
    ...
    async def _report_unread_counts(ws, req_id, params, session_id):
        ...
        try:
            ...
            logger.exception("[report.unread_counts] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")
    ...
    async def _missions_stats(ws, req_id, params, session_id):
        ...
        try:
            ...
            logger.exception("[missions.stats] %s", e)
            await channel.send_response(ws, req_id, ok=False, error=str(e), code="INTERNAL_ERROR")

    channel.register_method("personas.list", _personas_list)
    channel.register_method("personas.get", _personas_get)
    channel.register_method("personas.create", _personas_create)
    channel.register_method("personas.update", _personas_update)
    channel.register_method("personas.delete", _personas_delete)
    channel.register_method("personas.duplicate", _personas_duplicate)
    channel.register_method("personas.generate", _personas_generate)
    channel.register_method("coding.engines.status", _coding_engines_status)
    channel.register_method("coding.cli.retry_install", _coding_cli_retry_install)
    channel.register_method("avatars.list", _avatars_list)
    channel.register_method("avatars.get", _avatars_get)
    channel.register_method("avatars.create", _avatars_create)
    channel.register_method("avatars.update", _avatars_update)
    channel.register_method("avatars.delete", _avatars_delete)
    channel.register_method("triggers.list", _triggers_list)
    channel.register_method("triggers.get", _triggers_get)
    channel.register_method("triggers.create", _triggers_create)
    channel.register_method("triggers.update", _triggers_update)
    channel.register_method("triggers.delete", _triggers_delete)
    channel.register_method("missions.list", _missions_list)
    channel.register_method("missions.get", _missions_get)
    channel.register_method("missions.cancel", _missions_cancel)
    channel.register_method("missions.delete", _missions_delete)
    channel.register_method("missions.stats", _missions_stats)
    channel.register_method("reports.list", _reports_list)
    channel.register_method("reports.get", _reports_get)
    channel.register_method("report_read_state.get", _report_read_state_get)
    channel.register_method("report_read_state.set", _report_read_state_set)
    channel.register_method("report.unread_counts", _report_unread_counts)
    ...
    async def _permissions_owner_scopes_get(ws, req_id, params, session_id):
        from jiuwenavatar.common.config import get_permissions_owner_scopes
    ...
    async def _permissions_owner_scopes_set(ws, req_id, params, session_id):
        from jiuwenavatar.common.config import update_permissions_owner_scopes_in_config
    ...
    from jiuwenavatar.common.schema.message import ReqMethod as _PermReq
    ...
    async def _memory_forbidden_set(ws, req_id, params, session_id):
        from jiuwenavatar.common.config import update_memory_forbidden_in_config
        if not isinstance(params, dict):
            await channel.send_response(ws, req_id, ok=False, error="params must be object", code="BAD_REQUEST")
            return
        try:
    ...
    async def _forward_harness_to_agent(ws, req_id, params, session_id, *, req_method):
        ...
        if ac is None or not getattr(ac, "server_ready", False):
            ...
            from jiuwenavatar.agents.harness.common.auto_harness import (
    ...
    from jiuwenavatar.common.schema.message import ReqMethod as _HarnessReq
