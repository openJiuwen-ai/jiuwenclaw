...
from jiuwenavatar.extensions.hook_event import GatewayHookEvents
from jiuwenavatar.extensions.hooks_context import GatewayChatHookContext
from jiuwenavatar.common.hooks_config import load_hooks_config
from jiuwenavatar.gateway.hooks.handler import GatewayHookHandler
...
_KNOWN_JIUWENAVATAR_SESSION_PREFIXES = (
    "sess_",
    "tui_",
    "acp_",
    "cron_",
...
def apply_a2ui_text_fallback_to_gateway_payload(
    ...
    *,
    ...
    from jiuwenavatar.server.runtime.a2ui.integration import apply_non_web_text_fallback_to_payload
...
class MessageHandler(ABC):
    ...
    （``jiuwenavatar.e2a.wire_codec``）解析并还原为 ``AgentResponse`` / ``AgentResponseChunk``，
    ...
    def __init__(self, agent_client: "AgentServerClient") -> None:
        ...
        self._stream_result_text: dict[str, str] = {}  # request_id -> accumulated assistant output
        self._stream_mission_ids: dict[str, str] = {}  # request_id -> report mission id
    ...
    def set_outbound_pipeline(self, pipeline: Any) -> None:
        ...
        # 直接使用 jiuwenavatar.config 的 get_config_raw/set_config/update_channel_in_config
        # 避免在此处重复实现 config 模块加载逻辑。
        from jiuwenavatar.common.config import get_config_raw, update_channel_in_config
    ...
    @staticmethod
    def _is_chat_send_envelope(env: "E2AEnvelope") -> bool:
    ...
    def _pop_stream_tracking(self, rid: str) -> None:
        ...
        self._stream_result_text.pop(rid, None)
        self._stream_mission_ids.pop(rid, None)

    @staticmethod
    def _extract_agent_payload_text(payload: Any) -> str:
        """Best-effort clean text from an agent stream/response payload."""
        from jiuwenavatar.gateway.trigger.engine import TriggerEngine

        if isinstance(payload, dict):
            content = payload.get("content")
            if isinstance(content, dict):
                text = TriggerEngine._extract_text(content)
                if text:
                    return text
        return TriggerEngine._extract_text(payload)

    def _accumulate_stream_result(self, rid: str, chunk: "AgentResponseChunk") -> None:
        """Collect assistant output from stream chunks for mission result_summary."""
        payload = chunk.payload
        if not isinstance(payload, dict):
            return
        text = self._extract_agent_payload_text(payload)
        if not text:
            return
        event_type = str(payload.get("event_type") or "")
        if event_type == "chat.final":
            self._stream_result_text[rid] = text
        elif event_type == "chat.delta":
            self._stream_result_text[rid] = self._stream_result_text.get(rid, "") + text

    def _ensure_avatar_mission_started(
        self,
        env: "E2AEnvelope",
        session_id: str | None,
        rid: str,
        *,
        request_avatar_id: str = "",
    ) -> None:
        """Create a running Mission when an avatar manual chat stream starts."""
        avatar_id = request_avatar_id
        if not avatar_id and isinstance(env.params, dict):
            avatar_id = str(env.params.get("avatar_id") or "").strip()
        if not avatar_id and session_id:
            from jiuwenavatar.server.runtime.session.session_metadata import get_session_metadata

            avatar_id = str(get_session_metadata(session_id, cache_bust=True).get("avatar_id") or "").strip()
        if not avatar_id:
            return
        ...
        if isinstance(env.params, dict):
            prompt = str(
                env.params.get("content") or env.params.get("query") or ""
            ).strip()
        if not prompt:
            prompt = self._get_session_last_user_query(session_id)
        if not prompt:
            prompt = "手动聊天"
    ...
    async def _rewind_slash_notice(
        ...
        *,
        ...
        from jiuwenavatar.agents.harness.common.session_ops_service import rewind_session
        """受控通道 /rewind N：回退当前会话到指定轮次并通知。
    ...
    @classmethod
    def _is_known_jiuwenavatar_session_id(cls, session_id: str | None) -> bool:
        sid = str(session_id or "").strip()
        if not sid:
            return False
        if sid.startswith(_KNOWN_JIUWENAVATAR_SESSION_PREFIXES):
            return True
    ...
    async def _resolve_acp_internal_session_id(
        ...
        async with self._acp_session_alias_lock:
            ...
            desired = (
                external
                if self._is_known_jiuwenavatar_session_id(external)
    ...
    def message_to_e2a(msg: "Message") -> "E2AEnvelope":
        from jiuwenavatar.common.e2a.gateway_normalize import message_to_e2a_or_fallback
    ...
    async def _handle_agent_server_push(self, wire: dict[str, Any]) -> None:
        ...
        from jiuwenavatar.common.e2a.wire_codec import parse_agent_server_wire_chunk
        ...
        *,
        ...
        if is_chat_send:
            self._ensure_avatar_mission_started(
                ...
                session_id,
                rid,
                request_avatar_id=request_avatar_id,
            )
        try:
            async for chunk in self._agent_client.send_request_stream(env):
                ...
                if self._is_terminal_stream_chunk(chunk):
                    ...
                    continue
                self._accumulate_stream_result(rid, chunk)
            ...
            result_text = self._stream_result_text.pop(rid, "")
            stream_mission_id = self._stream_mission_ids.pop(rid, None)
            # 清理状态
            self._pop_stream_tracking(rid)
            ...
            if is_chat_send:
                await self._finalize_avatar_mission(
                    session_id,
                    cancelled=cancelled,
                    request_avatar_id=request_avatar_id,
                    result_text=result_text,
                    mission_id=stream_mission_id,
                    run_id=rid,
                )

    async def _finalize_avatar_mission(
        self,
        session_id: str | None,
        *,
        ...
        """为 avatar 会话创建/更新 Mission 记录。
        
        这使得手动聊天任务也能在「报告」页显示，并触发浮标通知。
        
        Args:
            session_id: 会话 ID
            cancelled: 任务是否被取消
            request_avatar_id: 请求参数中的 avatar_id（用于解决跨进程异步写入延迟问题）
            result_text: 流式响应中累积的助手输出
            mission_id: 流开始时创建的 Mission ID（若有）
            run_id: 本次 chat 请求的 request_id
        """
        if not session_id:
            return
        ...
        try:
            ...
            # 如果 metadata 中没有 avatar_id，但请求参数中有，则使用请求参数中的值
            # 这解决了 AgentServer 异步写入延迟导致的跨进程同步问题
            if not avatar_id and request_avatar_id:
                avatar_id = request_avatar_id
                # 同时补写到 metadata，确保后续读取一致
                update_session_metadata(session_id=session_id, avatar_id=avatar_id)
                logger.info(
                    "[MessageHandler] avatar_id 从请求参数补写到 metadata: session_id=%s avatar_id=%s",
                    session_id, avatar_id,
                )
            
            if not avatar_id:
                logger.debug(
                    "[MessageHandler] 非 avatar 会话，跳过 Mission 创建: session_id=%s",
                    session_id,
                )
                return  # 非 avatar 会话，不创建 Mission
            ...
            if mission_id:
                ...
                logger.info(
                    "[MessageHandler] Avatar mission updated: session_id=%s mission_id=%s status=%s summary_len=%d",
                    session_id, mission_id, status.value, len(summary),
                )
            else:
                # 创建新 Mission（手动聊天任务，流开始时未成功建 Mission 时的兜底）
            ...
            logger.warning("[MessageHandler] Failed to finalize avatar mission: %s", exc)
    ...
    async def stop_forwarding(self) -> None:
        ...
        self._stream_result_text.clear()
        self._stream_mission_ids.clear()
