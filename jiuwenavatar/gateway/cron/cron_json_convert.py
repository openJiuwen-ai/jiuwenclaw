...
from jiuwenavatar.gateway.cron.cron_expr import iso_to_seven_field_cron
...
def convert_cron_job_dict_to_flat(data: dict[str, Any]) -> dict[str, Any]:
    ...
    # 你的偏好：不管 OpenClaw 里 wakeMode 是什么，转换到 jiuwenavatar 时都统一写成 60s
    wake_offset_seconds = 300
...
def convert_flat_cron_job_to_openclaw_dict(flat_job: dict[str, Any]) -> dict[str, Any]:
    ...
    # 你的偏好：不管 jiuwenavatar 里 wake_offset_seconds 是多少，转换到 OpenClaw 时都统一写成 now
    wake_mode = "now"
...
def convert_file(input_path: Path, output_path: Path, to_format: str) -> None:
    ...
    for job in jobs_in:
        if not isinstance(job, dict):
            continue
        if to_format in ("jiuwenavatar", "jiuwenclaw"):
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--to",
        choices=["jiuwenavatar", "jiuwenclaw", "openclaw"],
        required=True,
        help="Target format: jiuwenavatar/jiuwenclaw (internal) or openclaw (nested)",
    )
