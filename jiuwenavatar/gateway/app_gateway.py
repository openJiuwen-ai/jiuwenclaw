...
parse_dotenv_early("jiuwenavatar-gateway")

# --- Now safe to import jiuwenavatar modules ---
from jiuwenavatar.gateway.channel_manager.protocol.acp.acp_connect import AcpGatewayBridge
from jiuwenavatar.gateway.routing.route_binding import GatewayRouteBinding
...
logger = logging.getLogger("jiuwenavatar.gateway")
...
def _normalize_gateway_message(msg):
    from jiuwenavatar.common.schema.message import Message, ReqMethod
...
class GatewayServer:
    ...
    async def _handle_raw_message(self, ws: Any, raw: str, request_path: str, route: RouteConfig) -> None:
        from jiuwenavatar.common.schema.message import Message, Mode, ReqMethod
...
async def _run(
    ...
    from jiuwenavatar.gateway.channel_manager.protocol.a2a.a2a_connect import A2AChannel, A2AChannelConfig
    from jiuwenavatar.gateway.channel_manager.im_platforms.dingtalk.dingtalk_connect import DingTalkChannel, \
        DingTalkConfig
    from jiuwenavatar.gateway.channel_manager.im_platforms.feishu.feishu_connect import FeishuChannel, FeishuConfig
    from jiuwenavatar.gateway.channel_manager.im_platforms.whatsapp.whatsapp_connect import WhatsAppChannel, \
        WhatsAppChannelConfig
    from jiuwenavatar.gateway.channel_manager.im_platforms.wechat.wechat_connect import WechatChannel, WechatConfig
    from jiuwenavatar.gateway.channel_manager.web.web_connect import WebChannel, WebChannelConfig
    from jiuwenavatar.gateway.channel_manager.im_platforms.xiaoyi.xiaoyi_connect import (
        XiaoyiChannel, XiaoyiChannelConfig,
    )
    from jiuwenavatar.gateway.channel_manager.im_platforms.telegram.telegram_connect import TelegramChannel, \
        TelegramChannelConfig
    from jiuwenavatar.gateway.channel_manager.im_platforms.discord.discord_connect import DiscordChannel, \
    ...
    from jiuwenavatar.gateway.channel_manager.web.app_web_handlers import (
    ...
    from jiuwenavatar.gateway.channel_manager.tui.tui_connect import (
        CliRouteBindParams,
    ...
    from jiuwenavatar.common.schema.message import Message
    from jiuwenavatar.common.updater import UpdaterService
    ...
    extension_registry, extensions_created = ExtensionRegistry.get_or_create_instance(
    ...
    if extensions_created:
        ...
        logger.info(
            "[App] reusing ExtensionRegistry from AgentServer (%d extensions)",
    ...
    from jiuwenavatar.gateway.im_pipeline.im_inbound import IMInboundPipeline
    from jiuwenavatar.gateway.im_pipeline.im_outbound import IMOutboundPipeline
    ...
    # Trigger 引擎：装配派发依赖（把触发转成一次 chat.send 真正驱动数字人），
    # 并恢复历史触发器（重启后已存在的 cron/heartbeat 触发器需重新拉起调度）。
    from jiuwenavatar.gateway.trigger import TriggerEngine
    trigger_engine = TriggerEngine.get_instance()
    trigger_engine.configure_dispatch(agent_client=client, message_handler=message_handler)
    await trigger_engine.start_all()
    # 监听 triggers.json 变化：AgentServer 进程只做 CRUD 落盘，调度统一在 Gateway，
    # 通过文件监听感知其它进程的增删改并起停调度，避免双重调度且保证可派发。
    await trigger_engine.start_watching(
        interval=float(os.getenv("TRIGGER_STORE_WATCH_INTERVAL", "5")),
    )
        ...
        def _on_a2a_task_done(task: asyncio.Task) -> None:
            try:
                ...
                logger.error(
                    ...
                    "`uv sync --extra a2a` or `pip install \"jiuwenavatar[a2a]\"`.",
                    exc,
                )
            ...
            if isinstance(feishu_conf, dict):
                ...
                if not enabled:
                    ...
                    if feishu_config.group_digital_avatar:
                        from jiuwenavatar.gateway.channel_manager.im_platforms.feishu.feishu_im_adapter import \
            ...
            if isinstance(wecom_conf, dict):
                ...
                if not enabled:
                    ...
                    if wecom_config.group_digital_avatar:
                        from jiuwenavatar.gateway.channel_manager.im_platforms.wecom.wecom_im_adapter import \
...
def main() -> None:
    from jiuwenavatar.dotenv_early import get_parsed_dotenv

    parser = argparse.ArgumentParser(
        prog="jiuwenavatar-gateway",
        description="Start JiuwenClaw Gateway + Channels (split deployment; connects to jiuwenavatar-agentserver).",
    )
    parser.add_argument(
        "--agent-server-url",
        "-u",
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="PORT",
        help=f"WebChannel bind port (default: WEB_PORT or {DEFAULT_WEB_PORT}).",
    )
    parser.add_argument(
        "--web-path",
        default=None,
    ...
    default_port = os.getenv("AGENT_SERVER_PORT") or os.getenv("AGENT_PORT", str(DEFAULT_AGENT_SERVER_PORT))
    agent_server_url = (
    ...
    web_port = args.port or int(os.getenv("WEB_PORT", str(DEFAULT_WEB_PORT)))
