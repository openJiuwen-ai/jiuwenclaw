...
from jiuwenavatar.common.e2a.constants import E2A_WIRE_SERVER_PUSH_KEY
...
class AgentServerClient(ABC):
    ...
    async def send_request(
        ...
        timeout 为 None 时使用默认非流式超时；长任务（如触发器驱动的代码检视）
        可显式传入更大的值，避免对端仍在执行时被过早判超时。
        """
        ...
...
class WebSocketAgentServerClient(AgentServerClient):
        ...
        try:
            while self._running and self._ws is not None:
                try:
                    ...
                    # 连接被对端关闭（如 AgentServer 重启，1001 going away）。recv() 会持续
                    # 抛此异常，必须退出循环等待上层重连，否则会以 ~100ms 频率刷错误日志。
                    logger.info(
                        "[WebSocketAgentServerClient] 连接已关闭，停止接收循环: %s", e
                    )
                    self._server_ready = False
                    break
                except Exception as e:
    ...
    async def send_request(
        ...
        unary_timeout = timeout if timeout is not None else _UNARY_REQUEST_TIMEOUT_SECONDS
        ...
        try:
            ...
            try:
                data = await asyncio.wait_for(queue.get(), timeout=unary_timeout)
                ...
                raise RuntimeError(
                    f"AgentServer 非流式请求超时 (request_id={rid}, timeout={unary_timeout}s)"
                ) from e
