...
Runs ``jiuwenavatar.server.app_agentserver`` then ``jiuwenavatar.gateway.app_gateway`` with the same
...
from jiuwenavatar.dotenv_early import parse_dotenv_early, get_parsed_dotenv
...
def main() -> None:
    ...
    if getattr(sys, "frozen", False):
        ...
        agent_cmd = [python, "-m", "jiuwenavatar.server.app_agentserver"]
        gateway_cmd = [python, "-m", "jiuwenavatar.gateway.app_gateway"]
    ...
    if "JIUWENAVATAR_START_CMD" not in os.environ:
        try:
            os.environ["JIUWENAVATAR_START_CMD"] = json.dumps(sys.argv[:])
        except (TypeError, ValueError, OverflowError):
            os.environ["JIUWENAVATAR_START_CMD"] = json.dumps([str(a) for a in sys.argv[:]])
