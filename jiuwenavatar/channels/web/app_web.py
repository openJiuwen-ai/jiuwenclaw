...
def _get_package_dir() -> Path:
    """Get the jiuwenavatar/channels/web package directory."""
    # app_web.py is at jiuwenavatar/channels/web/app_web.py
    # So parent is jiuwenavatar/channels/web/
...
class _SpaStaticHandler(SimpleHTTPRequestHandler):
    ...
    _SPA_CACHE_CONTROL = "no-cache, no-store, must-revalidate"
    _emit_spa_cache_control = False

    def end_headers(self) -> None:
        if self._emit_spa_cache_control:
            self.send_header("Cache-Control", self._SPA_CACHE_CONTROL)
    ...
    def _build_share_snapshot(
        ...
        *,
        ...
        filename = f"jiuwenavatar-share-{timestamp}.png"
        snapshot = {
    ...
    def _handle_file_api_get(self, parsed) -> None:
        ...
        if path == "/file-api/session-media":
            self._handle_session_media(query)
            return
    ...
    def _handle_session_media(self, query: dict[str, str]) -> None:
        """Serve session media files (images uploaded by users).

        URL: /file-api/session-media?session=<session_id>&file=<relative_path>
        """
        session_id = query.get("session", "")
        file_rel = query.get("file", "")
        if not session_id or not file_rel:
            self._write_json(400, {"error": "missing_session_or_file"})
            return

        from jiuwenavatar.common.utils import get_agent_sessions_dir

        sessions_dir = get_agent_sessions_dir()
        full_path = (sessions_dir / session_id / file_rel).resolve()

        # 安全检查：确保在 sessions 目录下
        try:
            if os.path.commonpath([str(sessions_dir.resolve()), str(full_path)]) != str(sessions_dir.resolve()):
    ...
    def _handle_file_api_post(self, parsed) -> None:
        ...
        if parsed.path == "/file-api/ws-debug-config":
            ...
            self.logger.info(
                "[jiuwenavatar-web] ws disable compress updated: %s",
                ws_disable_compress,
            )
    ...
    def send_head(self):
        self._emit_spa_cache_control = False
        parsed = urlparse(self.path)
        ...
        self._emit_spa_cache_control = True
...
def _build_frontend_server(
    ...
    *,
...
) -> tuple[ThreadingHTTPServer, logging.Logger]:
    """Build a configured ThreadingHTTPServer for the frontend static + proxy routes."""
    resolved_dist = (dist_dir or _default_dist_dir()).expanduser().resolve()
    if not resolved_dist.exists():
        raise FileNotFoundError(f"dist directory not found: {resolved_dist}")
    if not resolved_dist.is_dir():
        raise NotADirectoryError(f"dist path is not a directory: {resolved_dist}")
    ...
    handler = partial(_ConfiguredHandler, directory=str(resolved_dist))
    ...
    logger.info(
        "[jiuwenavatar-web] /file-api roots -> %s, %s, %s",
        workspace_root,
        agent_teams_root,
        logs_root,
    )
...
class EmbeddedFrontendServer:
    """Run the frontend static/proxy HTTP server in a background thread."""
    ...
    def __init__(
        ...
        *,
        ...
        self._ws_disable_compress = ws_disable_compress
    ...
    def start(self, *, wait_for_gateway: bool = True) -> None:
        if self._thread is not None:
            return
        self._server, self._logger = _build_frontend_server(
            self._host,
            self._port,
            self._proxy_target,
            log_level=self._log_level,
            ws_disable_compress=self._ws_disable_compress,
        )
        ws_target = _normalize_ws_target(self._proxy_target)
        if wait_for_gateway and self._logger is not None:
            _wait_for_gateway(ws_target, self._logger)
        self._thread = threading.Thread(
            target=self._server.serve_forever,
            name="embedded-frontend-web",
            daemon=True,
        )
        self._thread.start()
        if self._logger is not None:
            self._logger.info("[jiuwenavatar-web] embedded server thread started")
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            pass
        if self._logger is not None:
            self._logger.info("[jiuwenavatar-web] embedded server stopped")
        self._server = None
        self._thread = None
        self._logger = None
...
def _wait_for_gateway(ws_target: str, logger: logging.Logger) -> None:
    ...
    logger.info("[jiuwenavatar-web] waiting for gateway %s:%s ...", host, port)
    if wait_for_tcp_port(host, port, timeout=15.0, max_attempts=15, target_state="connected"):
        logger.info("[jiuwenavatar-web] gateway available")
    else:
        logger.warning("[jiuwenavatar-web] gateway not available after 15 seconds")
...
def main() -> None:
    ...
    default_port = int(os.getenv("FRONTEND_PORT", str(DEFAULT_FRONTEND_PORT)))
    web_port = os.getenv("WEB_PORT", str(DEFAULT_WEB_PORT))  # WebChannel websocket port (proxy target)
    ...
    try:
        ...
        api_target = args.api_target.strip()
        ws_target = args.ws_target.strip()
        server, logger = _build_frontend_server(
            args.host,
            args.port,
            proxy_target,
            dist_dir=dist_dir,
        ...
        resolved_ws = _normalize_ws_target(ws_target or proxy_target)
    except (ValueError, FileNotFoundError, NotADirectoryError) as exc:
        raise SystemExit(str(exc)) from exc

    _wait_for_gateway(resolved_ws, logger)
    ...
    try:
        ...
        server.server_close()
        logger.info("[jiuwenavatar-web] server closed")
