import { Message, MessageRole, UsageSummary, FileDownloadItem, WsEvent, MediaItem } from '../types';
...
function parseHistoryMediaItems(
  record: Record<string, unknown>,
  sessionId: string,
): MediaItem[] | undefined {
  const raw = record.media_items;
  if (!Array.isArray(raw) || raw.length === 0) return undefined;
  const items: MediaItem[] = [];
  for (const item of raw) {
    if (!item || typeof item !== 'object') continue;
    const m = item as Record<string, unknown>;
    const mimeType = typeof m.mimeType === 'string' ? m.mimeType : 'image/png';
    const filename = typeof m.filename === 'string' ? m.filename : 'image.png';
    const type = (typeof m.type === 'string' ? m.type : 'image') as MediaItem['type'];
    const path = typeof m.path === 'string' ? m.path : '';
    items.push({
      type,
      mimeType,
      filename,
      url: path ? `/file-api/session-media?session=${encodeURIComponent(sessionId)}&file=${encodeURIComponent(path)}` : undefined,
    });
  }
...
function parseHistoryTimelineEntry(
  ...
  if (role === 'user') {
    ...
    // 允许仅有图片无文本的用户消息
    if (!content.trim() && !Array.isArray(record.media_items)) {
      return null;
    }
    const id =
      pickFirstString(record, ['id', 'message_id', 'msg_id']) ?? `hist-user-${sessionId}-${at}`;
    const mediaItems = parseHistoryMediaItems(record, sessionId);
    return {
      kind: 'message',
      message: { id, role: 'user', content, timestamp: at, ...(mediaItems ? { mediaItems } : {}) },
    };
  }
