...
interface UseWebSocketReturn {
  ...
  sendMessage: (content: string, sessionId: string, images?: { base64Data: string; mimeType: string; filename: string }[]) => Promise<void>;
...
export function useWebSocket(options: UseWebSocketOptions): UseWebSocketReturn {
  ...
  const sendMessage = useCallback(
    async (content: string, sessionId: string, images?: { base64Data: string; mimeType: string; filename: string }[]) => {
      ...
      // 添加用户消息（含图片附件）
      const userMediaItems = images?.map((img) => ({
        type: 'image' as const,
      ...
      addMessage({
        ...
        ...(userMediaItems?.length ? { mediaItems: userMediaItems } : {}),
      });
