import { useState, useRef, useCallback, KeyboardEvent, useEffect, DragEvent, ClipboardEvent } from 'react';
import { useTranslation } from 'react-i18next';
import { Square, Image as ImageIcon, X } from 'lucide-react';
...
function readImageAsBase64(file: File): Promise<ImageAttachment> {
  return new Promise((resolve, reject) => {
    ...
    if (file.size > MAX_IMAGE_SIZE) {
      reject(new Error(`图片过大 (${(file.size / 1024 / 1024).toFixed(1)}MB)，最大 10MB`));
      return;
    }
...
export function InputArea({
  ...
  }, [inputValue, pendingVoiceText, images, isInterruptible, isListening, onSubmit, onInterrupt, stopListening, isAgentMode, isTeamMode, addToTaskQueue, setInputValue]);
  ...
  return (
    <div
      className={cx(
        ...
        isDragOver && 'chat-input-container--drag-over',
      )}
    ...
    >
      ...
      {images.length > 0 && (
        <div className="chat-input-image-preview">
          {images.map((img, idx) => (
            <div key={idx} className="chat-input-image-thumb">
              <img
                src={`data:${img.mimeType};base64,${img.base64Data}`}
                alt={img.filename}
                className="chat-input-image-thumb-img"
              />
              <button
                type="button"
                className="chat-input-image-thumb-remove"
      ...
      {imageError && (
        <div className="chat-input-image-error">{imageError}</div>
      )}
      ...
      <div className="chat-input-toolbar">
        ...
        <div className="chat-input-actions">
          ...
          <button
            ...
            title="上传图片"
          >
            <ImageIcon className="chat-input-btn-icon" aria-hidden="true" />
          </button>
