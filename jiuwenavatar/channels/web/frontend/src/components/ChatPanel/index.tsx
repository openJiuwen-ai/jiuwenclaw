...
import { AgentMode, Message, UserAnswer, Session } from '../../types';
import { MessageList } from './MessageList';
import { ContextCompressionLines } from './MessageItem';
import { InputArea, type ImageAttachment } from './InputArea';
...
import { AvatarSessionSwitcher } from './AvatarSessionSwitcher';
...
interface ChatPanelProps {
  ...
  /** 当前分身下的历史会话列表（来自 session.list） */
  avatarSessions?: Session[];
  isConnected?: boolean;
...
function WelcomeHeading() {
  ...
  if (isZh) {
    return (
      <>
        我是<span className="chat-welcome__brand">JiuwenAvatar</span>，很高兴认识你!
      </>
    );
  }
  ...
  return (
    <>
      Hi, I&apos;m <span className="chat-welcome__brand">JiuwenAvatar</span>. Nice to meet you!
    </>
  );
}
...
export function ChatPanel({
  ...
  const sessionList = avatarSessions ?? [];
  const {
    messages,
    isThinking,
    toolExecutionOrder,
    ...
    <div className="chat-panel-shell flex flex-col h-full" data-testid="chat-panel">
      ...
      <div className="sticky top-0 z-10 px-3 pt-2 bg-bg/95 backdrop-blur-sm">
        <div className="chat-panel-header">
          <AvatarChatSelector onNavigateToAvatars={onNavigateToAvatars} onAvatarChange={onAvatarChange} />
          {onSwitchSession ? (
            <AvatarSessionSwitcher
