...
/** 多模型列表与 legacy config 扁平字段（api_base/model 等）对齐 */
function legacyModelFieldsFromEntry(primary: ModelEntry | undefined): Record<string, string> {
  if (!primary) return {};
  return {
...
const KEY_DISPLAY_I18N: Record<string, string> = {
  ...
  gitcode_token: "config.keys.gitcodeToken",
...
const KEY_LABEL_HINT_I18N: Record<string, string> = {
  ...
  gitcode_token: "config.keyHelp.gitcodeToken",
...
export function ConfigPanel({
  ...
  useEffect(() => {
    const models = storeAvailableModels.map((m) => ({ ...m, alias: m.alias || "" }));
    setDraftModels(models);
    const legacy = legacyModelFieldsFromEntry(models[0]);
    if (Object.keys(legacy).length > 0) {
      setDraftValues((prev) => ({ ...prev, ...legacy }));
    }
    setModelError(null);
  ...
  const hasAgentsTeamsEntries = draftAgents.length > 0 || draftTeams.length > 0;
  const blockingAgentsTeamsValidationError =
    hasAgentsTeamsEntries ? agentsTeamsValidationError : null;
  ...
  return (
    <div className="flex-1 min-h-0">
      <div className="card w-full h-full flex flex-col">
        ...
        {
          !error && blockingAgentsTeamsValidationError ? (
            <div className="mb-4 rounded-md border border-[var(--border-danger)] bg-danger-subtle px-3 py-2 text-sm text-danger">
              {blockingAgentsTeamsValidationError}
            </div>
          ) : null
        }
