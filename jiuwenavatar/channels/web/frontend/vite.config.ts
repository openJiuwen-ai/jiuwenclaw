 ...
 * 优先级：环境变量 > 已存在的用户工作区 ~/.jiuwenavatar > 仓库根。
 */
function resolveProjectRootDir(): string {
  ...
  if (home) {
    ...
    const userWorkspace = path.join(home, '.jiuwenavatar')
    ...
    configureServer(server) {
      server.middlewares.use('/share-api/snapshot', (req, res) => {
        ...
        try {
          ...
          const now = new Date()
          const filename = `jiuwenavatar-share-${now.toISOString().replace(/[-:]/g, '').replace(/\..+$/, '').replace('T', '-')}.png`
          const snapshot = {
