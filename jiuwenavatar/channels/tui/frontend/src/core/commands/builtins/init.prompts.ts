...
export interface ExistingFiles {
  jiuwenavatarMd: boolean;
  jiuwenavatarLocalMd: boolean;
  claudeMd: boolean;
...
3. **Existing files pre-detected in workspace root**:
  - JIUWENSWARM.md: ${yesNo(existing.jiuwenavatarMd)} ${existing.jiuwenavatarMd ? "— you MUST read it first, propose a diff, then use `ask_user` with `questions` to ask the user whether to apply. Example: `ask_user(query='Update JIUWENSWARM.md?', questions=[{question: 'JIUWENSWARM.md already exists. What would you like to do?', header: 'Update', options: [{label: 'Apply update', description: 'Merge the proposed changes into the existing file'}, {label: 'Skip (keep current)', description: 'Leave the file unchanged and continue'}], multi_select: false}])`. If user chooses 'Apply update', use Edit to apply the diff; if 'Skip', leave the file unchanged and continue. NEVER silently overwrite." : ""}
   - JIUWENSWARM.local.md: ${yesNo(existing.jiuwenavatarLocalMd)} ${existing.jiuwenavatarLocalMd ? "— propose additions via Edit only, never overwrite." : ""}
...
${existing.jiuwenavatarMd ? "File EXISTS — read it, propose a merged diff, use `ask_user` with `questions` to get user confirmation (options: 'Apply update' / 'Skip (keep current)'), then apply via Edit if confirmed. DO NOT use Write to overwrite silently." : "File is absent — use Write to create it."}
...
This file provides guidance to JiuwenAvatar (and any compatible AI coding assistant) when working with code in this repository.
\`\`\`
...
${existing.jiuwenavatarLocalMd ? "File EXISTS — propose additions via Edit, never overwrite." : "File is absent — use Write to create it."}
