...
const { values } = parseArgs({
  options: {
    url: { type: "string", default: "ws://127.0.0.1:29001/tui" },
    session: { type: "string" },
...
if (values.help) {
  console.log(`jiuwenavatar-tui - Terminal CLI for JiuwenAvatar

Options:
  --url <url>       Gateway CLI WebSocket URL (default: ws://127.0.0.1:29001/tui)
...
if (!process.env.JIUWENSWARM_TUI_HEADLESS && (!process.stdin.isTTY || !process.stdout.isTTY)) {
  console.error("jiuwenavatar-tui requires an interactive TTY");
  process.exit(1);
}

const wsClient = new WsClient(values.url ?? "ws://127.0.0.1:29001/tui", values.token ?? "");
