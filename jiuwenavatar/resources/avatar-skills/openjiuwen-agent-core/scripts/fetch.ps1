...
#
# 用法:
#   powershell -ExecutionPolicy Bypass -File scripts\fetch.ps1 -Tag v0.1.13
#   powershell -ExecutionPolicy Bypass -File scripts\fetch.ps1 -Tag 0.1.13
...
#
# 参数:
#   -Tag    tag 名（如 v0.1.13 或 0.1.13），或 auto（扫描 references/v*.md 批量拉取）
...
function Get-ReferenceTags {
    ...
    if (-not $tags -or @($tags).Count -eq 0) {
        throw "references 中未找到 v*.md 版本索引，无法执行 auto"
    }
    return ,$tags
}
...
function Invoke-AutoFetch {
    ...
    if ($tags.Count -eq 0) {
        Write-Err "references 中未找到 v*.md，无法执行 auto"
        exit 1
    }
...
if ([string]::IsNullOrWhiteSpace($Tag)) {
    $Tag = Read-Host "请输入 tag (例如 v0.1.13) 或 auto"
}
$Tag = $Tag.Trim()
