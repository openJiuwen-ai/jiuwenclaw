#!/usr/bin/env bash
# ---------------------------------------------------------------
# 按 tag 拉取 openJiuwen agent-core 快照到 assets/<tag>
#
# 用法:
#   bash scripts/fetch.sh v0.1.13
#   bash scripts/fetch.sh 0.1.13      # 自动补 v 前缀
...
#   - 忽略 references/ 下非 v*.md 的补充文档（如 python-sdk-notes.md）
...
# agent-core tag 习惯 vX.Y.Z；用户输入 X.Y.Z 时补 v
normalize_tag() {
  local t="$1"
  t="$(echo "$t" | xargs)"
...
TAG="${1:-}"
if [[ -z "$TAG" ]]; then
  read -rp "请输入 tag (例如 v0.1.13) 或 auto: " TAG
fi
TAG="$(echo "$TAG" | xargs)"
