#!/usr/bin/env bash
# ---------------------------------------------------------------
# 按 tag 拉取 openJiuwen agent-core-java 快照到 assets/<tag>
#
# 用法:
#   bash scripts/fetch.sh v0.1.7
...
#   - 忽略 references/ 下非 v*.md 的补充文档（如 java-sdk-notes.md）
#   - assets/<tag> 已存在则跳过；不存在则拉取
# ---------------------------------------------------------------

set -euo pipefail

REPO_URL="https://gitcode.com/openJiuwen/agent-core-java.git"
FORCE="${FORCE:-0}"
...
list_reference_tags() {
  ...
  shopt -s nullglob
  local files=("$ref"/v*.md)
  shopt -u nullglob
  if [[ ${#files[@]} -eq 0 ]]; then
    err "references 中未找到 v*.md 版本索引，无法执行 auto"
    exit 1
  fi
  for f in "${files[@]}"; do
    base="$(basename "$f" .md)"
...
TAG="${1:-}"
if [[ -z "$TAG" ]]; then
  read -rp "请输入 tag (例如 v0.1.7) 或 auto: " TAG
fi
TAG="$(echo "$TAG" | xargs)"
