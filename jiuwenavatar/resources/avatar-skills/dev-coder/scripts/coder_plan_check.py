#!/usr/bin/env python3
"""Inspect doc/<module>/dev_plan.md checklist: status (query), verify (validate), set (legacy/manual)."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "dev_plan checklist tools for coder: status=query, verify=validate after md edits, "
            "set=legacy/manual (agents must edit dev_plan.md directly)."
        )
    )
    ...
    status = subparsers.add_parser("status", help="Query unfinished and completed dev plan items.")
    status.add_argument("--state", choices=["all", "todo", "done"], default="all")
    status.add_argument("--format", choices=["text", "json"], default="text")

    set_state = subparsers.add_parser(
        "set",
        help="(Legacy/manual) Mark exactly one checklist item done or todo. Agents should edit dev_plan.md directly.",
    )
    ...
    verify = subparsers.add_parser(
        "verify",
        help="Validate dev_plan.md checklist format and bidirectional parent/child consistency after agent edits.",
    )
...
def command_set(args: argparse.Namespace, root: Path, module: str) -> None:
    ...
    if args.state == item.state:
        print(f"[OK] No change: dev_plan line {item.line} already {args.state}.")
        return
