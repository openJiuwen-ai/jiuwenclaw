# ---------------------------------------------------------------
# 按 tag 拉取 openJiuwen Agent Studio 快照到 assets/<tag>
#
# 用法:
#   powershell -ExecutionPolicy Bypass -File scripts\fetch.ps1 -Tag v0.1.7
...
#
# 参数:
#   -Tag    tag 名（如 v0.1.7），或 auto（扫描 references/v*.md 批量拉取）
#   -Force  单 tag 模式：目标目录非空时仍尝试克隆
#
# auto 模式:
#   - 从 references/ 读取 v*.md（版本索引），文件名（去 .md）即 tag
#   - 忽略 references/ 下非 v*.md 的补充文档（如 studio-sdk-notes.md）
...
if ([string]::IsNullOrWhiteSpace($Tag)) {
    $Tag = Read-Host "请输入 tag (例如 v0.1.7) 或 auto"
}
$Tag = $Tag.Trim()
