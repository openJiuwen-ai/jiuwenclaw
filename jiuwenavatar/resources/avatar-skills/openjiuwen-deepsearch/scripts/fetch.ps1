# ---------------------------------------------------------------
# 按 tag 拉取 openJiuwen deepsearch 快照到 assets/<tag>
#
# 用法:
#   powershell -ExecutionPolicy Bypass -File scripts\fetch.ps1 -Tag v0.1.6
#   powershell -ExecutionPolicy Bypass -File scripts\fetch.ps1 -Tag auto
#
# 参数:
#   -Tag    tag 名（如 v0.1.6），或 auto（扫描 references/v*.md 批量拉取）
#   -Force  单 tag 模式：目标目录非空时仍尝试克隆
#
# auto 模式:
#   - 从 references/ 读取 v*.md（版本索引），文件名（去 .md）即 tag
#   - 忽略 references/ 下非 v*.md 的补充文档（如 deepsearch-sdk-notes.md）
# ---------------------------------------------------------------
...
$RepoUrl = "https://gitcode.com/openJiuwen/deepsearch.git"
...
function Test-GitAvailable { $null = Get-Command git -ErrorAction Stop }
...
function Get-ReferenceTags {
    ...
    $tags = Get-ChildItem -LiteralPath $ReferencesRoot -File -Filter "v*.md" |
        ...
        Sort-Object { [version]($_.TrimStart('v') -replace '\.post(\d+)$', '.$1') }
    return ,$tags
}

function Invoke-FetchTag {
    param([string]$TagName, [switch]$SkipIfExists, [switch]$AllowForce)
    ...
    if (Test-Path -LiteralPath $targetDir) {
        ...
        try { & git clone --branch $TagName --depth 1 --single-branch $RepoUrl . }
...
function Invoke-AutoFetch {
    ...
    if ($tags.Count -eq 0) { Write-Err "references 中未找到 v*.md 版本索引，无法执行 auto"; exit 1 }
    Write-Info "auto 模式：自 references/ 发现 $($tags.Count) 个 tag"
    Write-Info "仓库: $RepoUrl"
    $fetched = 0; $skipped = 0; $failed = 0
    foreach ($t in $tags) {
        Write-Host ""; Write-Info "---- $t ----"
        try {
            $r = Invoke-FetchTag -TagName $t -SkipIfExists
            if ($r.Status -eq "skipped") { $skipped++ } else { $fetched++ }
        } catch { $failed++; Write-Err "$t — $($_.Exception.Message)" }
    }
    Write-Host ""; Write-Info "auto 完成：拉取 $fetched，跳过 $skipped，失败 $failed"
    if ($failed -gt 0) { exit 1 }
}

if ([string]::IsNullOrWhiteSpace($Tag)) { $Tag = Read-Host "请输入 tag (例如 v0.1.6) 或 auto" }
$Tag = Normalize-Tag $Tag
Test-GitAvailable
if (-not (Test-Path -LiteralPath $AssetsRoot)) { New-Item -ItemType Directory -Path $AssetsRoot -Force | Out-Null }
if ($Tag -ieq "auto") { Invoke-AutoFetch; exit 0 }
if (-not (Test-TagSafe -Value $Tag)) { Write-Err "无效的 tag: $Tag"; exit 1 }
Write-Info "仓库: $RepoUrl"
try { Invoke-FetchTag -TagName $Tag -AllowForce | Out-Null }
