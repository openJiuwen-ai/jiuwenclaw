...
parse_dotenv_early("jiuwenavatar-start")

# --- Now safe to import jiuwenavatar modules ---
from jiuwenavatar.common.utils import get_root_dir, get_user_workspace_dir, is_package_installation
...
# - package mode: ~/.jiuwenavatar
DATA_ROOT = get_root_dir()

# Package source root:
# - source mode: <repo>/jiuwenavatar
# - package mode: <site-packages>/jiuwenavatar
...
class InstanceCommand:
    ...
    def validate_and_load(self) -> int | None:
        ...
        if self.config is None:
            ...
            print(f"[start_services] Run 'jiuwenavatar-init --name {self.name}' to create it.")
            return 1
...
def _build_commands(mode: str, dotenv_path: Path | None = None) -> list[tuple[str, list[str], Path]]:
    ...
    if mode in ("all", "app", "dev"):
        cmd = [python_cmd, "-m", "jiuwenavatar.app"] + dotenv_arg
        commands.append(("app", cmd, DATA_ROOT))

    if mode in ("all", "web"):
        cmd = [python_cmd, "-m", "jiuwenavatar.channels.web.app_web"] + dotenv_arg
...
def _start_process(name: str, cmd: list[str], cwd: Path) -> subprocess.Popen[bytes]:
    ...
    env = os.environ.copy()
    env["JIUWENAVATAR_START_CMD"] = json.dumps(sys.argv[:])
...
def _action_list() -> int:
    ...
    if not statuses:
        ...
        print("[start_services] Run 'jiuwenavatar-init' to initialize default instance.")
        return 0
