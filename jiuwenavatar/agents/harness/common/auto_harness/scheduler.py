# jiuwenavatar/agentserver/deep_agent/auto_harness/scheduler.py
...
class Scheduler:
    ...
    def _resolve_model(self, model_name: Optional[str] = None) -> Optional[Model]:
        """Resolve model from jiuwenavatar config (same approach as interface_deep).
    ...
    def _build_model_cache(self) -> None:
        """Build model cache from jiuwenavatar config.yaml (reuse interface_deep logic)."""
        try:
