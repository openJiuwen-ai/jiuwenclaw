...
"""ProjectMemoryRail -- jiuwenavatar product-side rail.
...
This rail lives in jiuwenavatar (not agent-core) so that:

* agent-core stays untouched
* jiuwenavatar can evolve memory-loading semantics without upstream PRs
"""
...
class ProjectMemoryRail(DeepAgentRail):
    ...
    Loaded sources (all read-only; only ``JIUWENAVATAR.md`` and
    ``JIUWENAVATAR.local.md`` are written by ``/init``):

    * **Project root**: ``JIUWENAVATAR.md``, ``JIUWENAVATAR.local.md``,
      ``.jiuwen/JIUWENAVATAR.md``, ``.jiuwen/rules/*.md``
    * **User level**: ``~/.jiuwen/JIUWENAVATAR.md``, ``~/.jiuwen/rules/*.md``
    * **Managed**: ``/etc/jiuwen/JIUWENAVATAR.md``, ``/etc/jiuwen/rules/*.md``
    * **Additional dirs**: explicit project-memory directories passed to the rail
    ...
    def set_additional_directories(self, dirs: tuple[str, ...] | list[str] | None) -> None:
        ...
        JIUWENAVATAR.md (typically the CLI process's cwd, which differs from
