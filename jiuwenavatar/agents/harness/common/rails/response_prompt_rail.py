...
from jiuwenavatar.common.context_keys import JIUWENAVATAR_CHANNEL_CONTEXT_KEY
from jiuwenavatar.agents.harness.common.prompt.prompt_builder import (
    LocalSectionName,
    PromptPriority,
    _response_prompt,
)
...
class ResponsePromptRail(DeepAgentRail):
    ...
    async def before_invoke(self, ctx: AgentCallbackContext) -> None:
        ...
        if isinstance(extra, dict) and channel:
            extra[JIUWENAVATAR_CHANNEL_CONTEXT_KEY] = channel
    ...
    def _resolve_channel(ctx: AgentCallbackContext) -> str | None:
        ...
        if isinstance(extra, dict):
            value = extra.get(JIUWENAVATAR_CHANNEL_CONTEXT_KEY)
            if value is not None:
                return str(value)
    ...
    def _sync_a2ui_prompt_section(self, channel: str | None) -> None:
        ...
        try:
            from jiuwenavatar.server.runtime.a2ui.integration import is_a2ui_channel

            if not is_a2ui_channel(channel):
                self.system_prompt_builder.remove_section(LocalSectionName.A2UI)
                return

            from jiuwenavatar.server.runtime.a2ui.config import get_current_a2ui_config
