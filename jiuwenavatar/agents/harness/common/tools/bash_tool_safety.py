...
"""Apply jiuwenavatar shell safety rules to openjiuwen BashTool / PowerShellTool.
...
def _wrap_invoke(
    ...
    invoke.jiuwenavatar_safety_wrapped = True
    return invoke
...
def _wrap_stream(
    ...
    stream.jiuwenavatar_safety_wrapped = True
    return stream


def _patch_tool_class(tool_cls: type) -> None:
    if not getattr(tool_cls.invoke, "jiuwenavatar_safety_wrapped", False):
        tool_cls.invoke = _wrap_invoke(tool_cls.invoke)
    if not getattr(tool_cls.stream, "jiuwenavatar_safety_wrapped", False):
