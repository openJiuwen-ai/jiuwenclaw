...
# ── jiuwenavatar-tui 反复 spawn 护栏 ────────────────────────────────
#
# Background:
#   The agent has been observed entering reflexive loops where it spawns the
#   ``jiuwenavatar-tui`` binary (directly or via ``@microsoft/tui-test`` specs)
...
_TUI_SPAWN_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"\bjiuwenavatar-tui(?:\s|$|['\"])", re.IGNORECASE),
...
def _enforce_tui_spawn_budget(command: str, session_id: str) -> str | None:
    ...
    if len(history) >= TUI_SPAWN_LIMIT:
        ...
        return (
            f"jiuwenavatar-tui spawn budget exceeded for this session "
...
_DANGEROUS_COMMAND_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    (
        re.compile(r"\bpkill\b[^\n\r;|&]*jiuwenavatar", re.IGNORECASE),
        "blocked pattern: pkill targeting jiuwenavatar (includes user TUI)",
    ),
    (
        re.compile(r"\bkillall\b[^\n\r;|&]*jiuwenavatar", re.IGNORECASE),
        "blocked pattern: killall targeting jiuwenavatar (includes user TUI)",
    ),
    (
    ...
    (
        ...
        "blocked pattern: kill targeting jiuwenavatar (includes user TUI)",
    ),
    (
        re.compile(
        ...
        "blocked pattern: xargs kill pipeline targeting jiuwenavatar",
    ),
]
...
async def mcp_exec_command(
    ...
    # Guardrail: rate-limit jiuwenavatar-tui spawn loops. Returns a friendly
