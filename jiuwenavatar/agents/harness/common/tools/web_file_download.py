...
- 密钥来源: 环境变量 JIUWENAVATAR_FILE_DOWNLOAD_SECRET 或自动生成并写入共享文件
"""
...
_SECRET_ENV_KEY = "JIUWENAVATAR_FILE_DOWNLOAD_SECRET"
_SECRET_FILE_NAME = ".file_download_secret"


def _get_secret_file_path() -> Path:
    workspace = os.getenv("JIUWENAVATAR_WORKSPACE")
    if workspace:
        return Path(workspace) / "config" / _SECRET_FILE_NAME
    return Path.home() / ".jiuwenavatar" / "config" / _SECRET_FILE_NAME
