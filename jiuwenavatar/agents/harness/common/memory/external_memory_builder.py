...
Dispatches on `memory.external.provider`:
  ...
  - <plugin>    -> user-installed plugin from ~/.jiuwenavatar/plugins/memory/
...
def _build_mem0_provider(ext_cfg: Dict[str, Any]):
    ...
    user_id = mem0_cfg.get("user_id") or os.environ.get("MEM0_USER_ID", "jiuwenavatar-user")
    agent_id = mem0_cfg.get("agent_id") or os.environ.get("MEM0_AGENT_ID", "jiuwenavatar")
