...
from jiuwenavatar.instance_manager.status import get_instance_config
from jiuwenavatar.instance_manager.yaml import get_instance_index
...
def create_bootstrap_env(config: InstanceConfig) -> Path:
    ...
    - JIUWENAVATAR_DATA_DIR: instance workspace path
    - JIUWENAVATAR_INSTANCE: instance name
    ...
    lines = [
        ...
        f"JIUWENAVATAR_DATA_DIR={config.workspace}",
        f"JIUWENAVATAR_INSTANCE={config.name}",
    ]
...
def _create_basic_bootstrap_env(
    ...
    IMPORTANT: This function must NOT depend on any jiuwenavatar modules
    ...
    lines = [
        ...
        f"JIUWENAVATAR_DATA_DIR={workspace}",
        f"JIUWENAVATAR_INSTANCE={name}",
...
def load_instance_bootstrap_by_name(name: str) -> Path | None:
    ...
    .env file to set JIUWENAVATAR_DATA_DIR and port environment variables.
    ...
    if config is None:
        ...
        _logger.error("Run 'jiuwenavatar-init --name %s' to create it.", name)
        return None
