...
RESERVED_NAMES = frozenset({'default', 'config', 'tmp', 'jiuwenavatar', 'all'})

# Base ports for default instance (index=0)
BASE_PORTS = {
    "agent_server": DEFAULT_AGENT_SERVER_PORT,
...
def collect_all_ports(exclude_name: Optional[str] = None) -> List[int]:
    ...
    from jiuwenavatar.instance_manager.yaml import load_instances_yaml
