...
默认根目录为 ``~/.jiuwenavatar``；若进程环境中已设置 ``JIUWENAVATAR_DATA_DIR``（须为可用绝对路径，
且应在启动本脚本前注入，见 ``jiuwenavatar.utils`` 中的 ``JIUWENAVATAR_DATA_DIR``），则初始化到该路径下。
...
使用方式:
- jiuwenavatar-init -f: 强制清理，删除整个用户数据根目录后重新初始化
- jiuwenavatar-init: 保留原有数据，执行迁移合并
- jiuwenavatar-init --name alice: 创建命名实例 alice
- jiuwenavatar-init -f --name alice: 强制重建命名实例 alice
"""
...
from jiuwenavatar.common.utils import get_user_home, init_user_workspace, get_user_workspace_dir
...
def run_init(force: bool = False, name: Optional[str] = None) -> int:
    ...
    if name:
        ...
        if validation_error:
            print(f"[jiuwenavatar-init] ERROR: {validation_error}")
            return 1

    # 2. Determine target workspace path and set env var
    if name:
        workspace_path = get_instance_workspace_path(name)
        print(f"[jiuwenavatar-init] Creating instance: {name}")
        print(f"[jiuwenavatar-init] Workspace: {workspace_path}")
    else:
        workspace_path = get_user_home() / ".jiuwenavatar"
        print(f"[jiuwenavatar-init] Initializing default workspace")
        print(f"[jiuwenavatar-init] Workspace: {workspace_path}")
    ...
    if name:
        ...
        if status.running:
            print(f"[jiuwenavatar-init] ERROR: Instance '{name}' is running (PID={status.pid}).")
            print(f"[jiuwenavatar-init] Stop it first with: jiuwenavatar-start --stop {name}")
            return 1
    elif force:
        # For default instance, use get_default_instance_status which includes port detection
        status = get_default_instance_status()
        if status.running:
            print(f"[jiuwenavatar-init] ERROR: Default instance is running (PID={status.pid or '-'}).")
            print(f"[jiuwenavatar-init] Stop it first with: jiuwenavatar-start --stop default")
            return 1
    ...
    if name and target != "cancelled":
        ...
        print(f"[jiuwenavatar-init] Instance '{name}' initialized successfully.")
        return 0

    if target == "cancelled":
        return 1

    print(f"[jiuwenavatar-init] initialized: {target}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Initialize jiuwenavatar workspace directory (~/.jiuwenavatar)"
    )
    parser.add_argument(
        "-f",
        "--force",
