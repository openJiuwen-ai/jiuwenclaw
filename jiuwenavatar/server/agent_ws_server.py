...
from jiuwenavatar.agents.harness.common.auto_harness import AutoHarnessService, reset_harness_packages_state
...
from jiuwenavatar.common.utils import get_agent_sessions_dir, get_config_file
...
from jiuwenavatar.agents.harness.common.rails.permissions.permissions_config_rpc import (
...
from jiuwenavatar.server.sandbox.jiuwenbox_runner import JiuwenBoxRunner
from jiuwenavatar.common.hooks_config import load_hooks_config
...
def _is_restorable_history_record(record: Any) -> bool:
    ...
    has_media = isinstance(record.get("media_items"), list) and len(record["media_items"]) > 0
...
def _canonicalize_sandbox_files_path(path: str) -> str:
    ...
    保留并扩成「绝对化 + resolve」是因为:
      ...
      写法时, jiuwenavatar server 直接拿去 stat / 入库 / 比较, 行为依赖
...
class AgentWebSocketServer:
    ...
    - is_stream=False：``process_message`` → 一条 **E2AResponse** JSON（``jiuwenavatar.e2a.wire_codec``）
    ...
    _checkpointer_ready: ClassVar[bool] = False
    _checkpointer_lock: ClassVar[asyncio.Lock | None] = None
    ...
    async def _ensure_checkpointer_if_needed(self) -> None:
        """Initialize sqlite checkpointer on first Gateway connection (not at bind time)."""
        if AgentWebSocketServer._checkpointer_ready:
            return
        if AgentWebSocketServer._checkpointer_lock is None:
            AgentWebSocketServer._checkpointer_lock = asyncio.Lock()
        async with AgentWebSocketServer._checkpointer_lock:
            if AgentWebSocketServer._checkpointer_ready:
                return
            from jiuwenavatar.server.runtime.agent_adapter.interface_deep import (
                ensure_persistent_checkpointer,
            )

            await ensure_persistent_checkpointer()
            AgentWebSocketServer._checkpointer_ready = True
    ...
    async def _connection_handler(self, ws: Any) -> None:
        ...
        await self._ensure_checkpointer_if_needed()
        ...
        try:
            ...
            try:
                from jiuwenavatar.agents.harness.team import cancel_all_team_stream_tasks_across_managers
    ...
    async def _handle_message(self, ws: Any, raw: str | bytes, send_lock: asyncio.Lock) -> None:
        ...
        try:
            ...
            if request.req_method == ReqMethod.AGENTS_TOOLS_LIST:
                ...
                return
            # Persona & Avatar（数字分身）
            if request.req_method == ReqMethod.PERSONAS_LIST:
                await self._handle_personas_list(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.PERSONAS_GET:
                await self._handle_personas_get(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.PERSONAS_CREATE:
                await self._handle_personas_create(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.PERSONAS_UPDATE:
                await self._handle_personas_update(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.PERSONAS_DELETE:
                await self._handle_personas_delete(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.PERSONAS_DUPLICATE:
                await self._handle_personas_duplicate(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.PERSONAS_GENERATE:
                await self._handle_personas_generate(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.AVATARS_LIST:
                await self._handle_avatars_list(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.AVATARS_GET:
                await self._handle_avatars_get(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.AVATARS_CREATE:
                await self._handle_avatars_create(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.AVATARS_UPDATE:
                await self._handle_avatars_update(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.AVATARS_DELETE:
                await self._handle_avatars_delete(ws, request, send_lock)
                return
            # Triggers（触发器）
            if request.req_method == ReqMethod.TRIGGERS_LIST:
                await self._handle_triggers_list(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.TRIGGERS_GET:
                await self._handle_triggers_get(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.TRIGGERS_CREATE:
                await self._handle_triggers_create(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.TRIGGERS_UPDATE:
                await self._handle_triggers_update(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.TRIGGERS_DELETE:
                await self._handle_triggers_delete(ws, request, send_lock)
                return
            # Missions & Reports
            if request.req_method == ReqMethod.MISSIONS_LIST:
                await self._handle_missions_list(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.MISSIONS_GET:
                await self._handle_missions_get(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.MISSIONS_CANCEL:
                await self._handle_missions_cancel(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.MISSIONS_DELETE:
                await self._handle_missions_delete(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.REPORTS_LIST:
                await self._handle_reports_list(ws, request, send_lock)
                return
            if request.req_method == ReqMethod.REPORTS_GET:
                await self._handle_reports_get(ws, request, send_lock)
                return
    ...
    async def _handle_session_rename(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        ...
        from jiuwenavatar.server.runtime.session.session_rename import apply_session_rename
    ...
    async def _ensure_persistent_checkpointer_response(
        ...
        try:
            from jiuwenavatar.server.runtime.agent_adapter.interface_deep import ensure_persistent_checkpointer
    ...
    async def _handle_team_delete(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        ...
        from jiuwenavatar.agents.harness.team import (
    ...
    async def _handle_session_rewind_full(
        ...
        from jiuwenavatar.agents.harness.common.session_ops_service import (
            rewind_session,
        ...
        try:
            ...
            if restore_files:
                from jiuwenavatar.agents.harness.common.session_ops_service import restore_session_files
        ...
        try:
            ...
            if action == "add_model":
                ...
                if not env_updates:
                    ...
                    try:
                        from jiuwenavatar.agents.harness.common.memory.config import clear_config_cache
                        clear_config_cache()
    ...
    async def _handle_sandbox_files_set(
        ...
        # / 比较行为依赖 jiuwenavatar server 当前 cwd; 见
        ...
        # / daily_memory / 项目目录 / jiuwenavatar config.yaml) 再次写进
    ...
    def _resolve_active_project_dir(
        ...
        process because that's typically ``~/.jiuwenavatar`` and would
    ...
    def _attach_effective_sandbox_files(
        ...
        the user-configured allow list, and jiuwenavatar's own ``config.yaml``
        ...
        cwd, which is usually ``~/.jiuwenavatar`` and not what the user means
        by "project".
    ...
    def _resolve_adapter(agent: Any) -> Any:
        """从 JiuwenAvatar 中提取底层 Deep/Code Adapter (持 _sys_operation_card 的实例)."""
        if agent is None:
            return None
    ...
    async def _handle_command_status(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        try:
            ...
            if action == "usage":
                ...
                config_dir = os.getenv("JIUWENAVATAR_CONFIG_DIR")
                if config_dir:
                    settings_sources.append(f"env:JIUWENAVATAR_CONFIG_DIR={config_dir}")
    ...
    async def _handle_browser_start(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        ...
        try:
            from jiuwenavatar.agents.harness.common.tools.browser_start_client import start_browser
    ...
    async def _handle_agents_list(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        ...
        from jiuwenavatar.server.runtime.agent_config_service import AgentConfigService
...
返回 JSON 对象，包含 whenToUse 和 systemPrompt 两个字段。不要返回其他内容。"""
    ...
    async def _handle_agents_create(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from dataclasses import asdict as dataclass_asdict
        from jiuwenavatar.server.runtime.agent_config_service import AgentConfigService, CreateAgentParams
    ...
    async def _handle_agents_update(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from dataclasses import asdict as dataclass_asdict
        from jiuwenavatar.server.runtime.agent_config_service import AgentConfigService, UpdateAgentParams
    ...
    # ------------------------------------------------------------------
    # Persona & Avatar handlers（数字分身）
    # ------------------------------------------------------------------

    async def _handle_personas_list(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            manager = PersonaManager.get_instance()
            result = await manager.handle_personas_list()
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=True, payload=result)
        except Exception as e:
            logger.exception("[AgentWebSocketServer] personas.list failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})

        wire = encode_agent_response_for_wire(resp, response_id=request.request_id)
        async with send_lock:
            await ws.send(json.dumps(wire, ensure_ascii=False))

    async def _handle_personas_get(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            params = request.params or {}
            manager = PersonaManager.get_instance()
            result = await manager.handle_personas_get(persona_id=params.get("persona_id", ""))
            ok = "error" not in result
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=ok, payload=result)
        except Exception as e:
            logger.exception("[AgentWebSocketServer] personas.get failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
    ...
    async def _handle_personas_create(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            manager = PersonaManager.get_instance()
            result = await manager.handle_personas_create(**(request.params or {}))
            ok = "error" not in result
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=ok, payload=result)
        except Exception as e:
            logger.exception("[AgentWebSocketServer] personas.create failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
    ...
    async def _handle_personas_update(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            params = dict(request.params or {})
            persona_id = params.pop("persona_id", "")
            manager = PersonaManager.get_instance()
            result = await manager.handle_personas_update(persona_id=persona_id, **params)
            ok = "error" not in result
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=ok, payload=result)
        except Exception as e:
            logger.exception("[AgentWebSocketServer] personas.update failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
    ...
    async def _handle_personas_delete(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            params = request.params or {}
            manager = PersonaManager.get_instance()
            result = await manager.handle_personas_delete(
                persona_id=params.get("persona_id", ""),
                cascade_avatars=bool(params.get("cascade_avatars")),
            )
            ok = "error" not in result
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=ok, payload=result)
        except Exception as e:
            logger.exception("[AgentWebSocketServer] personas.delete failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
    ...
    async def _handle_personas_duplicate(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            manager = PersonaManager.get_instance()
            result = await manager.handle_personas_duplicate(**(request.params or {}))
            ok = "error" not in result
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=ok, payload=result)
        except Exception as e:
            logger.exception("[AgentWebSocketServer] personas.duplicate failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
    ...
    async def _handle_personas_generate(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            manager = PersonaManager.get_instance()
            result = await manager.handle_personas_generate(**(request.params or {}))
            ok = "error" not in result
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=ok, payload=result)
        except Exception as e:
            logger.exception("[AgentWebSocketServer] personas.generate failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
    ...
    async def _handle_avatars_list(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.server.runtime.persona import PersonaManager
        ...
        try:
            ...
            logger.exception("[AgentWebSocketServer] avatars.list failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
    ...
    async def _handle_avatars_get(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            params = request.params or {}
            manager = PersonaManager.get_instance()
            result = await manager.handle_avatars_get(avatar_id=params.get("avatar_id", ""))
            ok = "error" not in result
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=ok, payload=result)
        except Exception as e:
            logger.exception("[AgentWebSocketServer] avatars.get failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
    ...
    async def _handle_avatars_create(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            params = request.params or {}
            manager = PersonaManager.get_instance()
            result = await manager.handle_avatars_create(**params)
            ok = "error" not in result
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=ok, payload=result)
        except Exception as e:
            logger.exception("[AgentWebSocketServer] avatars.create failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
    ...
    async def _handle_avatars_update(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            params = dict(request.params or {})
            avatar_id = params.pop("avatar_id", "")
            manager = PersonaManager.get_instance()
            result = await manager.handle_avatars_update(avatar_id=avatar_id, **params)
            ok = "error" not in result
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=ok, payload=result)
        except Exception as e:
            logger.exception("[AgentWebSocketServer] avatars.update failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
    ...
    async def _handle_avatars_delete(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.server.runtime.persona import PersonaManager

        try:
            params = request.params or {}
            manager = PersonaManager.get_instance()
            result = await manager.handle_avatars_delete(avatar_id=params.get("avatar_id", ""))
            ok = "error" not in result
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=ok, payload=result)
        except Exception as e:
            logger.exception("[AgentWebSocketServer] avatars.delete failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
    ...
    # ------------------------------------------------------------------
    # Trigger handlers（触发器）
    # ------------------------------------------------------------------

    async def _handle_triggers_list(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.gateway.trigger import TriggerEngine

        try:
            engine = TriggerEngine.get_instance()
            params = request.params or {}
            result = await engine.handle_triggers_list(**params)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=True, payload=result)
        except Exception as e:
            logger.exception("[AgentWebSocketServer] triggers.list failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
    ...
    async def _handle_triggers_get(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.gateway.trigger import TriggerEngine
        ...
        try:
            ...
            logger.exception("[AgentWebSocketServer] triggers.get failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
    ...
    async def _handle_triggers_create(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.gateway.trigger import TriggerEngine
        ...
        try:
            ...
            logger.exception("[AgentWebSocketServer] triggers.create failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
    ...
    async def _handle_triggers_update(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.gateway.trigger import TriggerEngine

        try:
            params = dict(request.params or {})
            trigger_id = params.pop("trigger_id", "")
            engine = TriggerEngine.get_instance()
            result = await engine.handle_triggers_update(trigger_id=trigger_id, **params)
            ok = "error" not in result
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=ok, payload=result)
        except Exception as e:
            logger.exception("[AgentWebSocketServer] triggers.update failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
    ...
    async def _handle_triggers_delete(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.gateway.trigger import TriggerEngine
        ...
        try:
            ...
            logger.exception("[AgentWebSocketServer] triggers.delete failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
    ...
    async def _handle_missions_list(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        ...
        try:
            ...
            logger.exception("[AgentWebSocketServer] missions.list failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
        wire = encode_agent_response_for_wire(resp, response_id=request.request_id)
        async with send_lock:
            await ws.send(json.dumps(wire, ensure_ascii=False))
    ...
    async def _handle_missions_get(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        ...
        try:
            ...
            logger.exception("[AgentWebSocketServer] missions.get failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
        wire = encode_agent_response_for_wire(resp, response_id=request.request_id)
        async with send_lock:
            await ws.send(json.dumps(wire, ensure_ascii=False))

    async def _handle_missions_cancel(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        from jiuwenavatar.gateway.trigger import TriggerEngine
        try:
            params = request.params or {}
            engine = TriggerEngine.get_instance()
            result = await engine.cancel_mission(params.get("mission_id", ""))
            ok = "error" not in result
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=ok, payload=result)
        except Exception as e:
            logger.exception("[AgentWebSocketServer] missions.cancel failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
        wire = encode_agent_response_for_wire(resp, response_id=request.request_id)
        async with send_lock:
            await ws.send(json.dumps(wire, ensure_ascii=False))
    ...
    async def _handle_missions_delete(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        ...
        try:
            ...
            logger.exception("[AgentWebSocketServer] missions.delete failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
        wire = encode_agent_response_for_wire(resp, response_id=request.request_id)
        async with send_lock:
            await ws.send(json.dumps(wire, ensure_ascii=False))
    ...
    async def _handle_reports_list(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        ...
        try:
            ...
            logger.exception("[AgentWebSocketServer] reports.list failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
        wire = encode_agent_response_for_wire(resp, response_id=request.request_id)
        async with send_lock:
            await ws.send(json.dumps(wire, ensure_ascii=False))
    ...
    async def _handle_reports_get(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        ...
        try:
            ...
            logger.exception("[AgentWebSocketServer] reports.get failed: %s", e)
            resp = AgentResponse(request_id=request.request_id, channel_id=request.channel_id, ok=False, payload={"error": str(e)})
        wire = encode_agent_response_for_wire(resp, response_id=request.request_id)
        async with send_lock:
            await ws.send(json.dumps(wire, ensure_ascii=False))
    ...
    def _resolve_model(self, model_name: Optional[str] = None) -> Optional[Any]:
        """Resolve model from jiuwenavatar config.
