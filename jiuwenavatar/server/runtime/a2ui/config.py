...
def get_a2ui_config(config: dict[str, Any] | None = None) -> A2UIConfig:
    ...
    env_enabled = os.getenv("JIUWENAVATAR_A2UI_ENABLED")
    if env_enabled is not None:
        enabled = _to_bool(env_enabled, enabled)

    protocol_version = str(
        os.getenv("JIUWENAVATAR_A2UI_PROTOCOL_VERSION")
    ...
    env_smoke = os.getenv("JIUWENAVATAR_A2UI_SMOKE_TOOLS")
...
def get_current_a2ui_config() -> A2UIConfig:
    """Read A2UI config from the active jiuwenavatar runtime config."""
