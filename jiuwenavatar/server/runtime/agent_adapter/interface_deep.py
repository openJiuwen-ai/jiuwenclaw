...
from jiuwenavatar.agents.harness.common.tools.todo_compat import (
...
from jiuwenavatar.agents.harness.common.rails.execution_guard import (
    CircuitBreakerRail,
...
from jiuwenavatar.agents.harness.common.memory import clear_memory_manager_cache
from jiuwenavatar.agents.harness.common.memory.config import (
    clear_config_cache,
    get_memory_mode,
    is_memory_enabled,
    is_proactive_memory,
)
from jiuwenavatar.agents.harness.common.memory.external_memory_config import is_builtin_memory_allowed
from jiuwenavatar.agents.harness.common.rails.permissions.tool_permission_context import TOOL_PERMISSION_CHANNEL_ID
...
from jiuwenavatar.server.runtime.skill import filter_visible_skill_names
...
from jiuwenavatar.server.utils.stream_utils import parse_ask_user_question_payload
from jiuwenavatar.agents.harness.common.tools.multimodal_config import (
...
from jiuwenavatar.agents.harness.common.tools.video_tools import video_understanding
from jiuwenavatar.agents.harness.common.tools.image_tools import generate_image

from jiuwenavatar.agents.harness.common.tools import SendFileToolkit, SkillToolkit
from jiuwenavatar.agents.harness.common.tools.wiki_tools import wiki_ingest, wiki_query, wiki_lint
from jiuwenavatar.agents.harness.common.tools.acp_output_tools import get_tools as get_acp_output_tools
from jiuwenavatar.agents.harness.common.tools.multi_session_toolkits import MultiSessionToolkit
from jiuwenavatar.agents.harness.common.tools.acp_chat import acp_chat
from jiuwenavatar.server.runtime.coding import coding_task
from jiuwenavatar.agents.harness.common.tools.xiaoyi_phone_tools import (
    get_user_location,
    create_note,
    search_notes,
    modify_note,
...
from jiuwenavatar.common.config import get_config, get_default_models, get_sandbox_runtime, resolve_env_vars
from jiuwenavatar.server.runtime.agent_adapter.sysop_builder import (
    build_filesystem_policy,
    create_local_sysop_card,
    create_sandbox_sysop_card,
)
from jiuwenavatar.agents.harness.common.auto_harness.service import _HARNESS_PACKAGES_FILE
...
class JiuWenClawDeepAdapter:
    ...
    def _inject_attribution_to_config(config: dict) -> None:
        ...
        from jiuwenavatar.common.openrouter_attribution import inject_attribution_to_config
    ...
    async def apply_sandbox_runtime_patch(
        ...
        if card is None or card.mode != OperationMode.SANDBOX:
            logger.info(
                "[JiuWenAvatarDeepAdapter] apply_sandbox_runtime_patch skipped: "
        ...
        if launcher is None:
            logger.warning(
                "[JiuWenAvatarDeepAdapter] apply_sandbox_runtime_patch: missing launcher_config"
            )
            return
        ...
        logger.info(
            "[JiuWenAvatarDeepAdapter] sandbox runtime patched "
        ...
        if files_changed:
            try:
                ...
                logger.warning(
                    "[JiuWenAvatarDeepAdapter] force_recreate_jiuwenbox_sandbox import "
                    "failed: %s",
                    exc,
                )
                return
            try:
                ...
                logger.info(
                    "[JiuWenAvatarDeepAdapter] sandbox instance recreated: %s",
                    new_sandbox_id,
                )
            except Exception as exc:  # noqa: BLE001
                logger.warning(
                    "[JiuWenAvatarDeepAdapter] force_recreate_jiuwenbox_sandbox "
                    "failed: %s",
                    exc,
                )
    ...
    @staticmethod
    def _build_persona_avatar_chat_rail() -> Any | None:
        """Build PersonaAvatarChatRail for web main-chat avatar selection."""
        try:
            from jiuwenavatar.server.runtime.persona.persona_avatar_chat_rail import (
                PersonaAvatarChatRail,
            )

            rail = PersonaAvatarChatRail()
            logger.info("[JiuWenClawDeepAdapter] PersonaAvatarChatRail create success")
            return rail
        except Exception as exc:
            logger.warning("[JiuWenClawDeepAdapter] PersonaAvatarChatRail create failed: %s", exc)
            return None
    ...
    def _build_agent_rails(
        ...
        rail_infos = [
            ...
            _RailBuildInfo("_persona_avatar_chat_rail", self._build_persona_avatar_chat_rail),
    ...
    def _get_current_agent_rails(
        ...
        if self._persona_avatar_chat_rail is not None:
            rails_list.append(self._persona_avatar_chat_rail)
        if self._permission_rail is not None:
    ...
    async def create_instance(
        ...
        agent_card = AgentCard(name=self._agent_name, id='jiuwenavatar')
        ...
        # 无需初始化 jiuwenavatar 内置 PermissionEngine（已弃用）。
        ...
        setattr(self._instance, "_jiuwenavatar_project_dir", self._project_dir or self._workspace_dir)
    ...
    @staticmethod
    def _extract_avatar_id_from_request(request: AgentRequest) -> str:
    ...
    async def _set_subagent_delegation_enabled(self, enabled: bool) -> None:
        ...
        if self._instance is None:
            return
        if self._subagent_rail is None and enabled:
            self._subagent_rail = self._build_subagent_rail()
        if self._subagent_rail is None:
            return
        if enabled:
    ...
    async def _apply_avatar_chat_context(self, runtime_config: "_RuntimeConfig") -> None:
        """Apply web avatar-chat constraints: skills allow-list + persona prompt."""
        from jiuwenavatar.server.runtime.persona.chat_context import (
            ensure_avatar_skills_installed,
            resolve_avatar_chat_context,
        )

        ctx = resolve_avatar_chat_context(runtime_config.avatar_id)
        ...
        if ctx and self._skill_manager is not None:
            ...
            if installed:
                logger.info(
                    "[JiuWenClawDeepAdapter] Avatar skills ready: %s",
                    installed,
                )
            if missing:
                logger.warning(
                    "[JiuWenClawDeepAdapter] Avatar skills missing: %s",
                    missing,
                )

        # 编码引擎：按分身 coding_engine 统一准备工作区；CLI 缺失时不在对话请求内安装，
        # 由创建/保存分身时触发后台安装。未安装完成则拒绝本次对话。
        ...
        if ctx and ctx.coding_engine:
            from jiuwenavatar.common.utils import get_agent_skills_dir

            # 先按分身绑定工作区，再 ensure_ready，使工作区直接在分身子目录下准备，
            # 不同分身产物互不覆盖。
            set_workspace_avatar(ctx.avatar_id)
            engine = get_coding_engine(ctx.coding_engine)
            status = engine.ensure_ready(get_agent_skills_dir(), auto_install=False)
            set_active_coding_engine(engine)
            logger.info(
                "[JiuWenClawDeepAdapter] Coding engine: kind=%s available=%s "
                "cli=%s workspace=%s detail=%s",
                status.kind,
                status.available,
                status.executable or "(native)",
                status.workspace,
                status.detail,
            )
            if engine.provides_tool() and not status.available:
                from jiuwenavatar.server.runtime.coding import CODING_ENGINE_JIUWEN
                from jiuwenavatar.server.runtime.coding.bootstrap import get_cli_install_status

                install_status = get_cli_install_status(ctx.coding_engine)
                
                # CLI 安装失败：自动回退到 jiuwen-coding
                if not install_status.running and install_status.last_detail and not install_status.last_detail.startswith("installed"):
                    logger.warning(
                        "[JiuWenClawDeepAdapter] CLI install failed for %s: %s. Falling back to jiuwen-coding.",
                        ctx.coding_engine,
                        install_status.last_detail,
                    )
                    # 切换到 jiuwen-coding
                    fallback_engine = get_coding_engine(CODING_ENGINE_JIUWEN)
                    fallback_status = fallback_engine.ensure_ready(get_agent_skills_dir(), auto_install=False)
                    set_active_coding_engine(fallback_engine)
                    
                    # 发送通知给用户（通过日志，UI 可捕获）
                    logger.info(
                        "[JiuWenClawDeepAdapter] Fallback to jiuwen-coding: kind=%s available=%s detail=%s",
                        fallback_status.kind,
                        fallback_status.available,
                        fallback_status.detail,
                    )
                    # jiuwen-coding 不提供 tool，移除之
                    self._remove_coding_task_tool()
                else:
                    # 仍在安装中或其他原因，拒绝本次对话
                    detail = "后台安装仍在进行中" if install_status.running else (install_status.last_detail or status.detail)
                    clear_active_coding_engine()
                    self._remove_coding_task_tool()
                    raise RuntimeError(
                        f"{engine.display_name} CLI 尚未就绪，无法开始对话。{detail}。"
                        "请稍后重试；如果持续失败，请检查 Node/npm 与网络后重试。"
                    )
            if engine.provides_tool():
                await self._ensure_coding_task_tool_registered()
            else:
                self._remove_coding_task_tool()
        else:
            clear_active_coding_engine()
        ...
        if self._skill_rail is not None:
            self._skill_rail.enabled_skills = set(ctx.skills) if ctx else None
            await self._skill_rail.reload_skills()

        if runtime_config.mode in ("agent.plan", "agent.fast"):
            if ctx:
                await self._set_subagent_delegation_enabled(False)
            else:
                await self._set_subagent_delegation_enabled(runtime_config.mode == "agent.plan")

        if ctx:
            logger.info(
                "[JiuWenClawDeepAdapter] Avatar chat mode: avatar=%s persona=%s "
                "coding_engine=%s skills=%s",
                ctx.avatar_id,
                ctx.persona_id,
                ctx.coding_engine,
                list(ctx.skills),
    ...
    async def _ensure_coding_task_tool_registered(self) -> None:
        """Register the unified coding_task tool for CLI-backed coding engines."""
        if self._instance is None:
            return
        try:
            if not Runner.resource_mgr.get_tool(coding_task.card.id):
                Runner.resource_mgr.add_tool(coding_task)
            registered = {
                str(getattr(card, "name", "") or "")
                for card in (self._instance.ability_manager.list() or [])
            }
            if coding_task.card.name not in registered:
                self._instance.ability_manager.add(coding_task.card)
        except Exception as exc:
            logger.warning(
                "[JiuWenClawDeepAdapter] coding_task tool registration failed: %s",
                exc,
            )

    def _remove_coding_task_tool(self) -> None:
        """Remove the coding_task tool (native jiuwen-coding / non-coding avatars)."""
        if self._instance is None:
            return
        try:
            self._instance.ability_manager.remove("coding_task")
        except Exception:
            pass
    ...
    async def _update_runtime_config(self, runtime_config: "_RuntimeConfig") -> None:
        ...
        await self._apply_avatar_chat_context(runtime_config)
        ...
        try:
            await self._update_runtime_config(
                self._RuntimeConfig(
                    ...
                    avatar_id=self._extract_avatar_id_from_request(request),
                )
            )
    ...
    async def process_message_stream_impl(
        ...
        if mode in ("team", "team.plan", "code.team"):
            from jiuwenavatar.server.runtime.agent_adapter.team_helpers import process_team_message_stream
    ...
    def _build_external_memory_rail(self):
        from jiuwenavatar.agents.harness.common.memory.external_memory_builder import (
    ...
    async def _handle_external_memory_rail_by_config(self):
        ...
        """
        from jiuwenavatar.agents.harness.common.memory.external_memory_config import (
    ...
    async def generate_recap(self, session_id: str) -> dict[str, Any]:
        ...
        """
        from jiuwenavatar.server.runtime.agent_adapter.recap_prompts import (
    ...
    def _get_recent_messages(self, session_id: str, window: int = 30) -> list[Any]:
        ...
        try:
            ...
            from jiuwenavatar.server.runtime.session.session_history import _read_history
            ...
            for rec in records:
                ...
                media_items = rec.get("media_items")
                has_media = isinstance(media_items, list) and len(media_items) > 0

                if role == "user":
                    if not has_content and not has_media:
                        continue
                    # 如果有图片附件，在内容中追加图片引用说明
                    recap_content = content if isinstance(content, str) else ""
                    if has_media:
                        image_names = []
                        for mi in media_items:
                            if isinstance(mi, dict):
                                fname = mi.get("filename", "image")
                                image_names.append(str(fname))
                        if image_names:
                            recap_content += f"\n[用户上传了图片: {', '.join(image_names)}]"
                    recapworthy.append(SimpleNamespace(role="user", content=recap_content))
                elif role == "assistant":
                    if not has_content:
                        continue
    ...
    async def try_start_dreaming(self, busy_checker: Callable[[], bool] | None = None) -> None:
        if self._dreaming_started:
            return
        try:
            from jiuwenavatar.agents.harness.common.memory.dreaming import start_dreaming
            from jiuwenavatar.common.utils import get_agent_sessions_dir
    ...
    async def try_stop_dreaming(self) -> None:
        if not self._dreaming_started:
            return
        try:
            from jiuwenavatar.agents.harness.common.memory.dreaming import stop_dreaming
            mode = getattr(self, "_dreaming_mode", "agent")
...
def _agent_def_to_subagent_config(
    ...
    Args:
        agent_def: 自定义 agent 定义（来自 .jiuwenavatar/agents/*.md）
