...
def init_session_metadata(
    *,
    ...
    avatar_id: 该会话绑定的分身 ID。空字符串表示默认助手（非分身会话）。
        会话与分身一一绑定后，切换分身即新建会话，天然隔离上下文/todo/产物。
    """
    metadata = {
...
def update_session_metadata(
    *,
    ...
    if not metadata:
        ...
        # avatar_id 仅在尚未绑定时补写，避免后续消息把已绑定的分身覆盖掉
        # 注意：只有当传入的 avatar_id 是有效值（非空字符串）且 metadata 中没有有效值时才更新
        if avatar_id and not metadata.get("avatar_id"):
...
def get_all_sessions_metadata(
    ...
    if avatar_id is not None:
        ...
        sessions = [
            ...
            if str(item.get("avatar_id") or "").strip() == expected
        ]
