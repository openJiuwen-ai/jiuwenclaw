...
Agent 定义来源（优先级从高到低）：
- project: <workspace>/.jiuwenavatar/agents/*.md
- user:    ~/.jiuwenavatar/agents/*.md
- local:   <workspace>/.jiuwenavatar/agents-local/*.md
- builtin: 代码内置
...
class AgentConfigService:
    ...
    def _get_project_agents_dir(self) -> Path:
        return self._workspace_dir / ".jiuwenavatar" / "agents"

    def _get_local_agents_dir(self) -> Path:
        return self._workspace_dir / ".jiuwenavatar" / "agents-local"
    ...
    def list_available_tools() -> dict:
        ...
        from jiuwenavatar.server.runtime.agent_adapter.code_agent_rail import TOOL_GROUPS, DISALLOWED_FOR_SUBAGENTS
