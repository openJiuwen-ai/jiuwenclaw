...
from jiuwenavatar.common.e2a.acp.protocol import build_acp_initialize_result
...
class AgentManager:
    ...
    async def _create_agent(
        ...
        setattr(agent, "_jiuwenavatar_agent_cache_key", agent_cache_key)
        setattr(agent, "_jiuwenavatar_agent_mode", mode_key)
        setattr(agent, "_jiuwenavatar_agent_sub_mode", sub_mode_key)
        setattr(agent, "_jiuwenavatar_agent_project_dir", project_dir)
    ...
    def get_agent_nowait(
        ...
        for agent in channel_agents.values():
            if requested_mode and getattr(agent, "_jiuwenavatar_agent_mode", "") != requested_mode:
                continue
            if requested_sub_mode and getattr(agent, "_jiuwenavatar_agent_sub_mode", "") != requested_sub_mode:
                continue
            if requested_project_dir and getattr(agent, "_jiuwenavatar_agent_project_dir", "") != requested_project_dir:
                continue
            return agent

        if mode is None and project_dir is None and sub_mode is None:
            for agent in channel_agents.values():
                if getattr(agent, "_jiuwenavatar_agent_mode", "") == "agent":
                    return agent
