...
from jiuwenavatar.common.utils import (
    ...
    get_builtin_skills_dirs,
    is_path_under_builtin_skills_dir,
...
_BUILTIN_SKILL_SYNC_SKIP_DIRS = frozenset({"__pycache__", ".git"})
_BUILTIN_SKILL_SYNC_PRESERVE_FILES = frozenset({
    "data/community.db",
})
_BUILTIN_SKILL_SYNC_HASH_CHUNK_SIZE = 1024 * 1024


def _should_skip_builtin_skill_sync_path(rel: Path) -> bool:
    return any(part in _BUILTIN_SKILL_SYNC_SKIP_DIRS for part in rel.parts)


def _hash_file_content(digest: "hashlib._Hash", file_path: Path) -> None:
    with file_path.open("rb") as f:
        while chunk := f.read(_BUILTIN_SKILL_SYNC_HASH_CHUNK_SIZE):
            digest.update(chunk)


def _skill_sync_fingerprint(skill_dir: Path) -> str:
    """计算 skill 目录内容指纹（用于判断是否需要从内置包同步）."""
    ...
    for file_path in sorted(skill_dir.rglob("*")):
        ...
        if rel in _BUILTIN_SKILL_SYNC_PRESERVE_FILES:
            continue
        if _should_skip_builtin_skill_sync_path(file_path.relative_to(skill_dir)):
            continue
...
def _merge_builtin_skill_tree(src: Path, dest: Path) -> None:
    """将内置 skill 与包内版本对齐：覆盖包内文件，删除废弃文件，保留用户运行时数据."""
    dest.mkdir(parents=True, exist_ok=True)

    for dest_entry in sorted(dest.rglob("*"), key=lambda p: len(p.parts), reverse=True):
        rel = dest_entry.relative_to(dest)
        rel_posix = rel.as_posix()
        if rel_posix in _BUILTIN_SKILL_SYNC_PRESERVE_FILES:
            continue
        if _should_skip_builtin_skill_sync_path(rel):
            continue
        src_entry = src / rel
        if dest_entry.is_dir() and not dest_entry.is_symlink():
    ...
    for src_file in src.rglob("*"):
        ...
        if _should_skip_builtin_skill_sync_path(rel):
            continue
        dest_file = dest / rel
        if rel_posix in _BUILTIN_SKILL_SYNC_PRESERVE_FILES and dest_file.is_file():
            continue
...
class SkillManager:
    ...
    def __init__(self, workspace_dir: str | None = None) -> None:
        ...
        # 防御：skills 可能是指向已被清理目录的悬空符号链接（如测试污染留下的临时
        # 目录链接），此时 mkdir(exist_ok=True) 会抛 FileExistsError。清理后重建。
        if self._skills_dir.is_symlink() and not self._skills_dir.exists():
            self._skills_dir.unlink()
    ...
    async def handle_skills_get(self, params: dict) -> dict:
        ...
        for child in self._skills_dir.iterdir():
            ...
            if meta and meta.get("name") == name:
                ...
                meta["is_builtin_source"] = resolve_builtin_skill_dir(child.name) is not None
    ...
    async def handle_skills_install(self, params: dict) -> dict:
        ...
        if "@" not in spec:
            ...
            try:
                ...
                return {"success": False, "detail": str(exc)}
            if resolve_builtin_skill_dir(safe_name) is not None:
    ...
    async def handle_skills_install_builtin(self, params: dict) -> dict:
        ...
        src = resolve_builtin_skill_dir(name)
        if src is None:
            return {"success": False, "detail": f"未找到内置技能: {name}"}
        try:
            src = _safe_child_path(src.parent, src.name, "skill")
        except ValueError as exc:
            return {"success": False, "detail": str(exc)}

        # 检查是否已经安装
        dest = _safe_child_path(self._skills_dir, name, "skill")
        # 防御：悬空软链时 dest.exists() 为 False，但 copytree 仍会因路径已存在
        # (lexists) 抛 FileExistsError；安装前先清理这类失效链接。
        if dest.is_symlink() and not dest.exists():
            try:
                dest.unlink()
                logger.info("清理悬空技能软链后重装: %s", dest)
            except OSError as exc:
                logger.warning("清理悬空技能软链失败 %s: %s", dest, exc)
    ...
    def _should_sync_builtin_skill(self, name: str) -> bool:
        """判断用户目录中的 skill 是否应由内置包自动同步."""
        for local_skill in self._state.get("local_skills", []):
            if local_skill.get("name") == name:
                return False

        plugin = next((p for p in self._get_installed_plugins() if p.get("name") == name), None)
        if plugin is not None:
            source = str(plugin.get("source") or "").strip().lower()
            marketplace = str(plugin.get("marketplace") or "").strip().lower()
            return source == "builtin" or marketplace == "builtin"

        if resolve_builtin_skill_dir(name) is None:
            return False
        try:
            dest = _safe_child_path(self._skills_dir, name, "skill")
        except ValueError:
    ...
    def sync_builtin_skills_from_package(self, *, force: bool = False) -> dict[str, Any]:
        """将已安装的内置 skill 与安装包内资源对齐。

        force=True：安装/升级时调用，直接合并覆盖，不做内容指纹对比。
        """
        ...
        seen: set[str] = set()
        for builtin_root in get_builtin_skills_dirs():
            ...
            for child in builtin_root.iterdir():
                ...
                if not self._should_sync_builtin_skill(name):
                    skipped_other.append(name)
                    continue

                if (dest / _EVOLUTION_FILENAME).is_file():
                    skipped_evolved.append(name)
                    continue

                if not force:
                    src_fp = _skill_sync_fingerprint(child)
                    dest_fp = _skill_sync_fingerprint(dest)
                    if src_fp == dest_fp:
                        skipped_same.append(name)
                        continue

                try:
                    _merge_builtin_skill_tree(child, dest)
                    updated.append(name)
                    logger.info(
                        "[SkillManager] synced builtin skill from package: %s (force=%s)",
                        name,
                        force,
                    )
                except Exception as exc:
                    logger.error("同步内置技能失败 %s: %s", name, exc)
                    errors[name] = str(exc)

        if updated:
            self._refresh_agent_data_indexes()

        self._state["builtin_skills_synced_app_version"] = __version__
        self._state["builtin_skills_synced_at"] = datetime.now(timezone.utc).isoformat()
        self._save_state()

        return {
            "updated": updated,
            "skipped_same": skipped_same,
            "skipped_evolved": skipped_evolved,
    ...
    async def handle_skills_team_skills_hub_install(self, params: dict) -> dict:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="jiuwenavatar_team_skills_hub_") as tmpdir:
                tmp_path = Path(tmpdir)
    ...
    async def handle_skills_team_skills_hub_publish(self, params: dict) -> dict:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="jiuwenavatar_teamskills_publish_") as tmpdir:
    ...
    def _skillnet_install_files_sync(
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="jiuwenavatar_skillnet_") as tmpdir:
                tmp_path = Path(tmpdir)
    ...
    async def handle_skills_uninstall(self, params: dict) -> dict:
        ...
        if is_path_under_builtin_skills_dir(dest):
    ...
    async def _import_skill_from_remote_archive(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(prefix="jiuwenavatar_import_local_") as tmpdir:
            tmp_path = Path(tmpdir)
    ...
    def _is_builtin_skill(self, skill_name: str, installed_plugins: list[dict], skill_path: Path | None = None) -> bool:
        ...
        try:
            ...
            # 如果提供了 skill_path，直接判断该路径是否在 builtin 目录下
            if skill_path is not None:
                return is_path_under_builtin_skills_dir(skill_path)
    ...
    def _scan_builtin_skills(self) -> list[dict]:
        ...
        for builtin_dir in get_builtin_skills_dirs():
    ...
    def _get_mirror_skills_dirs(self) -> list[Path]:
        ...
        try:
            ...
            source_resources_skills_dir = source_repo_root / "jiuwenavatar" / "resources" / "agent" / "skills"
            # 开发模式下不将源码目录作为镜像目标
