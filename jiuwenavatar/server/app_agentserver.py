...
Both processes share the same user workspace directory (~/.jiuwenavatar).
...
from jiuwenavatar.common.service_ports import DEFAULT_AGENT_SERVER_PORT
from jiuwenavatar.dotenv_early import parse_dotenv_early
parse_dotenv_early("jiuwenavatar-agentserver")
...
if _logging_yaml.exists():
    ...
    from jiuwenavatar.common.utils import get_logs_dir
    ...
    _perm_ns_logger = logging.getLogger("jiuwenavatar.agents.harness.common.rails.permissions")
...
async def _run(host: str, port: int) -> None:
    ...
    from jiuwenavatar.agents.harness.team.remote_member_bootstrap import run_teammate_bootstrap_daemon
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="PORT",
        help=f"Bind port (default: AGENT_SERVER_PORT env or {DEFAULT_AGENT_SERVER_PORT}).",
    )
    parser.add_argument(
        "--name",
        metavar="<name>",
