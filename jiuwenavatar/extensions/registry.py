...
from jiuwenavatar.extensions.callback_compat import unregister_callback_sync
from jiuwenavatar.gateway import AgentServerClient
...
from jiuwenavatar.extensions.types import ExtensionConfig
from jiuwenavatar.common.security.base_crypto import CryptoProvider
...
class ExtensionRegistry:
    ...
    def get_or_create_instance(
        ...
        config: dict[str, Any],
        logger: Any,
    ) -> tuple["ExtensionRegistry", bool]:
        """Return (registry, created). ``created`` is False when reusing an existing instance."""
        if cls._instance is not None:
            return cls._instance, False
        return cls.create_instance(callback_framework, config, logger), True
