...
class UpgradeExecutor(ABC):
    ...
    def _download_headers() -> dict[str, str]:
        from jiuwenavatar.common.updater import get_access_token

        headers: dict[str, str] = {
            "Accept": "application/octet-stream, */*",
            "User-Agent": f"JiuwenAvatar-Updater/{__version__}",
        }
...
tar xzf "{target}" -C /tmp/jiuwenavatar_update
...
class PipExecutor(UpgradeExecutor):
    ...
    def install(self) -> None:
        package = self._config.get("repo_name", "jiuwenavatar")
        timeout = self._config["timeout_seconds"]
    ...
    def upgrade(self) -> None:
        start_cmd_raw = os.getenv("JIUWENAVATAR_START_CMD")
        if start_cmd_raw:
            try:
        ...
        start_is_start_cmd = bool(start_cmd_raw) and (
            "jiuwenavatar-start" in str(stored_argv)
            or "start_services" in str(stored_argv)
        )

        if start_is_start_cmd:
            restart_argv = [sys.executable, "-m", "jiuwenavatar.start_services"] + stored_argv[1:]
        else:
            restart_argv = [sys.executable, "-m", "jiuwenavatar.app"] + stored_argv[1:]
                ...
                try:
                    ...
                    if isinstance(web_data.get("argv"), list):
                        ...
                        if raw_web_argv and raw_web_argv[0]:
                            web_argv = [sys.executable, "-m", "jiuwenavatar.channels.web.app_web"] + raw_web_argv[1:]
                        else:
                            web_argv = None
        ...
        frontend_port = int(os.getenv("FRONTEND_PORT", str(DEFAULT_FRONTEND_PORT)))
        restart_data = {
            "argv": restart_argv,
            "env": dict(os.environ),
            "cwd": os.getcwd(),
            ...
            "web_argv": web_argv,
            "web_pid": web_pid,
            "gateway_port": int(os.getenv("WEB_PORT", str(DEFAULT_WEB_PORT))),
        ...
        helper_args = [
            sys.executable, "-m", "jiuwenavatar.common.updater_restart_helper",
