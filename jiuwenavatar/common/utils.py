...
"""Path management for JiuWenAvatar.

根目录见 ``JIUWENAVATAR_DATA_DIR``（默认 ``~/.jiuwenavatar``；可由环境变量 ``JIUWENAVATAR_DATA_DIR`` 指定绝对路径）。
...
内置模板位于包内 ``jiuwenavatar/resources/``：
- ``agent/workspace/`` 仅保存初始化用户 DeepAgent 工作区所需的基础 Markdown 模板；
- ``avatar-skills/`` 保存纳入版本库的内置 Skill 源码；
- ``skills_state.json`` 保存初始化技能状态模板。
"""
...
def _log_component_from_logger_name(name: str) -> str:
    ...
    if name.startswith("jiuwenavatar.channels"):
        return "channel"
    if name.startswith("jiuwenavatar.agents.harness.common.rails.permissions"):
        return "permissions"
    if name.startswith("openjiuwen.harness.security") or name.startswith("openjiuwen.harness.rails.security"):
        return "permissions"
    if name.startswith("jiuwenavatar.agents") or name.startswith("jiuwenavatar.server"):
        return "agent_server"
    return "gateway"
...
def _load_logging_config_from_yaml() -> dict[str, Any]:
    """读取 ~/.jiuwenavatar/config/config.yaml 中的 logging 段（无则空）。"""
    try:
        cf = get_config_file()
        if not cf.exists():
            return {}
...
def get_user_home() -> Path:
    ...
    2. JIUWENAVATAR_HOME environment variable
    3. System default Path.home()
    """
    global _user_home
    if _user_home is not None:
        return _user_home
    env_home = os.getenv("JIUWENAVATAR_HOME")
    if env_home:
...
def get_user_workspace_dir() -> Path:
    """Get the user workspace directory path (~/.jiuwenavatar or custom path).

    Priority:
    1. Cached value (if already set via set_user_workspace_dir or previous call)
    2. JIUWENAVATAR_DATA_DIR environment variable (for multi-instance isolation)
    3. get_user_home() / ".jiuwenavatar" (default instance)

    Also performs one-time migration from ~/.jiuwenclaw/ to ~/.jiuwenavatar/ if needed.
    """
    global _workspace_base_dir
    if _workspace_base_dir is not None:
        return _workspace_base_dir
    env_workspace = os.getenv("JIUWENAVATAR_DATA_DIR")
    if env_workspace:
        _workspace_base_dir = Path(env_workspace)
        return _workspace_base_dir

    # One-time migration from .jiuwenclaw to .jiuwenavatar
    _migrate_from_jiuwenclaw_root()

    _workspace_base_dir = get_user_home() / ".jiuwenavatar"
    return _workspace_base_dir
...
def _find_source_root() -> Path:
    """Find the repository root in development mode (contains jiuwenavatar/ package)."""
    current = Path(__file__).resolve().parent.parent
    jw_pkg = current / "jiuwenavatar"
    if (jw_pkg / "resources" / "agent").exists():
        return current
    parent = current.parent
    jw_pkg2 = parent / "jiuwenavatar"
...
def _find_package_root() -> Path | None:
    """Best-effort detection of the jiuwenavatar package root.

    In package mode (whl), __file__ is at site-packages/jiuwenavatar/common/utils.py,
    so parent.parent is site-packages/jiuwenavatar/.
    In editable / source mode, __file__ is at <project>/jiuwenavatar/common/utils.py,
    so parent.parent is <project>/jiuwenavatar/.
    """
    current = Path(__file__).resolve().parent.parent
    jw_pkg = current / "jiuwenavatar"
...
def prompt_preferred_language() -> Optional[Literal["zh", "en"]]:
    ...
    if not _is_interactive():
        print("[jiuwenavatar-init] Non-interactive mode: using default language 'zh'")
        return "zh"
    print()
    print("[jiuwenavatar-init] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("[jiuwenavatar-init]  请选择默认语言 / Choose your default language")
    print("[jiuwenavatar-init] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("[jiuwenavatar-init]   [1] 中文（简体）")
    print("[jiuwenavatar-init]       → config: preferred_language: zh")
    print("[jiuwenavatar-init]   ────────────────────────────────────────────")
    print("[jiuwenavatar-init]   [2] English")
    print("[jiuwenavatar-init]       → config: preferred_language: en")
    print("[jiuwenavatar-init] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("[jiuwenavatar-init]  须明确选择：1 / 2 / zh / en（无默认语言）")
    print("[jiuwenavatar-init]  取消：no / n / q / cancel / 取消")
    print("[jiuwenavatar-init] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    raw = input(
        "[jiuwenavatar-init] 请输入选项 (1, 2, zh, en) 或 no 取消: "
    ).strip().lower()
    ...
    if raw in ("2", "en", "english", "e", "英文"):
        return "en"
    print("[jiuwenavatar-init] 无效选项；未选择有效语言，初始化已取消（与拒绝 yes/no 相同）。")
    return None
...
def _migrate_from_jiuwenclaw_root() -> bool:
    ...
    new_root = user_home / ".jiuwenavatar"
    ...
    if new_root.exists():
        ...
        print(f"[migration] Both .jiuwenclaw and .jiuwenavatar exist, skipping migration")
        return False
    ...
    try:
        ...
        print(f"[migration] ERROR: Failed to migrate from .jiuwenclaw to .jiuwenavatar: {e}")
        return False
...
def _migrate_jiuwenclaw_workspace_to_workspace(workspace_dir: Path) -> None:
    ...
    Migration:
    - Old: ~/.jiuwenavatar/agent/jiuwenclaw_workspace/
    - New: ~/.jiuwenavatar/agent/workspace/

    Args:
        workspace_dir: Path to workspace root (~/.jiuwenavatar).
    """
...
def _migrate_legacy_workspace(
    ...
    Migration:
    - Old: ~/.jiuwenavatar/agent/home/ (PRINCIPLE.md, TONE.md, HEARTBEAT.md)
    - Old: ~/.jiuwenavatar/agent/skills/
    - Old: ~/.jiuwenavatar/agent/memory/

    - New: ~/.jiuwenavatar/agent/workspace/ (DeepAgent standard)
...
def cleanup_team_files(workspace_dir: Path) -> None:
    ...
    Args:
        workspace_dir: JiuWenClaw 用户工作空间根目录 (~/.jiuwenavatar)
    """
...
def prepare_workspace(
    ...
    # ----- 内置模板根目录：<package>/resources（含 agent/workspace/、avatar-skills/、skills_state.json）-----
    ...
    # Copy the DeepAgent workspace template. It only contains workspace bootstrap files;
    # built-in skills are versioned under resources/avatar-skills and are not copied here.
    ...
    from jiuwenavatar.common.config import migrate_config_from_template, set_preferred_language_in_config_file
...
def _close_log_handlers() -> None:
    """Close all jiuwenavatar log handlers to release file locks.
    ...
    """
    root = logging.getLogger("jiuwenavatar")
...
def _print_diff_summary(diff_result: CopyDiffResult, overwrite: bool) -> None:
    ...
    if total_files == 0:
        print("[jiuwenavatar-init] 初始化完成：工作区已就绪，无新文件需创建 / Init complete: workspace ready, no new files needed")
        return

    print("[jiuwenavatar-init] 初始化完成，文件变更如下：/ Init complete, file changes:")
...
def init_user_workspace(
    ...
    """Initialize ~/.jiuwenavatar from package or source resources.

    资源布局:
    - 模板配置:   <package_root>/resources/config.yaml
    - .env 模板: <package_root>/resources/.env.template
    - 工作区模板: <package_root>/resources/agent/workspace（AGENT/HEARTBEAT/IDENTITY/SOUL/USER/MEMORY 等）
    - 内置技能:   <package_root>/resources/avatar-skills（版本化的内置 Skill 源码）
    - 技能状态:   <package_root>/resources/skills_state.json

    上述内容会被复制到:
    - ~/.jiuwenavatar/config/config.yaml（含 preferred_language）
    - ~/.jiuwenavatar/config/builtin_rules.yaml（内置 shell 安全规则模板，与 config 同目录）
    - ~/.jiuwenavatar/config/.env
    - ~/.jiuwenavatar/agent/...
    ...
    if workspace_dir.exists():
        if overwrite:
            ...
            print(
                f"[jiuwenavatar-init] With -f/--force flag, "
                f"entire {workspace_dir} will be deleted for clean initialization."
            )
            print("[jiuwenavatar-init] WARNING: This will delete all historical configuration and memory information.")
            print("[jiuwenavatar-init] This action cannot be undone.")
            if _is_interactive():
                confirmation = input(
                    "[jiuwenavatar-init] Do you want to confirm reinitialization? (yes/no): "
                ).strip().lower()

                if confirmation not in ("yes", "y"):
                    print("[jiuwenavatar-init] Initialization cancelled. Exiting.")
                    return "cancelled"
            else:
                print("[jiuwenavatar-init] Non-interactive mode: proceeding with reinitialization.")
            ...
            try:
                ...
                print(f"[jiuwenavatar-init] Removed workspace directory: {workspace_dir}")
            except OSError as e:
                print(f"[jiuwenavatar-init] ERROR: Failed to remove "
                  f"workspace: {e}")
                return "cancelled"
        else:
            # Merge mode: inform about preservation
            print("[jiuwenavatar-init] 增量初始化：只添加缺失文件，不覆盖已有文件 / "
              "Incremental init: only adds missing files, preserves existing")
            print("[jiuwenavatar-init] 此操作不可撤销 / This action cannot be undone.")
            if _is_interactive():
                confirmation = input("[jiuwenavatar-init] Do you want to continue? (yes/no): ").strip().lower()

                if confirmation not in ("yes", "y"):
                    print("[jiuwenavatar-init] Initialization cancelled. Exiting.")
                    return "cancelled"
            else:
                print("[jiuwenavatar-init] Non-interactive mode: proceeding with merge initialization.")

    lang = prompt_preferred_language()
    if lang is None:
        print("[jiuwenavatar-init] Initialization cancelled. Exiting.")
        return "cancelled"
    print(f"[jiuwenavatar-init] 将使用语言 / Language: {lang}")
...
def _resolve_paths() -> None:
    ...
    if user_config_dir.exists():
        ...
        # 尚未初始化 ~/.jiuwenavatar：从包内 resources 直读配置，工作区指向包内 agent/workspace
        package_root = _find_package_root()
...
def get_agent_workspace_dir() -> Path:
    ...
    Returns:
        Path to agent workspace: ~/.jiuwenavatar/agent/workspace
    """
...
def get_agent_memory_dir() -> Path:
    ...
    Returns:
        Path to memory directory: ~/.jiuwenavatar/agent/workspace/memory
    """
...
def get_agent_skills_dir() -> Path:
    ...
    Returns:
        Path to skills directory: ~/.jiuwenavatar/agent/workspace/skills
    """
...
def get_deepagent_todo_dir() -> Path:
    ...
    Returns:
        Path to todo directory: ~/.jiuwenavatar/agent/workspace/todo
    """
...
def get_deepagent_messages_dir() -> Path:
    ...
    Returns:
        Path to messages directory: ~/.jiuwenavatar/agent/workspace/messages
    """
...
def get_deepagent_agents_dir() -> Path:
    ...
    Returns:
        Path to agents directory: ~/.jiuwenavatar/agent/workspace/agents
    """
...
def get_deepagent_heartbeat_path() -> Path:
    ...
    Returns:
        Path to HEARTBEAT.md: ~/.jiuwenavatar/agent/workspace/HEARTBEAT.md
    """
...
def get_deepagent_agent_md_path() -> Path:
    ...
    Returns:
        Path to AGENT.md: ~/.jiuwenavatar/agent/workspace/AGENT.md
    """
...
def get_deepagent_soul_md_path() -> Path:
    ...
    Returns:
        Path to SOUL.md: ~/.jiuwenavatar/agent/workspace/SOUL.md
    """
...
def get_deepagent_identity_md_path() -> Path:
    ...
    Returns:
        Path to IDENTITY.md: ~/.jiuwenavatar/agent/workspace/IDENTITY.md
    """
...
def get_deepagent_user_md_path() -> Path:
    ...
    Returns:
        Path to USER.md: ~/.jiuwenavatar/agent/workspace/USER.md
    """
    return get_agent_workspace_dir() / "USER.md"


def get_builtin_skills_dirs() -> list[Path]:
    """返回所有内置技能目录（按优先级排序）.

    1. resources/avatar-skills — 分身预置 AIDLC 技能（纳入版本库）
    2. resources/agent/workspace/skills — 开发/构建时生成的技能目录
    3. resources/agent/skills — 旧版回退路径
    """
    package_root = _find_package_root()
    candidates = [
        package_root / "resources" / "avatar-skills",
        package_root / "resources" / "agent" / "workspace" / "skills",
        package_root / "resources" / "agent" / "skills",
    ]
    return [p for p in candidates if p.is_dir()]


def get_builtin_skills_dir() -> Path:
    """Get the primary built-in skills directory from package resources."""
    dirs = get_builtin_skills_dirs()
    if dirs:
        return dirs[0]
    package_root = _find_package_root()
    return package_root / "resources" / "avatar-skills"


def resolve_builtin_skill_dir(skill_name: str) -> Path | None:
    """在全部内置技能目录中查找指定技能文件夹."""
    name = (skill_name or "").strip()
    if not name:
        return None
    for base in get_builtin_skills_dirs():
        candidate = base / name
        if candidate.is_dir():
            return candidate
    return None
...
def is_path_under_builtin_skills_dir(path: Path) -> bool:
    """判断路径是否位于任一内置技能目录下."""
    try:
...
def _migrate_legacy_checkpoint_and_logs() -> None:
    """One-time migration: move ~/.jiuwenavatar/.checkpoint and .logs to ~/.jiuwenavatar/agent/."""
...
def setup_logger(log_level: Optional[str] = None) -> logging.Logger:
    """配置 ``jiuwenavatar`` 根日志：控制台 + 分组件文件 + 汇总 full.log。

    各模块应使用 ``logging.getLogger(__name__)``，分文件规则：
    - ``jiuwenavatar.channel.*`` → channel.log
    - ``jiuwenavatar.agents.*`` 或 ``jiuwenavatar.server.*`` → agent_server.log
    - 其余 ``jiuwenavatar.*``（含 ``jiuwenavatar.app``、gateway、evolution、utils 等）→ gateway.log

    所有分类日志同时写入 ``full.log``。输出目录：``~/.jiuwenavatar/agent/.logs/``。
