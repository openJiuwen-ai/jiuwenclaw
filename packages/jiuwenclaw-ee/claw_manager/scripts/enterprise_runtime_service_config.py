...
加载 ``service_config`` 槽位时，还会解析命中策略上的 ``service_id`` / ``agent_id`` /
...
典型用法（PowerShell，项目根目录；请在仓库根 ``.env`` 中配置与 Gateway 相同的 ``GATEWAY_*``）::
    ...
    # 单场景：alice → S1 销售组 AgentServer 池（2.5.1）+ workspace_dir=alice
...
_DEMO_SCENARIOS: dict[tuple[str, str], ServiceConfigExpectation] = {
    ...
    ("g_demo_sales", "bob"): ServiceConfigExpectation(
        ...
        min_idle_services=2,
        max_services=10,
        policy_source="继承 2.5.1 / 2.6.2",
        service_id="g_demo_sales::bot_main",
        agent_id="default_agent_id_1",
        workspace_dir="g_demo_sales::bot_main::bob",
    ),
    ("g_unknown", "bob"): ServiceConfigExpectation(
        ...
        min_idle_services=1,
        max_services=5,
        policy_source="2.7 全局兜底",
        service_id=None,
        agent_id=None,
        # 默认拼接在 Runtime _coalesce_loaded_invoke_ids，loader 未命中 Agent 策略时为 None
        workspace_dir=None,
    ),
}
...
def _log_expectation(group_id: str, user_id: str) -> ServiceConfigExpectation | None:
    ...
    if expect is None:
        ...
        return None
    workspace_key = (
        hashlib.md5(expect.workspace_dir.encode("utf-8")).hexdigest()
        if expect.workspace_dir
        else None
    )
    logger.info(
        "[expect] service_config=%s (%s); template_name=%r; min_idle=%s max_services=%s; "
        "service_id=%r agent_id=%r workspace_dir=%r workspace_key=%s path=workspace_%s/agent/…",
        expect.label,
...
def _policy_ids_match(loaded: Any, expect: ServiceConfigExpectation) -> list[str]:
    ...
    actual_ws = getattr(loaded, "workspace_dir", None)
    if actual_ws != expect.workspace_dir:
        errors.append(
            f"workspace_dir={actual_ws!r} 预期 {expect.workspace_dir!r}"
        )
    return errors


def _workspace_key(workspace_dir: str | None) -> str | None:
    if not workspace_dir:
        return None
    return hashlib.md5(str(workspace_dir).encode("utf-8")).hexdigest()
...
async def _verify_one(
    *,
    ...
    entity = entities[0]
    ws_dir = getattr(loaded, "workspace_dir", None)
    ws_key = _workspace_key(ws_dir)
    logger.info(
        ...
        "workspace_dir=%s workspace_key=%s path=workspace_%s/agent/…",
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="验证 Gateway Runtime Management 的 service_config / workspace_dir 企业策略加载",
    )
    p.add_argument(
        "jiuwenclaw_id",
        nargs="?",
    ...
    p.add_argument(
        "--all-scenarios",
        action="store_true",
        help="依次验证文档 3.2.1 alice / 3.2.2 bob / 3.2.3 g_unknown 三个 service_config / workspace_dir 场景",
    )
    return p.parse_args()
