...
@mapping_router.get("/{id}", response_model=ResponseModel)
async def get_template_mapping(
    ...
    svc = _mapping_svc(handler)
    try:
        row = await svc.get(jiuwenclaw_id, id)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    if row is None:
...
@mapping_router.patch("/{id}", response_model=ResponseModel)
async def update_template_mapping(
    ...
    svc = _mapping_svc(handler)
    try:
        row = await svc.update(jiuwenclaw_id, id, body)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    if row is None:
        raise HTTPException(status_code=404, detail="template mapping not found")
...
@mapping_router.delete("/{id}", response_model=ResponseModel)
async def delete_template_mapping(
    ...
    svc = _mapping_svc(handler)
    try:
        ok = await svc.delete(jiuwenclaw_id, id)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    if not ok:
        raise HTTPException(status_code=404, detail="template mapping not found")
    return ResponseModel(code=200, message="success", data={"deleted": True, "id": id})
...
@global_router.get("/{id}", response_model=ResponseModel)
...
@global_router.patch("/{id}", response_model=ResponseModel)
...
@global_router.delete("/{id}", response_model=ResponseModel)
...
@service_router.get("/{id}", response_model=ResponseModel)
...
@service_router.patch("/{id}", response_model=ResponseModel)
...
@service_router.delete("/{id}", response_model=ResponseModel)
...
@agent_router.get("/{id}", response_model=ResponseModel)
...
@agent_router.patch("/{id}", response_model=ResponseModel)
...
@agent_router.delete("/{id}", response_model=ResponseModel)
