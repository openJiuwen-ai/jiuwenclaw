...
class ServiceConfigTemplateService:
    ...
    def _template_dict_for_push(row: dict[str, Any], *, now: datetime) -> dict[str, Any]:
        return {
            ...
            "cpu_request": row.get("cpu_request", "500m"),
            "memory_request": row.get("memory_request", "512Mi"),
            "cpu_limit": row.get("cpu_limit", "1000m"),
            "memory_limit": row.get("memory_limit", "1Gi"),
    ...
    def _build_row_for_create(
        ...
        return {
            ...
            "cpu_request": body.cpu_request.strip(),
            "memory_request": body.memory_request.strip(),
            "cpu_limit": body.cpu_limit.strip(),
            "memory_limit": body.memory_limit.strip(),
