...
def _child_env_common(
    *,
    ...
    out["CLAWMANAGER_RABBITMQ_URL"] = settings.rabbitmq_url or ""
    out["CLAWMANAGER_MANAGER_ID"] = settings.manager_id
...
async def provision_local_jiuwenclaw(
    ...
    *,
    ...
    if not settings.allow_local_provision:
        raise ValueError("local provision disabled (set CLAWMANAGER_ALLOW_LOCAL_PROVISION=true)")
    if not settings.rabbitmq_url:
        raise ValueError("CLAWMANAGER_RABBITMQ_URL is required for heartbeat consumption")
