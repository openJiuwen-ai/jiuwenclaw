...
UpdateRecordFn = Callable[[DBHandler, int, Any, str], Awaitable[dict[str, Any] | None]]
...
async def delete_record_for_instance(
    ...
    existing = await get_row_for_instance(handler, table, row_id, jiuwenclaw_id)
    if existing is None:
        return False
...
async def apply_update_by_id(
    ...
    *,
    ...
    row = await update_record(handler, int(row_id), updates, jiuwenclaw_id)
    if row is None:
...
async def apply_delete_by_id(
    ...
    *,
    ...
    await delete_record_for_instance(handler, table, int(row_id), jiuwenclaw_id)
