...
"""Permissions 配置 WebSocket 同步：写入 Gateway 本地库并热更新权限引擎。"""
...
async def apply_permissions_config(payload: dict[str, Any]) -> dict[str, Any] | None:
    """应用 Claw Manager 经 WebSocket 下发的 permissions_config 变更。"""
    logger.info(
        "[ManagerWsClient] apply_permissions_config payload_keys=%s",
        list(payload.keys()),
    )
    op = str(payload.get("op") or "").strip()
    if not op:
        raise ValueError("permissions_config.op is required")
    ...
    if op == "upsert":
        ...
        _apply_permissions(body, op=op)
        logger.info(
            "[ManagerWsClient] permissions_config hot-reload upsert revision=%s",
            (result or {}).get("revision"),
        )

    elif op == "delete":
        await handler.delete(_TABLE, {"jiuwenclaw_id": jiuwenclaw_id})
        _apply_permissions(None, op=op)
        result = None
        logger.info("[ManagerWsClient] permissions_config deleted, reverted to yaml fallback")

    else:
        raise ValueError(f"unsupported permissions_config.op: {op!r}")
