...
def _binary_name() -> str:
    return "jiuwenavatar-tui.exe" if os.name == "nt" else "jiuwenavatar-tui"
...
def _source_dist_entry_path() -> Path | None:
    ...
    for package_dir in ("jiuwenavatar", "jiuwenclaw"):
...
def main() -> None:
    ...
    if not binary.exists():
        raise SystemExit(
            "\n".join(
                [
                    f"jiuwenavatar-tui binary not found for platform: {_platform_tag()}",
